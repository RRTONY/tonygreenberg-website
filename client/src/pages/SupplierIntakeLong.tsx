/**
 * Stage 2 Supplier Intake — /supplier-intake-long/:token
...
/* ─── Pricing rows component ─── */
type PricingRow = { compound: string; unitSize: string; pricePerUnit: string; moqTier: string };
const EMPTY_ROW: PricingRow = { compound: "", unitSize: "", pricePerUnit: "", moqTier: "" };
...
function PricingRows({
  ...
  // Serialize/deserialize as pipe-delimited rows: "compound|unitSize|pricePerUnit|moqTier"
  const deserialize = (raw: string): PricingRow[] => {
    if (!raw) return [{ ...EMPTY_ROW }];
    const rows = raw.split("\n").map((line) => {
      const [compound = "", unitSize = "", pricePerUnit = "", moqTier = ""] = line.split("|");
      return { compound, unitSize, pricePerUnit, moqTier };
    });
    return rows.length ? rows : [{ ...EMPTY_ROW }];
  };
  const serialize = (rows: PricingRow[]) =>
    rows.map((r) => `${r.compound}|${r.unitSize}|${r.pricePerUnit}|${r.moqTier}`).join("\n");

  const [rows, setRows] = useState<PricingRow[]>(() => deserialize(value));

  useEffect(() => {
    onChange(serialize(rows));
  }, [rows]);

  const setRow = (i: number, field: keyof PricingRow) => (v: string) => {
  ...
  const rowInputClass =
    "bg-[#111118] border border-neutral-700 text-neutral-200 placeholder:text-neutral-600 rounded-lg focus:border-[#D4B96A] focus:ring-1 focus:ring-[#D4B96A]/30 transition-colors h-9 text-sm px-3";

  return (
    <div>
      <label className="block text-sm font-mono tracking-wider text-[#D4B96A] mb-1.5">
        Pricing for Top Compounds <span className="text-[#D4B96A]/60">*</span>
      </label>
      <p className="text-xs text-neutral-500 mb-3">
        Add up to 5 rows — one per compound. All columns are optional but more detail helps.
      </p>
      <div className="space-y-2">
        <div className="grid grid-cols-[1fr_1fr_1fr_1fr_auto] gap-2 mb-1">
          {["Compound", "Unit Size", "Price / Unit", "MOQ Tier"].map((h) => (
            <span key={h} className="text-xs font-mono text-neutral-500 uppercase tracking-wide">{h}</span>
          ))}
          <span />
        </div>
        {rows.map((row, i) => (
          <div key={i} className="grid grid-cols-[1fr_1fr_1fr_1fr_auto] gap-2 items-center">
            <input className={rowInputClass} placeholder="BPC-157" value={row.compound} onChange={(e) => setRow(i, "compound")(e.target.value)} />
            <input className={rowInputClass} placeholder="1g" value={row.unitSize} onChange={(e) => setRow(i, "unitSize")(e.target.value)} />
            <input className={rowInputClass} placeholder="$120" value={row.pricePerUnit} onChange={(e) => setRow(i, "pricePerUnit")(e.target.value)} />
            <input className={rowInputClass} placeholder="10g min" value={row.moqTier} onChange={(e) => setRow(i, "moqTier")(e.target.value)} />
            <button
              type="button"
              onClick={() => removeRow(i)}
              disabled={rows.length <= 1}
              className="p-1.5 rounded text-neutral-500 hover:text-red-400 hover:bg-red-400/10 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
      ...
      {rows.length < 5 && (
        <button
          type="button"
          onClick={addRow}
          className="mt-3 flex items-center gap-1.5 text-xs font-mono text-[#D4B96A]/70 hover:text-[#D4B96A] transition-colors"
        >
...
function MultiUploadField({
  ...
  // value is a newline-joined list of S3 URLs
    ...
    try {
      ...
      for (const file of files) {
        const { url } = await getUploadUrl.mutateAsync({
          ...
          fileName: file.name,
          fileType: file.type,
          fieldName: fieldName as "upload_coa",
        });
        const res = await fetch(url, {
          method: "PUT",
          body: file,
  ...
  return (
    <Field label={label} sub={sub} error={error}>
      <div className="space-y-2">
        {urls.map((u, i) => (
          <div key={i} className="flex items-center gap-2 bg-[#D4B96A]/5 border border-[#D4B96A]/30 rounded-lg px-3 py-2">
            <CheckCircle2 className="w-4 h-4 text-[#D4B96A] shrink-0" />
            <span className="text-sm text-neutral-300 truncate flex-1">{u.split("/").pop()}</span>
            <button type="button" onClick={() => removeUrl(i)} className="p-1 rounded text-neutral-500 hover:text-red-400 hover:bg-red-400/10 transition-colors">
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
        ))}
        <label className="flex items-center gap-2 cursor-pointer border border-neutral-700 rounded-lg px-4 py-2.5 bg-[#111118] hover:border-[#D4B96A]/40 transition-colors">
          {uploading ? (
            <Loader2 className="w-4 h-4 text-[#D4B96A] animate-spin" />
          ) : (
            <Paperclip className="w-4 h-4 text-neutral-500" />
          )}
          <span className="text-sm text-neutral-400">
            {uploading ? "Uploading…" : urls.length ? "Add more files" : "Choose files"}
          </span>
          <input
            type="file"
            multiple
...
/* ─── Main component ─── */
export default function SupplierIntakeLong() {
  const [location] = useLocation();
  // Support both /supplier-intake-long/:token (path param) and ?token= (query param)
  const pathToken = location.split("/supplier-intake-long/")[1]?.split("?")[0] ?? "";
  ...
  function validateStep(stepKey: StepKey): boolean {
    ...
    // fda_registration_number: optional, but if provided must be 6-10 digits (strip dashes/spaces)
    if (stepKey === "regulatory" && form.fda_registration_number.trim()) {
      const digits = form.fda_registration_number.replace(/[\s-]/g, "");
      if (!/^\d{6,10}$/.test(digits)) {
        newErrors.fda_registration_number = "FDA Registration Number must be 6–10 digits (dashes and spaces are ignored)";
      }
    }
      ...
      <div className="min-h-screen bg-[#0A0A10] flex items-center justify-center px-4">
        <div className="max-w-xl w-full text-center">
          ...
          <p className="text-neutral-400 mb-6 leading-relaxed">
              Thank you for completing your supplier profile. Our team will review your
  ...
  const renderStep = () => {
    switch (step) {
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <Field label="Year Founded" required error={errors.year_founded}>
              <Select value={form.year_founded} onValueChange={set("year_founded")}>
                <SelectTrigger className={`${errors.year_founded ? inputErrorClass : inputClass} w-full`}>
                  <SelectValue placeholder="Select year" />
                </SelectTrigger>
                <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                  {Array.from({ length: new Date().getFullYear() - 1950 + 1 }, (_, i) => String(new Date().getFullYear() - i)).map((yr) => (
                    <SelectItem key={yr} value={yr} className="hover:bg-neutral-800 focus:bg-neutral-800">{yr}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </Field>
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <Field label="Peptide Synthesis Method" required error={errors.peptide_synthesis_method}>
                <Select value={form.peptide_synthesis_method} onValueChange={set("peptide_synthesis_method")}>
                  <SelectTrigger className={`${errors.peptide_synthesis_method ? inputErrorClass : inputClass} w-full`}>
                    <SelectValue placeholder="Select method" />
                  </SelectTrigger>
                  <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                    {["Solid Phase (SPPS)", "Liquid Phase", "Hybrid", "Contract Synthesis (third-party)"].map((o) => (
            ...
            <Field label="Sterile Fill Capability" required error={errors.sterile_fill_capability}>
              <Select value={form.sterile_fill_capability} onValueChange={set("sterile_fill_capability")}>
                ...
                <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                  {["Yes - In-house", "Yes - Contract", "In Development", "No"].map((o) => (
                    <SelectItem key={o} value={o} className="hover:bg-neutral-800 focus:bg-neutral-800">{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <Field label="Quality Management System" required error={errors.quality_management_system}>
              <Select value={form.quality_management_system} onValueChange={set("quality_management_system")}>
                ...
                <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                  {["ISO 9001:2015", "cGMP (FDA 21 CFR)", "Both ISO + cGMP", "Internal QMS only", "None"].map((o) => (
                    <SelectItem key={o} value={o} className="hover:bg-neutral-800 focus:bg-neutral-800">{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <Field label="Third-Party Testing" required error={errors.third_party_testing}>
              <Select value={form.third_party_testing} onValueChange={set("third_party_testing")}>
                ...
                <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                  {["All batches", "Representative samples only", "On request only", "None currently"].map((o) => (
                    <SelectItem key={o} value={o} className="hover:bg-neutral-800 focus:bg-neutral-800">{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <Field label="Stability Testing Program" required error={errors.stability_testing_program}>
              <Select value={form.stability_testing_program} onValueChange={set("stability_testing_program")}>
                ...
                <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                  {["ICH guidelines", "Accelerated only", "Real-time only", "None"].map((o) => (
                    <SelectItem key={o} value={o} className="hover:bg-neutral-800 focus:bg-neutral-800">{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="md:col-span-2">
              <Field
                ...
                sub="Describe every protocol you run - mentioning mass spectrometry / LC-MS / HPLC-MS earns bonus scoring points."
              >
                <Textarea
                  value={form.testing_protocols}
                  onChange={(e) => set("testing_protocols")(e.target.value)}
                  placeholder="Describe every protocol you run - mentioning mass spectrometry / LC-MS / HPLC-MS earns bonus scoring points."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <Field label="Payment Terms" required error={errors.payment_terms}>
              <Select value={form.payment_terms} onValueChange={set("payment_terms")}>
                ...
                <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                  {["Net 30", "Net 60", "50% upfront", "50% on delivery", "100% upfront", "Other"].map((o) => (
                    <SelectItem key={o} value={o} className="hover:bg-neutral-800 focus:bg-neutral-800">{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="md:col-span-2">
              <PricingRows
                ...
                onChange={set("pricing_for_top_compounds")}
                error={errors.pricing_for_top_compounds}
              />
            </div>
            <div className="md:col-span-2">
              <MultiUploadField
                label="Full Price List / Catalog"
                sub="Optional — upload your full price list or catalog (PDF, Excel, Word). Multiple files accepted."
                fieldName="full_price_list_catalog"
                token={token}
                value={form.full_price_list_catalog}
                onChange={set("full_price_list_catalog")}
              />
            </div>
          </div>
        );
      ...
      case "regulatory":
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <Field label="FDA Registration Number" sub="Optional — 6–10 digits; dashes and spaces are stripped" error={errors.fda_registration_number}>
              <Input
                value={form.fda_registration_number}
                onChange={(e) => set("fda_registration_number")(e.target.value)}
                placeholder="e.g. 3004567890"
                className={errors.fda_registration_number ? inputErrorClass : inputClass}
              />
            </Field>
            <Field label="DEA Registration" required error={errors.dea_registration}>
              <Select value={form.dea_registration} onValueChange={set("dea_registration")}>
                ...
                <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                  {["Yes - Schedule III/IV/V", "In Progress", "No"].map((o) => (
                    <SelectItem key={o} value={o} className="hover:bg-neutral-800 focus:bg-neutral-800">{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <Field label="Buyer Eligibility" required error={errors.buyer_eligibility}
              ...
              <Select value={form.buyer_eligibility} onValueChange={set("buyer_eligibility")}>
                ...
                <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                  {["Institutional/researcher only", "Clinics only", "Clinics and individuals", "All accounts"].map((o) => (
                    <SelectItem key={o} value={o} className="hover:bg-neutral-800 focus:bg-neutral-800">{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <Field label="Shipping Jurisdictions" required error={errors.shipping_jurisdictions}
              sub="List all countries and regions you ship to">
              <Textarea
                value={form.shipping_jurisdictions}
                onChange={(e) => set("shipping_jurisdictions")(e.target.value)}
                placeholder="United States, Canada, European Union, United Kingdom..."
                className={errors.shipping_jurisdictions ? textareaErrorClass : textareaClass}
              />
            </Field>
            ...
            <Field label="FDA Inspection Outcome" sub="Optional">
              <Select value={form.fda_inspection_outcome} onValueChange={set("fda_inspection_outcome")}>
                ...
                <SelectContent className="bg-[#111118] border border-neutral-700 text-neutral-200">
                  {["NAI", "VAI", "OAI", "Never Inspected"].map((o) => (
                    <SelectItem key={o} value={o} className="hover:bg-neutral-800 focus:bg-neutral-800">{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
  ...
  return (
    <div className="min-h-screen bg-[#0A0A10] text-neutral-200">
      ...
      <div className="max-w-3xl mx-auto px-6 pt-6">
        <BioChainCTA variant="supplier" context="The canonical supplier intake is at ramprate.com/biochain. This form feeds the same review pipeline." />
      </div>
