...
export function computeWow(item: CatalogItem): number {
  ...
  if (["anaerobic", "carbonic", "koji", "thermal-shock"].some(p =>
    item.processingMethod.toLowerCase().includes(p))) score += 8;
...
export function scoreAll(items: CatalogItem[], weights: ScoringWeights): CatalogItem[] {
  return items.map(item => {
    ...
    const overall = Math.round(
      qpr * weights.qpr + availability * weights.availability +
      wow * weights.wow + scarcity * weights.scarcity
    );
    const scores: ComputedScores = {
      qpr, availability, scarcity, wow,
...
export function gradeColor(g: string): string {
  return { A: "#22c55e", B: "#84cc16", C: "#eab308", D: "#f97316", F: "#ef4444" }[g] ?? "#9ca3af";
}
