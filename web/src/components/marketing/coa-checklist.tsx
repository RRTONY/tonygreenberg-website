...
function getScoreLabel(score: number): { label: string; color: string; description: string } {
  if (score === 7)
    return { label: "Clean", color: "border-emerald-200 bg-emerald-50 text-emerald-700", description: "This vendor is doing it right." };
  if (score >= 5)
    return { label: "Questionable", color: "border-amber-200 bg-amber-50 text-amber-700", description: "Ask for the missing items before ordering." };
  if (score >= 3)
    return { label: "Red Flag", color: "border-orange-200 bg-orange-50 text-orange-700", description: "Don't order until gaps are resolved." };
  return { label: "Walk Away", color: "border-red-200 bg-red-50 text-red-700", description: "This is not a vendor to trust with anything that ends up in your body." };
}
