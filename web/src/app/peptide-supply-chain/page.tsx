...
export default function PeptideSupplyChainPage() {
  const avgMarketing = Math.round(PROVIDERS.reduce((s, p) => s + p.breakdown.marketing, 0) / PROVIDERS.length);
  const avgManufacturing = Math.round(PROVIDERS.reduce((s, p) => s + p.breakdown.manufacturing, 0) / PROVIDERS.length);
  const avgProfit = Math.round(PROVIDERS.reduce((s, p) => s + p.breakdown.profit, 0) / PROVIDERS.length);
  const avgPhysician = Math.round(PROVIDERS.reduce((s, p) => s + p.breakdown.physician, 0) / PROVIDERS.length);
    ...
    <div>
      ...
      <section className="bg-linear-to-br from-[#0A0A10] to-[#1a1a2e] px-6 py-24 text-center">
        <div className="mx-auto max-w-2xl">
          ...
          <p className="mx-auto mb-8 max-w-lg leading-relaxed text-[#E8E4DC]/60">
            ...
            dollar you spend, here&apos;s where it actually goes — and what that tells you about
            who to trust.
          </p>
      ...
      <div className="mx-auto max-w-4xl px-6 py-12 sm:px-10">
        ...
        <p className="mb-6 leading-relaxed text-foreground/80">
          ...
          oversight = they&apos;re investing in the product and your safety. High marketing + high
          profit = you&apos;re funding their Instagram ads and shareholder returns.
        </p>

        <div className="mb-12 grid gap-3 sm:grid-cols-2">
          {COST_CATEGORIES.map((cat) => (
            <div key={cat.key} className="flex items-start gap-2.5 rounded-md border border-brand-gold/10 bg-card p-3">
              <span
        ...
        <div className="mt-12 rounded-md bg-brand-gold/5 p-6">
          <p className="text-sm leading-relaxed text-muted-foreground">
            ...
            allocation (25%), physician oversight (25%), R&amp;D investment (20%), compliance
            spend (15%), and transparency (15%). This page is for educational purposes only and
            does not constitute financial or medical advice. Data current as of February 2026.
          </p>
        </div>
      </div>
    </div>
