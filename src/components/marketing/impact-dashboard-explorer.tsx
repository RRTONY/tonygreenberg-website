  ...
  return (
    <div
      ...
      className={`rounded-2xl border border-brand-gold/15 bg-card/60 backdrop-blur-xl transition-all duration-300 ${
        onClick || interactive
          ? "cursor-pointer hover:-translate-y-1 hover:border-brand-gold/50 hover:shadow-lg"
          : ""
      } ${className}`}
...
function SoulScoreLink({ label }: { label: string }) {
  return (
    ...
    >
      {label} <ForwardIcon aria-hidden="true" />
    </Link>
  );
}
      ...
      <main className="mx-auto max-w-6xl px-6 py-10 sm:px-10">
            ...
            {[
              {
                ...
                value: `${AGGREGATE.totalTokenHolders.toLocaleString()}+`,
                sub: "Across 4 ecosystems",
                color: "#8B6914",
              },
              {
                label: "Impact Capital Deployed",
                value: AGGREGATE.totalImpactDeployed,
                sub: "Direct regenerative funding",
                color: "#27AE60",
              },
              {
                label: "Projects Funded",
                value: AGGREGATE.totalProjectsFunded.toString(),
                sub: "Ocean, education, digital, mental health",
                color: "#0077B6",
              },
              {
                label: "Community Members",
                value: `${AGGREGATE.totalCommunityMembers.toLocaleString()}+`,
                sub: "Active participants worldwide",
                color: "#6C5CE7",
              },
              {
                label: "Avg Hawkins Score",
                value: AGGREGATE.avgHawkins.toString(),
                sub: "Above integrity threshold (200+)",
                color: "#D4B96A",
              },
              {
                label: "Portfolio Companies",
                value: `${AGGREGATE.portfolioCompanies}+`,
                sub: "Psychedelic, impact, Web3, health",
                color: "#9B59B6",
              },
            ].map((kpi) => (
              <GlassCard key={kpi.label} className="p-6">
                <div className="mb-2 font-mono text-[0.6rem] tracking-wide text-muted-foreground uppercase">
                  {kpi.label}
                </div>
                ...
                >
                  ...
                  <div className="flex justify-between font-mono text-xs">
                    ...
                    <span style={{ color: token.color }}>
                      {token.metrics.impactMultiplier} multiplier
                    </span>
                  </div>
                </GlassCard>
              );
            ...
            <Link href="/charity-scorecard">
              <GlassCard className="p-5" interactive>
                ...
                <div className="text-sm text-muted-foreground">
                  100 charities, 8 evaluators unified
                </div>
              </GlassCard>
            </Link>
            <Link href="/intel">
              <GlassCard className="p-5" interactive>
                ...
                <div className="text-sm text-muted-foreground">
                  35+ companies, Hawkins-calibrated
                </div>
              </GlassCard>
            </Link>
            <Link href="/invest">
        ...
        <div className={activeTab === "Token Ecosystems" ? "" : "hidden"}>
          ...
          <p className="mb-8 max-w-2xl leading-relaxed text-muted-foreground">
            ...
            holders who create impact directly or grow the asset to unlock the next donation. Every
            ABIT is backed by a real asset, structured for impact, and designed to compound
            regeneratively.
          </p>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      {
                        label: "Community",
                        value: token.metrics.communityMembers.toLocaleString(),
                      },
                      ...
                      <div
                        ...
                        style={{ background: `${token.color}10`, borderColor: `${token.color}30` }}
                      >
                  ...
                  {isExpanded && (
                    <div className="mt-4 border-t border-brand-gold/10 pt-4">
                      <div className="mb-4 grid grid-cols-2 gap-4">
                        <div>
                          <div className="mb-1 font-mono text-[0.6rem] tracking-wide text-muted-foreground uppercase">
                            Partner NGO
                          </div>
                      ...
                      <div className="mt-4 flex gap-6">
                        <div className="text-center">
                          <div className="font-heading text-lg font-bold text-brand-gold">
                            {token.hawkinsScore}
                          </div>
                          <div className="font-mono text-[0.55rem] text-muted-foreground">
                            HAWKINS
        ...
        <div className={activeTab === "Portfolio Impact" ? "" : "hidden"}>
            ...
            {PORTFOLIO_CATEGORIES.map((cat) => {
              ...
              return (
                <GlassCard key={cat.name} className="p-6" accent={cat.color}>
                  ...
                  />
                  <div
                    className="mt-3 rounded-md p-2.5 font-mono text-xs"
          ...
          <GlassCard className="p-6">
            ...
            >
              View Full Portfolio Intel <ForwardIcon aria-hidden="true" />
            </Link>
          </GlassCard>
        </div>
              ...
              <table className="w-full border-collapse font-mono text-xs">
                ...
                <tbody>
                  {BENCHMARK_COMPARISON.map((row) => (
                    <tr
                      key={row.name}
                      className={`border-b border-brand-gold/8 ${row.name.includes("ImpactSoul") ? "bg-brand-gold/8" : ""}`}
                    >
                      <td
                        className={`p-2.5 text-foreground ${row.name.includes("ImpactSoul") ? "font-bold" : ""}`}
                      >
                        {row.name}
                      </td>
        ...
        <div className={activeTab === "Charity Index" ? "" : "hidden"}>
          ...
          <p className="mb-8 max-w-2xl leading-relaxed text-muted-foreground">
            One hundred charities. Eight existing scorecards unified. Seven new dimensions measured.
            The first accountability framework that asks the question every donor deserves answered:
          ...
          <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {[
              {
                label: "Charities Scored",
                value: CHARITY_SUMMARY.totalCharities.toString(),
                color: "#8B6914",
                Icon: BarChart3,
              },
              {
                label: "Avg Composite Score",
                value: CHARITY_SUMMARY.avgScore.toString(),
                color: "#27AE60",
                Icon: TrendingUp,
              },
              {
                label: "Evaluators Unified",
                value: CHARITY_SUMMARY.evaluatorsUnified.toString(),
                color: "#3498DB",
                Icon: Link2,
              },
              {
                label: "Scoring Dimensions",
                value: CHARITY_SUMMARY.dimensions.toString(),
                color: "#9B59B6",
                Icon: Target,
              },
              {
                label: "Sectors Covered",
                value: CHARITY_SUMMARY.sectors.toString(),
                color: "#E67E22",
                Icon: Globe,
              },
            ].map((stat) => (
          ...
          <GlassCard className="mb-6 p-6">
            ...
            <div className="mb-2 flex h-10 gap-1.5 overflow-hidden rounded-lg">
              <div
                className="flex items-center justify-center font-mono text-xs font-bold text-white"
                style={{ flex: CHARITY_SUMMARY.clearTransparency, background: "#27AE60" }}
              >
                {CHARITY_SUMMARY.clearTransparency}%
              </div>
              <div
                className="flex items-center justify-center font-mono text-xs font-bold text-white"
                style={{ flex: CHARITY_SUMMARY.hazyTransparency, background: "#E67E22" }}
              >
                {CHARITY_SUMMARY.hazyTransparency}%
              </div>
              <div
                className="flex items-center justify-center font-mono text-xs font-bold text-white"
                style={{ flex: CHARITY_SUMMARY.opaqueTransparency, background: "#E74C3C" }}
              >
          ...
          <GlassCard className="p-6">
            ...
            <Link
              ...
              className="border-b border-brand-gold/30 font-mono text-xs tracking-wide text-brand-gold"
            >
              View Full Charity Scorecard <ForwardIcon aria-hidden="true" />
            </Link>
          </GlassCard>
        </div>
            ...
            {IRR_HIGHLIGHTS.map((item) => (
              <GlassCard key={item.entity} className="p-6" accent={item.color}>
                ...
                <div
                  ...
                  style={{ background: `${item.color}10`, color: "var(--muted-foreground)" }}
                >
                  {item.metric}
                </div>
              </GlassCard>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {CONSCIOUSNESS_ZONES.map((z) => (
                <div
                  ...
                  style={{ background: `${z.color}0F`, borderColor: `${z.color}30` }}
                >
                  <div
                    className="mb-1 font-mono text-xs font-bold tracking-wide"
                    style={{ color: z.color }}
                  >
                    {z.zone}
                  </div>
