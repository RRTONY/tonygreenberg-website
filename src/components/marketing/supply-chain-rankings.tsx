...
function TransparencyTag({ level }: { level: Provider["transparency"] }) {
  const label = level === "high" ? "High Transparency" : level === "medium" ? "Medium Transparency" : level === "low" ? "Low Transparency" : "Opaque";
  return (
    <span className={`rounded px-1.5 py-0.5 font-mono text-[0.55rem] tracking-wide uppercase ${TRANSPARENCY_CLASSES[level]}`}>
      {label}
    </span>
  );
}
          ...
          {(
            ...
            <button
              ...
              className={`rounded border px-3 py-1.5 font-mono text-xs tracking-wide uppercase ${
                sortBy === s.key ? "border-brand-gold bg-brand-gold/10 text-brand-gold" : "border-border text-muted-foreground"
              }`}
            >
              {s.label}
            </button>
            ...
            >
              ...
              >
                <div className="flex flex-wrap items-center gap-3">
                  <span className={`min-w-7 font-mono text-sm font-bold ${rank <= 3 ? "text-brand-gold" : "text-muted-foreground"}`}>
                    #{rank}
                  </span>
                ...
                <div className="border-t border-border p-5 pt-4">
                  ...
                  <div className="mb-6 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                    {COST_CATEGORIES.map((cat) => {
                      ...
                      return (
                        <div key={cat.key} className="rounded-md border border-border p-2 text-center">
                          <div className="font-mono text-[0.55rem] tracking-wide text-muted-foreground uppercase">
                            {cat.label}
                          </div>
                          <div className="font-heading text-lg font-bold" style={{ color: cat.color }}>
                            ${dollars}
                          </div>
                  ...
                  <p className="mb-4 text-sm leading-relaxed text-foreground/80">{provider.notes}</p>
