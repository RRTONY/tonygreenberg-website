...
export function GradeBadge({ grade, size = "sm" }: { grade: Grade; size?: "sm" | "lg" }) {
  const sizeClass =
    size === "lg" ? "px-4 py-2 text-2xl font-bold" : "px-2 py-0.5 text-xs font-bold";
  return (
    <span
      className={`inline-flex items-center rounded border font-mono ${GRADE_CLASSES[grade]} ${sizeClass}`}
    >
      {grade}
    </span>
  );
...
const CLOAK_CONFIG: Record<
  CloakStatus,
  { icon: typeof Eye; label: string; cls: string; desc: string }
> = {
  clear: {
    icon: Eye,
    label: "Clear",
    cls: "text-emerald-600",
    desc: "Full transparency. Open data. Nothing hidden.",
  },
  "mostly-clear": {
    icon: Eye,
    label: "Mostly Clear",
    cls: "text-blue-600",
    desc: "Good disclosure with minor gaps.",
  },
  hazy: {
    icon: EyeOff,
    label: "Hazy",
    cls: "text-amber-600",
    desc: "Significant information gaps.",
  },
  opaque: {
    icon: EyeOff,
    label: "Opaque",
    cls: "text-red-600",
    desc: "Material information withheld.",
  },
};
...
export function CloakPanel({ status }: { status: CloakStatus }) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 rounded-lg border px-3 py-2 ${CLOAK_PANEL_CLASSES[status]}`}
    >
...
export function evaluatorBadges(charity: {
  ...
  if (charity.charityWatch) {
    badges.push({
      ...
      cls: charity.charityWatch.startsWith("A")
        ? "bg-emerald-100 text-emerald-800"
...
export function EvaluatorBadges({
  ...
  if (badges.length === 0)
    return <span className="text-xs text-muted-foreground italic">Not rated</span>;
  return (
    <div className="flex flex-wrap gap-1">
      {badges.map((b) => (
        <span
          key={b.label}
          className={`inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[10px] font-medium ${b.cls}`}
        >
          {b.label}: {b.value}
        </span>
      ))}
