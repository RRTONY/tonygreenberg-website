...
import { Brain, Cog, Flame, Orbit, Sparkles, Sprout, Zap, type LucideIcon } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
export type JourneyIconKey =
  "thinker" | "operator" | "impact" | "body" | "crusade" | "relationship" | "all";
export type Journey = {
  id: string;
  icon: JourneyIconKey;
  title: string;
...
const JOURNEY_ICONS: Record<JourneyIconKey, LucideIcon> = {
  thinker: Brain,
  operator: Cog,
  impact: Sprout,
  body: Zap,
      ...
      {journeys.map((journey) => {
        ...
        return (
          <AccordionItem
            key={journey.id}
            value={journey.id}
            className="mb-4 rounded-lg border border-border px-6"
          >
            <AccordionTrigger className="py-6 hover:no-underline">
              <div className="flex gap-4 text-left">
                <Icon aria-hidden="true" className="mt-0.5 size-7 shrink-0 text-brand-gold" />
                <span>
