...
export function AbitWaitlistForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const body = encodeURIComponent(
      `Please add me to the ABIT (Asset-Backed Impact Tokens) waitlist.\n\nEmail: ${email.trim()}`,
    );
  ...
  return (
    <div className="max-w-120">
      ...
      <form
        ...
        className="flex overflow-hidden rounded-sm border border-brand-gold-light/35 bg-white/6 backdrop-blur-sm"
      >
        <input
          type="email"
          value={email}
        ...
        <button
          type="submit"
          className="shrink-0 bg-brand-gold/85 px-5 py-2.5 font-mono text-xs font-semibold tracking-[0.08em] text-white uppercase transition-colors hover:bg-brand-gold"
        >
          Join Waitlist
        </button>
      </form>
