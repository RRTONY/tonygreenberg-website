...
export async function GemSparkStrip() {
  const gemSparkSlugs = Object.keys(GEM_SPARK_SUBTITLES);
  const posts = (await getPostsBySlugs(gemSparkSlugs)).map((post) => ({
      ...
      <div className="mx-auto flex max-w-6xl flex-col flex-wrap items-start gap-8 sm:flex-row">
        <div className="w-full min-w-0 flex-1 sm:min-w-55 sm:basis-70">
          ...
          <p className="max-w-[340px] text-sm text-[#5C3A1E] dark:text-[#D9C79E]">
            ...
            ideas — that illuminates something larger. Not a theory. Not a framework. A spark. Every
            essay Tony publishes includes one.
          </p>
        </div>
        <div className="flex w-full min-w-0 flex-2 flex-col gap-3 sm:basis-100">
          ...
          {posts.map((post) => {
            ...
            return (
              ...
              >
                ...
                <ArrowRight aria-hidden="true" className="ml-auto size-4 shrink-0 text-[#C8860A]" />
              </Link>
            );
          })}
        </div>
