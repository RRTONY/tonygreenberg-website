...
function RiskChip({ risk }: { risk: RiskLevel }) {
  return (
    <span className={`inline-block rounded px-2 py-0.5 text-xs font-bold ${RISK_COLORS[risk]}`}>
      {risk}
    </span>
  );
}
...
export function PeptideLegalTable({ data }: { data: PeptideRegEntry[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "medium")
      items = items.filter((d) => ["LOW-MEDIUM", "MEDIUM"].includes(d.userRisk));
    if (filter === "high") items = items.filter((d) => ["HIGH", "VERY HIGH"].includes(d.userRisk));
    if (search)
      items = items.filter((d) => d.compound.toLowerCase().includes(search.toLowerCase()));
    return items;
  }, [data, filter, search]);
