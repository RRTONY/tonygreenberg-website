    ...
    <section className="mb-12">
      ...
      {expanded && (
        <div>
          ...
          {selected && (
            <div
              className={`mt-6 rounded-xl border bg-[#0A0A10] p-6 ${RISK_BORDER_CLASSES[selected.riskLevel]}`}
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 font-mono text-xs tracking-wide text-[#E8E4DC]/40 uppercase">
                    {selected.type} · Tier {selected.tier ?? "N/A"} · Wall Score{" "}
                    {selected.wallScore ?? "N/A"}
                  </p>
                </div>
                <span
                  className={`rounded-full px-3 py-1 font-mono text-xs font-bold ${RISK_BADGE_CLASSES[selected.riskLevel]}`}
                >
      ...
      <h2 className="mt-12 mb-6 font-heading text-2xl font-bold text-foreground">
        All Entities Ranked
      </h2>
            ...
            {rankedEntities.map((entity) => (
              ...
              >
                ...
                <td className="px-2 py-2.5">
                  <span
                    className={`rounded px-1.5 py-0.5 font-mono text-xs ${RISK_BADGE_CLASSES[entity.riskLevel]}`}
                  >
                    {entity.quadrant}
                  </span>
                </td>
                <td className="px-2 py-2.5">
                  <span
                    className={`rounded-full px-2 py-0.5 font-mono text-[0.65rem] font-bold ${RISK_BADGE_CLASSES[entity.riskLevel]}`}
                  >
                    {entity.riskLevel}
                  </span>
                </td>
