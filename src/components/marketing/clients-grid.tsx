  ...
  return (
    <button
      ...
      className={`rounded-sm border px-3 py-1.5 font-mono text-xs tracking-wide ${
        active ? "border-brand-gold bg-brand-gold text-white" : "border-border text-muted-foreground"
      }`}
    >
      {children}
    </button>
  ...
  return (
    <div>
      <div className="mb-3 flex flex-wrap items-center gap-3">
        <span className="mr-1 font-mono text-xs tracking-wide text-brand-gold uppercase">Filter:</span>
      ...
      <div className="mb-8 flex flex-wrap items-center gap-3">
        <span className="mr-1 font-mono text-xs tracking-wide text-brand-gold uppercase">Sort:</span>
              ...
              <div className="flex items-center gap-2">
                <div className="h-1 flex-1 overflow-hidden rounded-full bg-muted">
                  <div
                    ...
                    style={{ width: `${Math.min((client.hawkins / 500) * 100, 100)}%`, background: level.color }}
                  />
                </div>
                <span className="min-w-8 shrink-0 text-right font-mono text-xs" style={{ color: level.color }}>
                  {client.hawkins}
                </span>
              </div>
              <p className="mt-1 font-mono text-xs tracking-wide uppercase" style={{ color: level.color }}>
                {level.label}
              </p>
            </div>
          );
