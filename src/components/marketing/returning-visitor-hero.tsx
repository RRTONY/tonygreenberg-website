...
// Component reading the visit counter cookie and the server-side reading
// state instead of legacy's browser storage. The banner renders correct
// markup on the next request with no post-hydration flash. Legacy's
...
export async function ReturningVisitorHero() {
  ...
  const readingState = await getVisitorState("reading");
  const lastSlug = typeof readingState?.slug === "string" ? readingState.slug : undefined;
  const lastTitle = typeof readingState?.title === "string" ? readingState.title : undefined;
    ...
    <div className="border-b border-brand-gold-light/10 bg-brand-gold-light/4 px-3 py-1.5">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2 overflow-hidden">
          <span className="shrink-0 font-heading text-[0.82rem] text-brand-gold-light">
            Welcome back.
          </span>
          {lastSlug && truncTitle && (
            <Link
        ...
        <Link
          href="/find-my"
          className="flex shrink-0 items-center gap-1 rounded-sm bg-brand-gold-light/10 px-2 py-0.5 font-mono text-[0.58rem] tracking-[0.06em] text-brand-gold-light uppercase"
        >
