...
export function SubmissionForm() {
    ...
    return (
      <div className="rounded-lg border border-facilitator-amber-light/35 bg-facilitator-amber-light/12 p-6 text-center text-facilitator-amber">
        ...
        <div className="text-[.88rem] text-facilitator-ink/60">
          Your email client should be open with your responses pre-filled — send it and they&apos;re
          logged under your code.
        </div>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div>
        <label className="mb-1.5 block text-xs font-bold tracking-[0.08em] text-facilitator-ink/60 uppercase">
          Your Code (optional)
        </label>
        <input
          type="text"
      ...
      <div>
        <label className="mb-1.5 block text-xs font-bold tracking-[0.08em] text-facilitator-ink/60 uppercase">
          Your Responses — paste all 108 items with your answers
        </label>
        <textarea
      ...
      <div className="mb-5">
        <label className="flex cursor-pointer items-start gap-3">
          <input
            ...
            className="mt-0.5 accent-facilitator-amber-light"
          />
          <span className="text-[.88rem] leading-[1.55] text-facilitator-ink/70">
            I&apos;d like Tony to make a personal introduction to a vetted practitioner or facility
            in my region when relevant. This is not a lead-generation list. Tony reviews these
            himself.
          </span>
        </label>
      </div>
