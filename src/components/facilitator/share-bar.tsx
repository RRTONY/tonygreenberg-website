...
const SHARE_TEXT =
  "The Facilitator Index — a philosophy-first instrument for practitioners who hold non-ordinary states.";

const BTN_CLASS =
  "flex items-center gap-1.5 rounded-lg border border-facilitator-amber-light/28 bg-facilitator-amber-light/10 px-4.5 py-2 text-[.82rem] text-facilitator-ink/80 no-underline";
...
export function FacilitatorShareBar() {
  ...
  return (
    <div className="mb-8 rounded-2xl border border-facilitator-amber-light/28 bg-white/82 p-6 shadow-[0_8px_32px_rgba(180,120,0,0.12),0_2px_8px_rgba(0,0,0,.06)] backdrop-blur-2xl">
      <div className="mb-4 font-heading text-[1.05rem] font-bold text-facilitator-ink">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <a
          href={`mailto:?subject=${encodeURIComponent("The Facilitator Index")}&body=${encodeURIComponent(SHARE_TEXT + "\n\n" + SHARE_URL)}`}
          className={BTN_CLASS}
        >
