  ...
  const bandsHtml = FACILITATOR_BANDS.map((band) => {
    const tradRows = band.traditions
      ? band.traditions
          .map(
            (t) =>
              `<div class="item"><span class="item-num">T${t.n}.</span><span class="item-text">${t.text}</span></div>`,
          )
          .join("")
      : "";
    const itemRows = band.items
      .map(
        (item) =>
          `<div class="item"><span class="item-num">${item.n}.</span><span class="item-text">${item.text}<span class="answer-line-long"></span></span></div>`,
      )
      .join("");
    return `
...
export function PrintQuestionsButton({ label }: { label: string }) {
  return (
    <button
      ...
      className="inline-flex items-center gap-1.5 rounded-lg bg-linear-to-br from-facilitator-amber-deep to-facilitator-amber-light px-6 py-2.5 text-[.82rem] font-bold tracking-[0.06em] text-facilitator-ink"
    >
