...
const LEVELS: Level[] = [
  { level: 20, name: "Shame", emotion: "Humiliation", textClass: "text-[#1a0a0a]", dotClass: "bg-[#1a0a0a]", borderLeftClass: "border-l-[#1a0a0a]" },
  { level: 30, name: "Guilt", emotion: "Blame", textClass: "text-[#2d1515]", dotClass: "bg-[#2d1515]", borderLeftClass: "border-l-[#2d1515]" },
  { level: 50, name: "Apathy", emotion: "Despair", textClass: "text-[#3d2020]", dotClass: "bg-[#3d2020]", borderLeftClass: "border-l-[#3d2020]" },
  { level: 75, name: "Grief", emotion: "Regret", textClass: "text-[#4a2828]", dotClass: "bg-[#4a2828]", borderLeftClass: "border-l-[#4a2828]" },
  { level: 100, name: "Fear", emotion: "Anxiety", textClass: "text-[#5a3030]", dotClass: "bg-[#5a3030]", borderLeftClass: "border-l-[#5a3030]" },
  { level: 125, name: "Desire", emotion: "Craving", textClass: "text-[#6b3a1a]", dotClass: "bg-[#6b3a1a]", borderLeftClass: "border-l-[#6b3a1a]" },
  { level: 150, name: "Anger", emotion: "Hate", textClass: "text-[#7a4010]", dotClass: "bg-[#7a4010]", borderLeftClass: "border-l-[#7a4010]" },
  { level: 175, name: "Pride", emotion: "Scorn", textClass: "text-[#8b5a1a]", dotClass: "bg-[#8b5a1a]", borderLeftClass: "border-l-[#8b5a1a]" },
  { level: 200, name: "Courage", emotion: "Affirmation", textClass: "text-[#3a6b30]", dotClass: "bg-[#3a6b30]", borderLeftClass: "border-l-[#3a6b30]" },
  { level: 250, name: "Neutrality", emotion: "Trust", textClass: "text-[#2e7a50]", dotClass: "bg-[#2e7a50]", borderLeftClass: "border-l-[#2e7a50]" },
  { level: 310, name: "Willingness", emotion: "Optimism", textClass: "text-[#268b5a]", dotClass: "bg-[#268b5a]", borderLeftClass: "border-l-[#268b5a]" },
  { level: 350, name: "Acceptance", emotion: "Forgiveness", textClass: "text-[#1e9a6a]", dotClass: "bg-[#1e9a6a]", borderLeftClass: "border-l-[#1e9a6a]" },
  { level: 400, name: "Reason", emotion: "Understanding", textClass: "text-[#3a6b9a]", dotClass: "bg-[#3a6b9a]", borderLeftClass: "border-l-[#3a6b9a]" },
  { level: 500, name: "Love", emotion: "Reverence", textClass: "text-[#5a4aaa]", dotClass: "bg-[#5a4aaa]", borderLeftClass: "border-l-[#5a4aaa]" },
  { level: 540, name: "Joy", emotion: "Serenity", textClass: "text-[#7a5acc]", dotClass: "bg-[#7a5acc]", borderLeftClass: "border-l-[#7a5acc]" },
  { level: 600, name: "Peace", emotion: "Bliss", textClass: "text-[#9a6aee]", dotClass: "bg-[#9a6aee]", borderLeftClass: "border-l-[#9a6aee]" },
  { level: 700, name: "Enlightenment", emotion: "Ineffable", textClass: "text-[#d4b96a]", dotClass: "bg-[#d4b96a]", borderLeftClass: "border-l-[#d4b96a]" },
];
        ...
        <div className="fixed inset-x-0 top-0 z-50">
          ...
          {liveLevel && Object.keys(answers).length > 0 && (
            <div className="flex flex-wrap items-center gap-3 bg-[#0A0A10] px-6 py-2 sm:px-10">
              <span className="font-mono text-[0.55rem] tracking-[0.15em] text-[#888] uppercase">Running average</span>
              ...
              return (
                <button
                  ...
                  className={`rounded-sm border border-l-4 px-5 py-4 text-left text-[1rem] leading-relaxed transition-all ${
                    isSelected ? "border-[#0A0A10] border-l-brand-gold-light bg-[#0A0A10] text-[#F5F0E0]" : "border-black/10 border-l-transparent bg-white/50 text-[#333]"
                  }`}
                >
                  {opt.text}
                </button>
          ...
          <div className="mt-8 flex items-center justify-between">
            ...
            <button
              ...
              className={`rounded-sm px-7 py-2.5 font-mono text-[0.7rem] tracking-[0.15em] uppercase transition-colors ${
                answers[currentQ.id] ? "bg-brand-gold text-[#F5F0E0]" : "cursor-default bg-black/10 text-[#aaa]"
              }`}
            >
  ...
  return (
    <div className="relative z-1 min-h-screen font-sans text-[#2C1810]">
      ...
      <div className="bg-[#0A0A10] px-6 pt-12 pb-8 text-center sm:px-16">
        <p className="mb-2 font-mono text-[0.65rem] tracking-[0.25em] text-brand-gold-light uppercase">Your Calibration</p>
        <div className={`font-heading text-[clamp(4rem,10vw,7rem)] leading-none ${level.textClass}`}>{totalScore}</div>
        <div className="mt-2 font-heading text-[clamp(1.5rem,3vw,2rem)] text-[#F5F0E0]">{level.name}</div>
        <div className="mt-1 font-mono text-[0.75rem] tracking-[0.15em] text-[#999]">{level.emotion}</div>
      </div>
      ...
      <section className="mx-auto max-w-3xl px-6 pb-10">
        <div className="rounded-sm bg-brand-gold/8 p-6">
          <p className="mb-4 font-mono text-[0.65rem] tracking-[0.2em] text-brand-gold uppercase">Share Your Level</p>
          <div className="flex flex-wrap gap-3">
            <button onClick={() => handleShare("x")} className="rounded-sm bg-[#0A0A10] px-5 py-2.5 font-mono text-[0.7rem] tracking-[0.1em] text-[#F5F0E0]">
              Post on X
            </button>
            <button onClick={() => handleShare("linkedin")} className="rounded-sm bg-[#0077b5] px-5 py-2.5 font-mono text-[0.7rem] tracking-[0.1em] text-[#F5F0E0]">
              Share on LinkedIn
            </button>
            <button onClick={() => handleShare("copy")} className="rounded-sm border border-brand-gold px-5 py-2.5 font-mono text-[0.7rem] tracking-[0.1em] text-brand-gold">
              Copy Link
            </button>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-3xl px-6 pb-10">
        <div className="rounded-sm bg-[#0A0A10] p-6 text-[#F5F0E0]">
          <p className="mb-3 font-mono text-[0.65rem] tracking-[0.2em] text-brand-gold-light uppercase">Take It Again in 6 Months</p>
          <p className="mb-4 text-[0.95rem] leading-[1.7] text-[#ccc]">
            David Hawkins noted that consciousness levels rarely shift quickly — but they do shift. The movement matters more than the number. Save
            your score today and return in six months to see what has changed.
          </p>
          {reminderSet ? (
            <p className="font-mono text-[0.75rem] text-brand-gold-light">Reminder saved. Come back in 6 months.</p>
          ) : (
            <button onClick={handleSetReminder} className="rounded-sm bg-brand-gold px-6 py-2.5 font-mono text-[0.7rem] tracking-[0.1em] text-[#F5F0E0]">
        ...
        <div className="grid gap-0.5">
          {[...LEVELS].reverse().map((l) => {
            ...
            return (
              ...
              >
                <span className={`w-12 shrink-0 font-mono text-[0.7rem] ${isYou ? "text-brand-gold-light" : "text-[#999]"}`}>{l.level}</span>
                <span className={`flex-1 text-[0.95rem] ${isYou ? "font-semibold text-[#F5F0E0]" : "text-[#555]"}`}>{l.name}</span>
                <span className={`font-mono text-[0.7rem] ${isYou ? "text-brand-gold-light" : "text-[#aaa]"}`}>{l.emotion}</span>
                {isYou && <span className="font-mono text-[0.6rem] text-brand-gold-light">← you</span>}
              </div>
            );
          })}
        </div>
      ...
      <section className="mx-auto max-w-3xl px-6 pb-10">
        <div className="rounded-sm bg-[#0A0A10] p-8 text-[#F5F0E0]">
          <p className="mb-4 font-mono text-[0.65rem] tracking-[0.2em] text-brand-gold-light uppercase">Tony&apos;s Note</p>
          <p className="text-[1.05rem] leading-[1.85] text-[#ccc]">
            David R. Hawkins spent decades mapping human consciousness using applied kinesiology and clinical observation. His scale is
            logarithmic — the difference between 200 and 300 is not 100 points, it is an order of magnitude of energetic force. Most of humanity
            calibrates between 150 and 250. The critical threshold is 200 (Courage), where you shift from force to power, from taking to giving,
            from contraction to expansion. The questions in this assessment are designed to reveal your actual operating level, not your
            aspirational one. The most common mistake is answering who you want to be. The most useful thing is answering who you are on a Tuesday
            at 2pm when nothing is going your way.
          </p>
        </div>
      </section>
