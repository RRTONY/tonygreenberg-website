...
const JOURNEY_PHASES: { phase: number; title: string; subtitle: string; ids: string[] }[] = [
  { phase: 1, title: "The Reckoning", subtitle: "Who you are right now", ids: ["find-your-me", "find-your-mirror", "find-your-score", "soulscore"] },
  { phase: 2, title: "The Deepening", subtitle: "What drives you beneath the surface", ids: ["find-your-purpose", "find-your-level", "find-your-spirit"] },
  {
    ...
    ids: ["find-your-chemistry", "find-your-water", "find-your-diet", "find-your-movement", "find-your-sleep", "find-your-peptide"],
  },
  {
    phase: 4,
    title: "The Connections",
    subtitle: "How you love and who you build with",
    ids: ["find-your-partner", "find-your-tribe", "find-your-team", "find-your-attachment", "find-your-love-language"],
  },
  {
    phase: 5,
    title: "The Rituals",
    subtitle: "What you pour, sip, and savor",
    ids: ["find-your-mezcal", "find-your-tequila", "find-your-sake", "find-your-coffee", "find-your-kitchen"],
  },
  {
    phase: 6,
    title: "The Integration",
    subtitle: "Systems for the whole self",
    ids: ["find-your-blueprint", "find-your-capital", "find-your-therapy", "find-your-religion", "find-your-style"],
  },
];

function ExperienceCard({ experience, isCompleted }: { experience: JourneyExperience; isCompleted: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4">
        {experience.isExternal ? (
          <a href={experience.url} target="_blank" rel="noopener noreferrer" className={actionClass}>
            {actionLabel}
          </a>
        ) : (
            ...
            <div key={phase.phase} className="relative">
              ...
              <h4
                className={`mb-0.5 font-heading text-base font-bold ${
                  isComplete ? "text-brand-gold-light" : isActive ? "text-[#E8E4DC]" : "text-[#E8E4DC]/40"
                }`}
              >
                {phase.title}
              </h4>
...
export function MyJourneyDashboard() {
  ...
  const timeInvested = useMemo(
    () => JOURNEY_MAP.filter((e) => completed.has(e.id)).reduce((sum, e) => sum + e.estimatedMinutes, 0),
    [completed],
  );
      ...
      <div className="relative z-1 min-h-screen bg-[#0A0A10] pb-16 text-[#E8E4DC]">
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            ...
            <div className="text-center">
              <svg width="120" height="120" viewBox="0 0 120 120">
                ...
                />
                <text x="60" y="58" textAnchor="middle" fill="#D4B96A" className="font-heading text-[1.8rem] font-bold">
                  {stats.pct}%
                </text>
                <text x="60" y="76" textAnchor="middle" fill="rgba(232,228,220,0.4)" className="font-mono text-[0.5rem] tracking-[0.1em] uppercase">
                  Complete
                </text>
              </svg>
            </div>
          ...
          <div className="mb-8 grid grid-cols-2 gap-4 border-y border-brand-gold-light/10 py-6 sm:grid-cols-4">
            {statTiles.map((stat) => (
              <div key={stat.label} className="text-center">
                <div className="font-heading text-2xl leading-none font-bold text-brand-gold-light">{stat.value}</div>
                <div className="mt-1 font-mono text-[0.55rem] tracking-[0.15em] text-[#E8E4DC]/40 uppercase">{stat.label}</div>
        ...
        {nextRecommended && (
          <div className="mx-auto mb-8 max-w-300 px-6">
            <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-brand-gold-light/15 bg-linear-to-br from-brand-gold-light/8 to-brand-gold/4 px-8 py-6">
              <div>
                <span className="font-mono text-[0.55rem] tracking-[0.2em] text-brand-gold-light/50 uppercase">Recommended Next</span>
                <h3 className="mt-1 mb-0.5 font-heading text-xl font-bold text-[#E8E4DC]">{nextRecommended.name}</h3>
                <p className="mb-0 text-[0.85rem] text-[#E8E4DC]/40">
                  {nextRecommended.questionCount ? `${nextRecommended.questionCount} questions · ` : ""}~{nextRecommended.estimatedMinutes} min ·{" "}
        ...
        <div className="mx-auto mb-4 flex max-w-300 flex-wrap items-center justify-between gap-3 px-6">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <button onClick={() => setFilterCategory("all")} className={filterCategory === "all" ? CHIP_ACTIVE : CHIP_INACTIVE}>
              All
            </button>
            {CATEGORY_ORDER.map((cat) => (
              <button key={cat} onClick={() => setFilterCategory(cat)} className={filterCategory === cat ? CHIP_ACTIVE : CHIP_INACTIVE}>
                {CATEGORY_STYLES[cat].label}
              </button>
            ))}
            <span className="mx-1 h-4 w-px bg-brand-gold-light/10" />
            {(["all", "done", "todo"] as const).map((s) => (
              <button key={s} onClick={() => setFilterStatus(s)} className={filterStatus === s ? CHIP_ACTIVE : CHIP_INACTIVE}>
          ...
          <div className="mx-auto mt-12 max-w-170 px-6 text-center">
            <div className="rounded-[20px] border border-brand-gold-light/20 bg-linear-to-br from-brand-gold-light/10 to-brand-gold/5 px-8 py-12">
              ...
              <p className="mx-auto mb-6 max-w-130 text-base leading-relaxed text-[#E8E4DC]/60">
                ...
                unfolding. Come back anytime. The answers evolve as you do.
              </p>
              <Link
                href="/find-your-me"
        ...
        <footer className="mx-auto mt-12 max-w-300 border-t border-brand-gold-light/8 px-6 pt-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            <Link href="/find-your-me" className="font-mono text-[0.6rem] tracking-[0.1em] text-brand-gold-light/40 uppercase">
              Find Your Me
            </Link>
            ...
            <Link href="/self-portrait" className="font-mono text-[0.6rem] tracking-[0.1em] text-brand-gold-light/40 uppercase">
              Self-Portrait
            </Link>
            <Link href="/" className="font-mono text-[0.6rem] tracking-[0.1em] text-brand-gold-light/40 uppercase">
              Home
            </Link>
          </div>
        </footer>
