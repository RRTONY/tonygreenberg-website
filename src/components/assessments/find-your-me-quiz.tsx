...
export function FindYourMeQuiz({ articleTitles }: { articleTitles: Record<string, string> }) {
  ...
  const dimensionScores = useMemo(() => {
    ...
    DIMENSIONS.forEach((d) => {
      if (counts[d] > 0)
        scores[d] = Math.min(10, Math.round((scores[d] / (counts[d] * 9)) * 10 * 10) / 10);
    });
    return scores;
  }, [answers]);

  const archetype = useMemo(() => {
    const sorted = (Object.entries(dimensionScores) as [Dimension, number][]).sort(
      (a, b) => b[1] - a[1],
    );
  ...
  const handleShare = useCallback(
    (platform: string) => {
      ...
      if (platform === "twitter")
        ...
        window.open(
          `mailto:?subject=${encodeURIComponent(`Find My — ${archetype.name}`)}&body=${encodeURIComponent(`${text}\n\n${url}`)}`,
          "_blank",
        );
      ...
      DIRECTORY.map((section) => ({
        ...
        items: section.items.filter((item) => {
          ...
          const searchMatch =
            ...
            item.hook.toLowerCase().includes(directorySearch.toLowerCase());
    ...
    return (
      <div className="relative z-1 min-h-screen font-sans text-[#2C1810]">
        ...
        <div className="relative z-1 flex min-h-[70vh] flex-col items-center justify-center px-6 py-24 text-center">
          <div className="mb-8 font-mono text-[0.7rem] tracking-[0.3em] text-brand-gold/70 uppercase">
            Five Questions. No Escape Hatch.
          </div>
          <h1 className="mb-2 font-heading text-[clamp(2.8rem,8vw,5.5rem)] leading-[1.1] font-normal">
            Find Y<span className="text-brand-gold">our</span>{" "}
            <span className="text-brand-gold italic">Me</span>
          </h1>
          <h2 className="mb-8 font-heading text-[clamp(1.5rem,4vw,2.8rem)] leading-[1.2] font-normal text-[#5C4A3A]">
            to Find Y<span className="text-brand-gold">our</span>{" "}
            <span className="text-brand-gold italic">We</span>
          </h2>
          <div className="font-mono text-[0.65rem] tracking-[0.2em] text-brand-gold/50">
            SCROLL TO BEGIN
            <ArrowDown aria-hidden="true" className="ml-1 inline size-3.5" />
          </div>
        </div>
        ...
        <div className="relative z-1 mx-auto max-w-3xl px-6 pb-24">
          <p className="mb-16 text-center font-heading text-[clamp(1.6rem,4vw,2.4rem)] leading-[1.5] font-normal">
            Look closely.{" "}
            <span className="text-brand-gold italic underline decoration-brand-gold-light/40 underline-offset-4">
              Your
            </span>{" "}
          ...
          <div className="mb-16 grid gap-6">
            <GlassPanel className="border-l-2 border-l-brand-gold-light/30 p-10">
              ...
              <p className="text-[1.1rem] leading-[1.8] text-[#F5F0E0]/80">
                Five questions that don&apos;t have right answers — only honest ones. Each answer
                tilts a mirror. By the end, you&apos;ll see which dimension of life is pulling you
                hardest right now: purpose, relationships, body, truth, consciousness, or tribe. Not
                a personality quiz. A reckoning.
              </p>
            </GlassPanel>
            <GlassPanel className="border-l-2 border-l-brand-gold-light/30 p-10">
              <h3 className="mb-4 font-mono text-[0.7rem] tracking-[0.25em] text-brand-gold-light/60 uppercase">
              ...
              <p className="text-[1.1rem] leading-[1.8] text-[#F5F0E0]/80">
                Your archetype unlocks a map: essays that read like the conversation you&apos;ve
                been waiting to have, assessments that go deeper than you expected, tools that
                actually work, and a community of people who think like you do at 2am. Every minute
                you spend here shifts something. The hour after changes everything else.
              </p>
            </GlassPanel>
            <GlassPanel className="border-l-2 border-l-brand-gold-light/30 p-10">
              <h3 className="mb-4 font-mono text-[0.7rem] tracking-[0.25em] text-brand-gold-light/60 uppercase">
              ...
              <p className="text-[1.1rem] leading-[1.8] text-[#F5F0E0]/80">
                You can&apos;t be of service to the world until you know what you&apos;re serving
                from. Self-knowledge isn&apos;t selfish — it&apos;s prerequisite. Find your{" "}
                <em className="text-brand-gold not-italic">me</em> and you find the{" "}
                <em className="text-brand-gold not-italic">our</em> that was hiding inside{" "}
                <em className="text-brand-gold not-italic">your</em> all along. Then you find your
                people. Then you change the world around you. That&apos;s the sequence. That&apos;s
                the only sequence.
              </p>
            </GlassPanel>
          </div>
          ...
          <div className="mb-20 grid grid-cols-2 gap-6 text-center sm:grid-cols-4">
            {[
              ...
              <div key={stat.label}>
                ...
                <div className="mt-1 font-mono text-[0.65rem] tracking-[0.2em] text-brand-gold/50 uppercase">
                  {stat.label}
                </div>
              </div>
            ))}
          ...
          <div className="mb-20">
            <div className="mb-12 text-center">
              <div className="mb-3 font-mono text-[0.65rem] tracking-[0.3em] text-brand-gold/50 uppercase">
                Or Skip the Quiz
              </div>
              ...
              <p className="mx-auto max-w-125 text-base text-[#2C1810]/50">
          ...
          <div className="mb-20">
            <div className="mb-8 text-center">
              ...
              <p className="mx-auto max-w-135 text-base text-[#2C1810]/50">
                {DIRECTORY_TOTAL} doorways to self-knowledge. Some are live. Some are coming. All of
                them are you.
              </p>
                    ...
                    const card = (
                      ...
                      >
                        <div className="mb-1.5 flex items-start justify-between gap-2">
                          <span
                            className={`font-heading text-[0.95rem] ${isLive ? "text-brand-gold-light" : "text-brand-gold-light/50"}`}
                          >
                            {item.name}
                          </span>
                          <span
                            className={`shrink-0 rounded-sm border px-1.5 py-0.5 font-mono text-[0.55rem] tracking-[0.1em] uppercase ${
                              isLive
                                ? "border-[#7BC9A4]/20 text-[#7BC9A4]/70"
                                : "border-brand-gold-light/10 text-brand-gold-light/30"
                            }`}
                          >
                            {isLive ? "Live" : "Coming"}
                          </span>
            ...
            <div className="mt-10 flex flex-wrap justify-center gap-12 border-t border-b border-brand-gold-light/8 py-8 text-center">
              {[
                ...
                <div key={s.label}>
                  ...
                  <div className="mt-1 font-mono text-[0.6rem] tracking-[0.15em] text-brand-gold/40 uppercase">
                    {s.label}
                  </div>
                </div>
              ))}
          ...
          <div className="mb-8 text-center font-mono text-[0.65rem] tracking-[0.3em] text-brand-gold/30 uppercase">
          ...
          <div className="text-center">
            ...
            <p className="mt-6 text-[0.9rem] text-[#2C1810]/40 italic">
  ...
  if (phase === "assessment") {
    ...
    return (
      <div
        ref={questionRef}
        className="relative z-1 flex min-h-screen flex-col items-center justify-center px-6 py-16 font-sans text-[#2C1810]"
      >
        <ThemedBackground theme="selfportrait" />
        <div className="fixed inset-x-0 top-0 z-50 h-[2px] bg-brand-gold/10">
          <div
            className="h-full bg-linear-to-r from-brand-gold to-brand-gold-light transition-[width] duration-600"
        ...
        <div
          className={`w-full max-w-2xl transition-all duration-500 ${transitioning ? "-translate-y-5 opacity-0" : "translate-y-0 opacity-100"}`}
        >
          <h2 className="mb-3 font-heading text-[clamp(1.5rem,4vw,2.2rem)] leading-[1.4] font-normal">
            {q.stem}
          </h2>
            ...
            {q.choices.map((choice, idx) => (
              <button
                ...
                className={`w-full rounded-xl border px-6 py-5 text-left text-[1.05rem] leading-relaxed backdrop-blur-sm transition-all ${
                  selectedChoice === idx
                    ? "border-brand-gold bg-brand-gold-light/15"
                    : "border-brand-gold-light/10 bg-[#0A0A10]/40 hover:border-brand-gold-light/40 hover:bg-brand-gold-light/8"
  ...
  return (
    <div className="relative z-1 min-h-screen py-16 font-sans text-[#2C1810]">
      ...
      <div className="mx-auto max-w-4xl px-6">
        ...
        <h1 className="mb-2 text-center font-heading text-[clamp(2rem,5vw,3.5rem)] leading-[1.2] font-normal text-brand-gold">
          {archetype.name}
        </h1>
        ...
        <div className="mb-12 grid gap-8 md:grid-cols-2">
          ...
          <GlassPanel className="p-8">
            <p className="text-[1.02rem] leading-[1.8] text-[#F5F0E0]/85">
        ...
        <GlassPanel className="mb-12 p-8 text-center">
          <p className="mb-2 font-heading text-[1.3rem] text-brand-gold">
            You found y<span className="text-brand-gold">our</span> Me. Now find y
            <span className="text-brand-gold">our</span> We.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {[
              ...
              { platform: "linkedin", label: "Share on LinkedIn", icon: Share2 },
              { platform: "email", label: "Share by email", icon: Mail },
              { platform: "copy", label: "Copy link", icon: Clipboard },
          ...
          {shareCount > 0 && (
            <p className="mt-4 font-mono text-[0.7rem] text-brand-gold-light/50">
              {shareCount} of 10 shared ·{" "}
              {10 - shareCount > 0 ? `${10 - shareCount} to go` : "You're a Tribe Weaver already"}
            </p>
          )}
        </GlassPanel>
            ...
            {archetype.articles.map((slug, idx) => (
              <Link key={slug} href={`/blog/${slug}`}>
                <GlassPanel className="h-full p-5 transition-colors hover:border-brand-gold-light/30">
                  <span className="font-mono text-[0.65rem] text-brand-gold-light/40">
                    {String(idx + 1).padStart(2, "0")}
                  </span>
                  <p className="mt-1 font-heading text-[0.95rem] leading-[1.4] text-[#F5F0E0]/85">
          ...
          <div className="mb-6 grid gap-3 sm:grid-cols-2">
            {archetype.deeperAssessments.map((assess) => (
              <Link key={assess.path} href={assess.path}>
                <GlassPanel className="h-full p-5 transition-colors hover:border-brand-gold-light/30">
                  <p className="mb-1.5 font-heading text-base text-brand-gold-light">
                    {assess.label}
                    <ArrowRight aria-hidden="true" className="ml-1 inline size-3.5" />
                  </p>
            ...
            {archetype.nextStep.path.startsWith("http") ? (
              <a href={archetype.nextStep.path} target="_blank" rel="noopener noreferrer">
                <GlassPanel className="animate-pulse px-8 py-4">
                  <span className="font-mono text-[0.8rem] tracking-[0.15em] text-brand-gold">
            ...
            {archetype.nextStep.path !== "/community" && (
              <Link href="/community">
                <GlassPanel className="px-8 py-4">
                  <span className="font-mono text-[0.8rem] tracking-[0.15em] text-[#F5F0E0]/60">
                    Find Your Tribe
                    <ArrowRight aria-hidden="true" className="ml-1 inline size-3.5" />
                  </span>
                </GlassPanel>
