...
import { MIRROR_ARTICLES, MIRROR_DIMENSIONS, MIRROR_QUESTIONS, type MirrorDimensionId } from "@/lib/content/mirror-data";
...
export function TheMirrorQuiz({ articleTitles }: { articleTitles: Record<string, string> }) {
  ...
  const dimensionScores = useMemo(() => {
    ...
    MIRROR_DIMENSIONS.forEach((d) => {
      const vals = MIRROR_QUESTIONS.map((q, idx) => (q.dimension === d.id ? answers[idx] : undefined)).filter(
    ...
    return (
      <div className="relative z-1 min-h-screen font-sans text-[#2C1810]">
        ...
        <AssessmentIntro
          ...
          stats={{ questions: MIRROR_QUESTIONS.length, dimensions: MIRROR_DIMENSIONS.length, minutes: 4 }}
          whatYouGet={[
  ...
  if (phase === "quiz") {
    ...
    const dimProgress = MIRROR_QUESTIONS.slice(0, currentQ + 1).filter((qq) => qq.dimension === q.dimension).length;
      ...
      <div className="relative z-1 flex min-h-screen flex-col font-sans text-[#2C1810]">
        ...
        <div className="fixed inset-x-0 top-0 z-50">
          <div className="h-[3px] bg-brand-gold/10">
            <div className="h-full bg-linear-to-r from-brand-gold to-brand-gold-light transition-[width] duration-400" style={{ width: `${progress}%` }} />
          </div>
            ...
            {q.options.map((opt, i) => (
              <button
                ...
                className={`rounded-lg border px-5 py-4 text-left text-[0.92rem] leading-relaxed transition-all disabled:cursor-wait ${
                  selectedOption === i ? "border-brand-gold-light/40 bg-brand-gold-light/15" : "border-black/8 bg-white/40"
                }`}
              >
                {opt.text}
              </button>
  ...
  return (
    <div className="relative z-1 min-h-screen font-sans text-[#2C1810]">
      ...
      <section className="mx-auto max-w-3xl px-6 py-16 text-center">
        <div className="mb-4 font-mono text-[0.6rem] tracking-[0.2em] text-brand-gold uppercase">Your Mirror</div>
        <h1 className="mb-6 font-heading text-[clamp(2rem,5vw,3rem)] font-normal">Your Distance From Flow</h1>
      ...
      <section className="mx-auto max-w-4xl px-6 pb-16">
        <div className="grid items-center gap-12 md:grid-cols-2">
          <div>
            <div className="mb-1 font-mono text-[0.6rem] tracking-[0.2em] text-brand-gold uppercase">Your Map</div>
      ...
      <section className="mx-auto max-w-3xl px-6 pb-16">
        <div className="mb-1 font-mono text-[0.6rem] tracking-[0.2em] text-brand-gold uppercase">Dimension Insights</div>
      ...
      {recommendedArticles.length > 0 && (
        <section className="mx-auto max-w-3xl px-6 pb-16">
          <div className="mb-1 font-mono text-[0.6rem] tracking-[0.2em] text-brand-gold uppercase">Your Prescribed Reading</div>
          <h2 className="mb-4 font-heading text-2xl font-normal">Articles That Mirror Your Growth Edge</h2>
          <p className="mb-8 text-sm text-[#5A5A52]">
            Based on your assessment, these articles will challenge the dimensions where you have the most room to grow. Each one is a
                ...
                <Link key={article.slug} href={`/blog/${article.slug}`} className="block rounded-lg border border-black/8 bg-white/40 p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm text-[#5A5A52] italic">This article is a mirror for {article.reflection}</p>
                    </div>
                  </div>
                </Link>
              );
      ...
      {satelliteSites.length > 0 && (
        <section className="mx-auto max-w-3xl px-6 pb-16">
          <div className="mb-1 font-mono text-[0.6rem] tracking-[0.2em] text-brand-gold uppercase">Go Deeper</div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {satelliteSites.map((site) => (
              ...
              >
                <p className="mb-2 font-mono text-xs tracking-wider text-brand-gold">{MIRROR_DIMENSIONS.find((d) => d.id === site.dimension)?.name}</p>
      ...
      <section className="mx-auto max-w-3xl px-6 pb-16 text-center">
        <div className="mb-1 font-mono text-[0.6rem] tracking-[0.2em] text-brand-gold uppercase">The Invitation</div>
        ...
        <p className="mx-auto mb-8 max-w-xl text-sm text-[#5A5A52]">
          This assessment was designed to be shared. Send it to at least 10 people — your partner, your team, your family, your tribe.
          When everyone maps their dimensions, you can see where you complement each other, where you collide, and where you can grow
          together.
        </p>
        ...
        {!showShare ? (
          ...
          <div className="mx-auto max-w-md space-y-4">
            ...
            <div className="flex justify-center gap-3">
              <button onClick={() => handleShare("twitter")} className="border border-brand-gold/30 px-4 py-2 font-mono text-xs tracking-wider text-brand-gold">
                𝕏 Twitter
              </button>
              <button onClick={() => handleShare("linkedin")} className="border border-brand-gold/30 px-4 py-2 font-mono text-xs tracking-wider text-brand-gold">
                LinkedIn
              </button>
              <button onClick={() => handleShare("email")} className="border border-brand-gold/30 px-4 py-2 font-mono text-xs tracking-wider text-brand-gold">
                Email
              </button>
            </div>
      ...
      <section className="mx-auto max-w-3xl px-6 pb-16">
        <p className="mb-2 text-center font-mono text-[0.65rem] tracking-[0.25em] text-brand-gold uppercase">The Journey Continues</p>
      ...
      <section className="mx-auto max-w-3xl px-6 pb-16">
        <div className="grid gap-6 sm:grid-cols-2">
          <Link href="/community" className="rounded-lg bg-[#0A0A10] p-8 text-center">
            <h3 className="mb-2 font-heading text-xl font-normal text-[#FAFAF7]">Join The Community</h3>
            <p className="text-sm text-[#FAFAF7]/60">
              Find your tribe, your partner, your collaborators. Upload your contacts and start connecting.
            </p>
          </Link>
          <button onClick={resetAssessment} className="rounded-lg border-2 border-brand-gold-light/30 p-8 text-center">
            <h3 className="mb-2 font-heading text-xl font-normal">Retake The Mirror</h3>
            <p className="text-sm text-[#5A5A52]">Come back in a month. See how far you&apos;ve moved. The measurement is the medicine.</p>
          </button>
        </div>
      </section>
    </div>
