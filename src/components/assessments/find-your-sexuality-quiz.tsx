...
const DIMENSIONS: Dimension[] = ["attraction", "behavior", "identity", "fantasy", "emotional", "fluidity"];
const INITIAL_SCORES: Record<Dimension, number> = { attraction: 0, behavior: 0, identity: 0, fantasy: 0, emotional: 0, fluidity: 0 };
const ACCENT = "#C2185B";
...
const QUESTIONS: { question: string; context?: string; options: { text: string; score: Partial<Record<Dimension, number>> }[] }[] = [
  { question: "When you notice someone across a room, what draws your attention?", context: "Based on the Klein Sexual Orientation Grid — attraction operates independently of behavior.", options: [
    opt("I'm drawn almost exclusively to people of a different gender than mine.", { attraction: 1 }),
    opt("Mostly a different gender, but occasionally someone of the same gender catches my eye.", { attraction: 2, fluidity: 1 }),
    opt("Gender isn't the primary filter — I notice energy, presence, confidence.", { attraction: 4, fluidity: 2 }),
  ...
  { question: "Think about your most intense crush — the one that kept you up at night. Who was it?", context: "Fantasy and desire often reveal patterns that social conditioning suppresses.", options: [
    opt("Always someone of a different gender.", { fantasy: 1, attraction: 1 }),
    opt("Usually a different gender, but I've had at least one same-gender crush that surprised me.", { fantasy: 2, fluidity: 2, attraction: 2 }),
    opt("It's been pretty evenly split across genders.", { fantasy: 4, attraction: 4, fluidity: 1 }),
    opt("Mostly same-gender, with rare exceptions.", { fantasy: 5, attraction: 5 }),
    opt("Exclusively same-gender.", { fantasy: 6, attraction: 6 }),
    opt("My intense connections aren't really about gender — they're about the person's mind.", { fantasy: 3, emotional: 3, fluidity: 2 }),
  ] },
  { question: "In your actual sexual experiences, your partners have been...", context: "Behavior and attraction don't always align — and that's normal. (Kinsey, 1948)", options: [
    opt("Exclusively a different gender.", { behavior: 1 }),
    opt("Mostly a different gender, with some same-gender experiences.", { behavior: 2, fluidity: 1 }),
    ...
    opt("I haven't had sexual experiences, or they don't define my orientation.", { behavior: 0, identity: 1 }),
  ] },
  { question: "When you imagine your ideal long-term partner, what comes to mind?", context: "Emotional bonding patterns often differ from sexual attraction patterns.", options: [
    opt("A partner of a different gender — that's always been the picture.", { emotional: 1, identity: 1 }),
    opt("Probably a different gender, but I wouldn't rule anything out.", { emotional: 2, fluidity: 2, identity: 2 }),
    opt("Gender genuinely doesn't factor in — I imagine a person, not a gender.", { emotional: 4, identity: 4, fluidity: 2 }),
    opt("A same-gender partner feels most natural.", { emotional: 5, identity: 5 }),
    opt("Exclusively same-gender — anything else would feel wrong.", { emotional: 6, identity: 6 }),
    opt("I don't really imagine a long-term partner. I'm complete on my own.", { emotional: 0, identity: 0 }),
  ] },
  { question: "How would you describe your sexual orientation to a trusted friend?", context: "Self-identification is the most personal dimension — and often the last to shift.", options: [
    opt("Straight / heterosexual — no ambiguity.", { identity: 1 }),
    opt("Mostly straight, but I'm open-minded.", { identity: 2, fluidity: 1 }),
    opt("Bisexual, pansexual, or queer — I'm attracted to more than one gender.", { identity: 4, fluidity: 1 }),
    opt("Gay or lesbian — that's who I am.", { identity: 6 }),
    opt("I don't love labels. I just like who I like.", { identity: 3, fluidity: 3 }),
    opt("Asexual or on the ace spectrum — sexual attraction isn't really my thing.", { identity: 0, emotional: 2 }),
  ] },
  { question: "Has your sense of who you're attracted to changed over time?", context: "Lisa Diamond's research (2008) shows sexual fluidity is common and normal, especially in women — but not only in women.", options: [
    ...
    opt("It's been an ongoing evolution. I'm different now than I was 5 years ago.", { fluidity: 4 }),
  ...
  { question: "When consuming media (films, books, art), which romantic or sexual scenarios resonate most?", context: "Fantasy is often the most honest dimension — free from social performance.", options: [
    opt("Opposite-gender romance and intimacy.", { fantasy: 1 }),
    opt("Mostly opposite-gender, but same-gender stories intrigue me.", { fantasy: 2, fluidity: 1 }),
    opt("I'm drawn to compelling chemistry regardless of the genders involved.", { fantasy: 4, fluidity: 1 }),
    opt("Same-gender stories feel most authentic to me.", { fantasy: 5 }),
    opt("I'm more interested in emotional depth than the sexual component.", { fantasy: 3, emotional: 3 }),
    opt("I don't really connect with romantic or sexual media.", { fantasy: 0 }),
  ] },
  { question: "In your deepest emotional bonds — the ones that feel like home — the pattern is...", context: "Emotional orientation can differ from sexual orientation. (van Anders, 2015)", options: [
    ...
    opt("My deepest bonds cross gender lines — some romantic, some not.", { emotional: 3, fluidity: 1 }),
    opt("My most intense emotional connections have been with the same gender.", { emotional: 5 }),
    opt("I bond deeply with people regardless of gender — it's about the soul.", { emotional: 4, fluidity: 2 }),
    opt("I keep emotional distance from everyone. Deep bonds are rare for me.", { emotional: 0 }),
  ] },
  { question: "How much does gender play a role in who you find sexually attractive?", options: [
    opt("It's the primary factor — I'm attracted to specific gender presentations.", { attraction: 2, identity: 2 }),
    opt("It matters, but it's not the only thing.", { attraction: 3, fluidity: 1 }),
    opt("It's one factor among many — personality, intelligence, humor matter more.", { attraction: 3, fluidity: 2 }),
    opt("Gender is almost irrelevant to my attraction.", { attraction: 4, fluidity: 3 }),
    opt("I'm specifically attracted to gender nonconformity or androgyny.", { attraction: 4, fluidity: 3, identity: 3 }),
    opt("I don't experience sexual attraction in a way that involves gender.", { attraction: 0 }),
  ] },
  { question: "If all social judgment were removed — no one would ever know — would your behavior change?", context: "The gap between desire and behavior often reveals internalized social pressure.", options: [
    ...
    opt("Yes — I'd probably explore same-gender experiences.", { fluidity: 3, behavior: 3, fantasy: 3 }),
    opt("I'd be more open about my existing same-gender attractions.", { identity: 4, behavior: 4, fluidity: 2 }),
    opt("I'd be more publicly visible about my orientation.", { identity: 5, behavior: 5 }),
    opt("Social judgment doesn't really factor into my sexual decisions.", { identity: 3, fluidity: 1 }),
  ] },
  { question: "When you experience jealousy, it's most triggered by...", context: "Jealousy patterns can reveal attraction patterns we haven't consciously acknowledged.", options: [
    opt("My partner being close to someone of a different gender.", { attraction: 2, emotional: 1 }),
    opt("My partner being close to someone of the same gender.", { attraction: 4, emotional: 5 }),
    opt("My partner being close to anyone — gender doesn't matter.", { emotional: 3, attraction: 3 }),
    opt("I don't really experience jealousy.", { emotional: 1 }),
    opt("Emotional intimacy with others bothers me more than physical.", { emotional: 4, attraction: 3 }),
    opt("Physical intimacy with others bothers me more than emotional.", { attraction: 3, behavior: 3 }),
  ] },
  { question: "Your relationship to your own body and gender expression is...", context: "Gender identity and sexual orientation are distinct but often interrelated.", options: [
    opt("I'm comfortable with my assigned gender and express it conventionally.", { identity: 1 }),
    opt("I'm comfortable with my gender but enjoy playing with expression.", { identity: 2, fluidity: 1 }),
    opt("I sometimes feel my gender expression doesn't match expectations.", { identity: 3, fluidity: 2 }),
    opt("Gender feels like a spectrum to me — I exist somewhere in the middle.", { identity: 4, fluidity: 3 }),
    opt("My gender identity is different from what was assigned at birth.", { identity: 4, fluidity: 2 }),
    opt("I don't think about gender much — it's not central to who I am.", { identity: 3, fluidity: 1 }),
  ] },
  { question: "In your private fantasies (the ones you'd never share), the recurring theme involves...", context: "Private fantasy is often the most unfiltered window into desire.", options: [
    opt("Exclusively opposite-gender scenarios.", { fantasy: 1 }),
    opt("Mostly opposite-gender, with occasional same-gender elements.", { fantasy: 2, fluidity: 2 }),
    ...
    opt("The focus isn't on gender — it's on power, connection, or sensation.", { fantasy: 3, emotional: 2 }),
    opt("I don't have sexual fantasies, or they're very rare.", { fantasy: 0 }),
  ] },
  { question: "How do you feel about the concept of sexual orientation labels?", options: [
    ...
    opt("Labels are useful shorthand, but mine doesn't capture everything.", { identity: 3, fluidity: 2 }),
    opt("I actively resist labels — they feel limiting.", { identity: 3, fluidity: 4 }),
    opt("I've changed my label at least once as I've learned more about myself.", { identity: 3, fluidity: 4 }),
    opt("I use 'queer' as an umbrella because nothing else fits.", { identity: 4, fluidity: 3 }),
  ] },
  { question: "If you could design your ideal intimate life with zero constraints, it would include...", options: [
    opt("One committed partner of a different gender.", { behavior: 1, emotional: 1, identity: 1 }),
    opt("One committed partner of the same gender.", { behavior: 6, emotional: 6, identity: 6 }),
    opt("One committed partner — gender isn't the deciding factor.", { behavior: 3, emotional: 4, fluidity: 2 }),
    opt("Multiple partners across the gender spectrum.", { behavior: 4, fluidity: 4, attraction: 4 }),
    opt("Deep emotional intimacy with minimal sexual component.", { emotional: 5, behavior: 0, fantasy: 0 }),
    opt("Freedom to explore without commitment to any pattern.", { fluidity: 5, behavior: 3, identity: 3 }),
  ] },
  { question: "Looking at your life as a whole — past, present, and imagined future — your sexuality is best described as...", context: "Integration question — how all dimensions come together in your self-understanding.", options: [
    ...
    opt("A journey of discovery — I've learned a lot about myself.", { identity: 4, fluidity: 3 }),
    opt("Clear and consistent — I'm gay/lesbian and always have been.", { identity: 6, fluidity: 0 }),
...
export function FindYourSexualityQuiz() {
  ...
  return (
    <div className="relative z-1 flex min-h-screen flex-col items-center justify-center px-8 py-8 text-center font-sans text-[#2C1810]">
        ...
        <div className="relative z-1 w-full max-w-2xl">
          <div className="mb-8">
            ...
            <div className="h-0.5 overflow-hidden rounded-full bg-[#C97B7B]/15">
              <div className="h-full bg-[#C97B7B] transition-[width] duration-400" style={{ width: `${progress}%` }} />
            </div>
          </div>

          <h2 className="mb-2 font-heading text-2xl leading-[1.3]">{QUESTIONS[step].question}</h2>
          {QUESTIONS[step].context && <p className="mb-6 text-[0.78rem] leading-relaxed text-[#8B7B6B] italic">{QUESTIONS[step].context}</p>}
      ...
      {phase === "results" && !emailGated && <EmailGate assessmentSlug="sexuality" onUnlock={() => setEmailGated(true)} />}

      {phase === "results" && emailGated && (
        <div className="relative z-1 w-full max-w-2xl">
          <div className="mb-2 font-mono text-[0.7rem] tracking-[0.12em] text-[#C97B7B] uppercase">Your Profile</div>
          <h2 className="mb-1 font-heading text-4xl text-brand-gold">{archetype.name}</h2>
          <div className="mb-6 font-mono text-[0.82rem] tracking-[0.05em] text-[#C97B7B]">{archetype.title}</div>
          ...
          <div className="mb-8 grid grid-cols-3 gap-3">
            {DIMENSIONS.map((dim) => (
              <div key={dim} className="rounded-md border border-[#C97B7B]/15 p-2.5 text-center">
                <div className="mb-0.5 font-mono text-[0.6rem] tracking-[0.08em] text-[#5A4A3A] uppercase">{dim}</div>
          ...
          <div className="mb-8 rounded-lg border border-[#C97B7B]/15 bg-[#C97B7B]/4 p-5 text-left">
            <div className="mb-2 font-mono text-[0.65rem] tracking-[0.08em] text-[#C97B7B] uppercase">Research Note</div>
          ...
          <p className="mx-auto mb-8 max-w-125 text-[0.78rem] leading-relaxed text-[#4A3A2A] italic">
            This assessment is a reflection tool, not a diagnosis. Sexual orientation is complex, personal, and can
            evolve over a lifetime. If you&apos;re exploring your identity, consider speaking with a qualified
            therapist who specializes in sexuality and gender.
          </p>
