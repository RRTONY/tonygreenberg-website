...
function ch(text: string, weights: Partial<Record<Axis, number>>, extra?: { subtext?: string; contraFlag?: string }): Choice {
  return { text, weights, ...extra };
}

const QUESTIONS: Question[] = [
  { stem: "How does your body handle physical damage?", subtext: "Think about the last time you were injured, had surgery, or pushed your body hard. How did recovery go?", citation: "Sikiric et al., J Physiol Pharmacol, 2018 — BPC-157 tissue repair mechanisms", choices: [
    ch("I heal quickly — minor injuries resolve in days, not weeks", { recovery: 1, vitality: 1 }, { subtext: "Strong baseline recovery" }),
    ch("Recovery takes longer than it used to — I notice a slow decline", { recovery: 3, vitality: 2, longevity: 1 }, { subtext: "Age-related repair slowdown" }),
    ch("I have chronic pain or an injury that never fully healed", { recovery: 4, immune: 1 }, { subtext: "Persistent tissue damage" }),
    ch("My gut is the problem — bloating, inflammation, or digestive issues dominate", { recovery: 3, immune: 2, metabolic: 1 }, { subtext: "Gut-specific repair needs" }),
    ch("I'm recovering from surgery or a major physical event right now", { recovery: 5, vitality: 2 }, { subtext: "Acute recovery window", contraFlag: "active_recovery" }),
  ] },
  { stem: "What's your relationship with your metabolism?", subtext: "Not what the scale says. How your body processes energy, stores fat, and responds to food.", citation: "Wilding et al., NEJM, 2021 — Semaglutide STEP trial outcomes", choices: [
    ch("My metabolism works fine — I maintain weight without much effort", { metabolic: 1, vitality: 1 }),
    ch("I've gained weight that won't respond to diet or exercise anymore", { metabolic: 4, vitality: 1, longevity: 1 }, { subtext: "Metabolic resistance" }),
    ch("I crash after meals — energy spikes and drops control my day", { metabolic: 3, cognitive: 2, vitality: 1 }, { subtext: "Insulin/glucose dysregulation" }),
    ch("I have a family history of diabetes, heart disease, or metabolic syndrome", { metabolic: 4, longevity: 2, immune: 1 }, { subtext: "Genetic metabolic risk", contraFlag: "metabolic_family_history" }),
    ch("I'm actively trying to recompose my body — lose fat, gain muscle", { metabolic: 3, vitality: 3 }, { subtext: "Body recomposition goal" }),
  ] },
  { stem: "How do you sleep — and how do you feel when you wake up?", subtext: "Sleep architecture is the foundation of growth hormone release, cellular repair, and cognitive consolidation.", citation: "Van Cauter et al., JAMA, 2000 — GH secretion and sleep quality correlation", choices: [
    ch("I sleep well and wake rested — 7-8 hours of solid, uninterrupted sleep", { vitality: 1, cognitive: 1 }),
    ch("I sleep enough hours but wake exhausted — the quality is gone", { vitality: 4, cognitive: 2, longevity: 1 }, { subtext: "Disrupted sleep architecture" }),
    ch("I can't fall asleep — my mind races or my body won't settle", { cognitive: 3, vitality: 2, immune: 1 }, { subtext: "Onset insomnia / HPA axis dysregulation" }),
    ch("I wake at 2-4 AM and can't get back to sleep", { vitality: 4, longevity: 2, metabolic: 1 }, { subtext: "Cortisol/GH imbalance pattern" }),
    ch("I've noticed my recovery from workouts and illness has slowed with poor sleep", { vitality: 3, recovery: 3, immune: 2 }, { subtext: "Sleep-recovery cascade failure" }),
  ] },
  { stem: "How often do you get sick — and how hard does it hit?", subtext: "Your immune system is a peptide-driven orchestra. This question maps its current performance.", citation: "Tuthill et al., Ann NY Acad Sci, 2010 — Thymosin alpha-1 immune modulation", choices: [
    ch("Rarely sick — maybe once a year, and I bounce back fast", { immune: 1, vitality: 1 }),
    ch("I catch everything going around — colds, flu, infections hit me hard", { immune: 4, vitality: 2, longevity: 1 }, { subtext: "Immune suppression pattern" }),
    ch("I have autoimmune issues — my immune system attacks my own body", { immune: 5, recovery: 2, longevity: 1 }, { subtext: "Immune dysregulation", contraFlag: "autoimmune" }),
    ch("Chronic low-grade inflammation — I'm not 'sick' but I'm never fully well", { immune: 3, longevity: 3, recovery: 1 }, { subtext: "Inflammaging pattern" }),
    ch("I'm immunocompromised or on immunosuppressive medication", { immune: 5, longevity: 2 }, { subtext: "Medical immune status", contraFlag: "immunocompromised" }),
  ] },
  { stem: "What's happening with your brain?", subtext: "Not your intelligence. Your cognitive machinery — focus, recall, processing speed, mental stamina.", citation: "Uchida et al., Behav Brain Res, 2017 — Semax/Selank nootropic mechanisms", choices: [
    ch("Sharp as ever — I can focus for hours and recall details easily", { cognitive: 1, vitality: 1 }),
    ch("Brain fog is real — I lose words, forget why I walked into rooms", { cognitive: 4, longevity: 2, vitality: 1 }, { subtext: "Cognitive decline pattern" }),
    ch("I can focus but I'm mentally exhausted by 2 PM every day", { cognitive: 3, metabolic: 2, vitality: 1 }, { subtext: "Cognitive fatigue / neurotransmitter depletion" }),
    ch("Anxiety and racing thoughts dominate — I can't quiet my mind", { cognitive: 4, immune: 1, sexual: 1 }, { subtext: "HPA axis / GABAergic imbalance" }),
    ch("I have a family history of cognitive decline, Alzheimer's, or dementia", { cognitive: 4, longevity: 4 }, { subtext: "Neuroprotection priority", contraFlag: "neuro_family_history" }),
  ] },
  { stem: "How's your hormonal and sexual health?", subtext: "Libido, energy, mood, and sexual function are downstream signals of peptide and hormonal balance.", citation: "Diamond et al., J Sex Med, 2016 — PT-141 (bremelanotide) clinical efficacy", choices: [
    ch("Everything works well — libido, energy, and function are all solid", { sexual: 1, vitality: 1 }),
    ch("My libido has dropped noticeably — I'm less interested than I used to be", { sexual: 4, vitality: 2, longevity: 1 }, { subtext: "Hormonal decline pattern" }),
    ch("Function is impaired — it's not just desire, it's physical response", { sexual: 5, vitality: 2 }, { subtext: "Physiological sexual dysfunction", contraFlag: "sexual_dysfunction" }),
    ch("Mood swings, hot flashes, or hormonal symptoms are disrupting my life", { sexual: 3, immune: 2, metabolic: 1 }, { subtext: "Endocrine disruption" }),
    ch("I'm on hormone replacement therapy and want to optimize further", { sexual: 3, vitality: 2, longevity: 2 }, { subtext: "HRT optimization context", contraFlag: "on_hrt" }),
  ] },
  { stem: "How do you think about aging?", subtext: "Not vanity. The biological reality of cellular senescence, telomere shortening, and oxidative damage.", citation: "Khavinson et al., Bull Exp Biol Med, 2003 — Epitalon telomerase activation", choices: [
    ch("I feel my biological age matches my chronological age — aging normally", { longevity: 1, vitality: 1 }),
    ch("I feel older than my age — my body is aging faster than my years", { longevity: 4, vitality: 3, recovery: 1 }, { subtext: "Accelerated biological aging" }),
    ch("I'm proactively trying to slow aging — I want to optimize healthspan", { longevity: 3, vitality: 2, cognitive: 1 }, { subtext: "Longevity optimization mindset" }),
    ch("Skin, hair, and visible aging markers are progressing rapidly", { longevity: 3, vitality: 2, sexual: 1 }, { subtext: "Collagen/elastin decline" }),
    ch("I've had cancer, serious illness, or major health events that accelerated aging", { longevity: 5, immune: 3, recovery: 2 }, { subtext: "Post-illness cellular damage", contraFlag: "cancer_history" }),
  ] },
  { stem: "What does your daily stress load look like?", subtext: "Chronic stress is a peptide disruptor — it suppresses GH, inflames the gut, crashes immunity, and accelerates aging simultaneously.", citation: "McEwen, NEJM, 1998 — Allostatic load and stress-mediated physiological damage", choices: [
    ch("Manageable — I have stress but I recover from it well", { cognitive: 1, immune: 1, vitality: 1 }),
    ch("High but functional — I perform under pressure but I'm burning reserves", { cognitive: 2, immune: 2, vitality: 2, metabolic: 1 }, { subtext: "Allostatic overload building" }),
    ch("Chronic and unrelenting — I haven't felt truly relaxed in months or years", { cognitive: 3, immune: 3, vitality: 2, longevity: 2 }, { subtext: "HPA axis dysregulation" }),
    ch("My stress manifests physically — headaches, GI issues, muscle tension, insomnia", { recovery: 3, immune: 2, cognitive: 2, metabolic: 1 }, { subtext: "Somatized stress" }),
    ch("I've experienced burnout, trauma, or PTSD that changed my baseline", { cognitive: 4, immune: 3, longevity: 2, sexual: 1 }, { subtext: "Neurological stress injury", contraFlag: "trauma_history" }),
  ] },
  { stem: "Medical history check — select everything that applies.", subtext: "This is a safety screening question. Certain peptides are contraindicated with specific conditions. Select the most relevant.", citation: "FDA Safety Communication, 2023 — Peptide therapy contraindications and monitoring requirements", choices: [
    ch("No significant medical history — generally healthy", { vitality: 1 }),
    ch("I take prescription medications daily (blood pressure, thyroid, psychiatric, etc.)", { immune: 1, metabolic: 1 }, { subtext: "Drug interaction screening required", contraFlag: "prescription_meds" }),
    ch("I have a history of cancer or am in remission", { longevity: 2, immune: 2 }, { subtext: "Growth factor contraindication flag", contraFlag: "cancer_history" }),
    ch("I have kidney or liver disease", { metabolic: 1, immune: 1 }, { subtext: "Peptide clearance concern", contraFlag: "organ_disease" }),
    ch("I'm pregnant, breastfeeding, or trying to conceive", { sexual: 1 }, { subtext: "Absolute contraindication for most peptides", contraFlag: "pregnancy" }),
  ] },
  { stem: "What matters most to you right now?", subtext: "This weights your results toward your primary goal. All axes still factor in — but your priority gets amplified.", choices: [
    ch("Heal something specific — an injury, gut issue, or chronic pain", { recovery: 5 }, { subtext: "Recovery-first protocol" }),
    ch("Lose weight and fix my metabolism", { metabolic: 5 }, { subtext: "Metabolic-first protocol" }),
    ch("Feel younger — more energy, better sleep, stronger body", { vitality: 5 }, { subtext: "Vitality-first protocol" }),
    ch("Protect my brain — sharper thinking, better memory, less anxiety", { cognitive: 5 }, { subtext: "Cognitive-first protocol" }),
    ch("Slow aging and optimize long-term health", { longevity: 5 }, { subtext: "Longevity-first protocol" }),
  ] },
];
...
const PROFILES: PeptideProfile[] = [
  { name: "The Rebuilder", tagline: "Repair what's broken. Restore what's lost.", axes: ["recovery", "vitality"],
  ...
  { name: "The Metabolic Reset", tagline: "Rewire how your body processes energy.", axes: ["metabolic", "recovery"],
  ...
  { name: "The Optimizer", tagline: "Peak performance starts at the molecular level.", axes: ["vitality", "cognitive"],
  ...
  { name: "The Guardian", tagline: "Fortify the body's first and last line of defense.", axes: ["immune", "recovery"],
  ...
  { name: "The Architect", tagline: "Redesign your body from the inside out.", axes: ["metabolic", "vitality"],
  ...
  { name: "The Sentinel", tagline: "Defend the mind. Protect the system.", axes: ["immune", "cognitive"],
  ...
  { name: "The Restorer", tagline: "Rebuild the physical foundation of vitality.", axes: ["recovery", "sexual"],
  ...
  { name: "The Strategist", tagline: "Fuel the brain. Fix the engine.", axes: ["cognitive", "metabolic"],
  ...
  { name: "The Vanguard", tagline: "Cellular defense meets cellular longevity.", axes: ["longevity", "immune"],
  ...
  { name: "The Renaissance", tagline: "Vitality and desire, restored together.", axes: ["vitality", "sexual"],
  ...
  { name: "The Catalyst", tagline: "Metabolic clarity drives mental clarity.", axes: ["metabolic", "cognitive"],
  ...
  { name: "The Centurion", tagline: "Age is a number. Biology is a choice.", axes: ["longevity", "vitality"],
  ...
  { name: "The Healer", tagline: "Deep repair meets deep defense.", axes: ["recovery", "immune"],
  ...
  { name: "The Alchemist", tagline: "Protect the mind across decades, not days.", axes: ["longevity", "cognitive"],
  ...
  { name: "The Phoenix", tagline: "Rise from cellular damage. Regenerate from within.", axes: ["recovery", "longevity"],
  ...
  { name: "The Sovereign", tagline: "Balanced optimization across every system.", axes: ["vitality", "longevity"],
...
function getProfile(scores: Record<Axis, number>): PeptideProfile {
  ...
  const match = PROFILES.find((p) => (p.axes[0] === top1 && p.axes[1] === top2) || (p.axes[0] === top2 && p.axes[1] === top1));
  if (!match) return PROFILES.find((p) => p.axes[0] === top1 || p.axes[1] === top1) || PROFILES[PROFILES.length - 1];
  return match;
}
...
export function FindYourPeptideQuiz() {
  ...
  const handleShare = useCallback(
    (platform: string) => {
      ...
      if (platform === "email") window.open(`mailto:?subject=${encodeURIComponent("My Peptide Clarity Index™ Results")}&body=${encodeURIComponent(`${text}\n\n${url}`)}`, "_blank");
      ...
      <div className="relative z-1 min-h-screen px-6 py-8 font-sans text-[#2C1810]">
        ...
        <div className="mx-auto mb-8 max-w-150">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="font-mono text-[0.65rem] text-[#8B7B6B]">{Math.round(progress * 100)}%</span>
          </div>
            ...
            {q.choices.map((choice, idx) => (
              <button
                ...
                className={`rounded-md border px-5 py-4 text-left transition-all ${
                  answers[currentQ] === idx ? "border-brand-gold-light/40 bg-brand-gold-light/12" : "border-black/6 bg-white/40"
                }`}
              >
                <div className="text-[0.95rem] leading-relaxed">{choice.text}</div>
                {choice.subtext && <div className="mt-1 font-mono text-[0.7rem] text-[#8B7B6B]">{choice.subtext}</div>}
              </button>
            ))}
          </div>
      ...
      <div className="relative z-1 flex min-h-screen items-center justify-center px-6 py-16 font-sans text-[#2C1810]">
        ...
        <div className="max-w-110 text-center">
          ...
          <p className="mb-2 text-base leading-relaxed text-[#8B7B6B]">
            ...
            citations, suggested lab work, and contraindication screening results.
          </p>
          ...
          <p className="mt-3 font-mono text-[0.65rem] text-[#8B7B6B]">No spam. Unsubscribe anytime. Your data stays private.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="relative z-1 min-h-screen px-6 py-16 font-sans text-[#2C1810]">
      ...
      <div className="mx-auto max-w-3xl">
        <div className="mb-12 text-center">
          <div className="mb-2 font-mono text-[0.7rem] tracking-[0.2em] text-brand-gold/60 uppercase">Your Peptide Clarity Index™ Result</div>
          <h1 className="mb-2 font-heading text-[clamp(2rem,5vw,3rem)] text-brand-gold">{profile.name}</h1>
        ...
        <div className="mb-10 rounded-lg border border-brand-gold/15 bg-white/40 p-6 text-center">
          <div className="mb-2 font-mono text-[0.65rem] text-[#8B7B6B]">PEPTIDE READINESS SCORE</div>
        ...
        <div className="mb-10">
          <h3 className="mb-4 font-mono text-[0.7rem] tracking-[0.15em] text-brand-gold/60 uppercase">Primary Peptide Recommendations</h3>
        ...
        <div className="mb-10">
          <h3 className="mb-4 font-mono text-[0.7rem] tracking-[0.15em] text-brand-gold/60 uppercase">Secondary Considerations</h3>
        ...
        <div className="mb-10 rounded-lg border border-brand-gold/15 bg-brand-gold-light/8 p-5">
          <h3 className="mb-2 font-mono text-[0.7rem] tracking-[0.15em] text-brand-gold uppercase">Stacking Protocol Notes</h3>
          <p className="text-sm leading-relaxed">{profile.stackingNotes}</p>
        </div>

        <div className="mb-10">
          <h3 className="mb-4 font-mono text-[0.7rem] tracking-[0.15em] text-brand-gold/60 uppercase">Suggested Lab Work Before Starting</h3>
          <div className="flex flex-wrap gap-2">
            {profile.suggestedLabWork.map((lab) => (
              <span key={lab} className="rounded-full border border-black/8 bg-white/40 px-3 py-1.5 font-mono text-[0.7rem] text-[#8B7B6B]">
                {lab}
              </span>
            ))}
          </div>
        ...
        <div className="mb-10 text-center">
          ...
          {shareCount > 0 && <p className="mt-2 font-mono text-[0.6rem] text-brand-gold/50">Shared {shareCount}x</p>}
        </div>
        ...
        <div className="mb-10 rounded-lg border border-black/6 bg-white/20 p-5">
          <p className="text-center font-mono text-[0.65rem] leading-relaxed text-[#8B7B6B]">
            <strong className="text-[#5A4A3A]">Medical Disclaimer:</strong> This assessment is for educational
            purposes only and does not constitute medical advice. Peptide therapy should only be pursued under the
            supervision of a qualified healthcare provider. Always consult your physician before starting any new
            therapeutic protocol. Individual results vary. Citations reference published research but do not
            guarantee outcomes.
          </p>
        </div>
