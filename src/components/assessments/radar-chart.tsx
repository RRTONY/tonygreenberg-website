  ...
  return (
    <svg viewBox="0 0 300 340" className="mx-auto w-full max-w-80">
      {rings.map((frac) => {
        ...
        return <path key={frac} d={path} fill="none" stroke="currentColor" strokeWidth="0.5" opacity="0.15" />;
      })}
      {dims.map((_, i) => {
        const p = getPoint(i, max);
        return <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="currentColor" strokeWidth="0.5" opacity="0.15" />;
      })}
