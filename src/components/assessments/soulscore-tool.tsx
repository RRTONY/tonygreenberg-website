...
export function SoulScoreTool() {
  ...
  const [tierScores, setTierScores] = useState<number[][]>(() => SUPPLY_TIERS.map(() => [50, 50, 50, 50, 50]));
      ...
      <div className="relative z-1 min-h-screen font-sans text-[#2C1810]">
        ...
        <div className="relative z-1 flex min-h-screen flex-col items-center justify-center px-6 py-16 text-center">
          ...
          <p className="mb-10 max-w-130 text-lg leading-relaxed text-[#5A4A32]">
            The first impact measurement engine that scores <em>consciousness</em>, not just carbon. Twelve
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7]">
      <header className="relative overflow-hidden bg-linear-to-br from-[#3D2E14] via-[#5A4020] to-[#3D2E14] px-6 pt-12 pb-8 text-center text-[#FAFAF7]">
        <p className={`mb-2 font-mono text-[0.7rem] tracking-[0.25em] text-[#D4B96A] uppercase`}>by ImpactSoul</p>
        ...
        {TAB_NAMES.map((t) => (
          <button
            ...
            className={`shrink-0 border-b-2 px-4 py-3 font-mono text-[0.65rem] tracking-[0.12em] uppercase transition-colors ${
              activeTab === t ? `${PALETTE.gold.text} ${PALETTE.gold.border} font-bold` : "border-transparent text-[#666]"
            }`}
          >
            {t}
          </button>
      ...
      <footer className="bg-linear-to-br from-[#3D2E14] to-[#5A4020] px-6 py-8 text-center font-mono text-[0.65rem] tracking-[0.1em] text-[#FAFAF7]/70">
        ...
        <p>Consciousness-Adjusted NPV · {SUPPLY_TIERS.length}-Tier Supply Chain · Impact Gig Economy · On-Chain Verification</p>
...
function MeasureTab({
  ...
  return (
    <>
      ...
      <div className={`${CARD} mb-4 flex flex-wrap items-center justify-center gap-8 border-2 ${PALETTE.goldLight.border} bg-linear-to-br from-[#FAFAF7] to-[#F5F0E6] p-6`}>
        ...
        <div className={`flex size-20 items-center justify-center rounded-full border-3 bg-white ${gradeC.border}`}>
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-3">
        ...
        <div className={CARD}>
          ...
          <div className="mt-1 font-mono text-[0.65rem] text-[#999]">SCI×30 + LJI×20 + C×15 + RCI×15 + CPI×10 + HDI×10</div>
        </div>
        <div className={CARD}>
          <div className={MONO_LABEL}>C-NPV Premium</div>
          <div className={`mt-1 font-heading text-2xl font-bold ${cnpvPremium >= 0 ? PALETTE.gold.text : PALETTE.red.text}`}>
            <AnimNum value={cnpvPremium} suffix="%" />
          </div>
          <div className="mt-1 font-mono text-[0.65rem] text-[#999]">Consciousness-adjusted value</div>
        </div>
      </div>
      ...
      <div className={CARD}>
        ...
        {diagnosis.weak.length === 0 && diagnosis.strong.length === 0 && (
          <p className="text-[0.85rem] text-[#999]">Adjust the sliders above to see auto-diagnosis.</p>
        )}
      </div>
    </>
  );
...
function BenchmarkTab({ scores, entityName }: { scores: number[]; entityName: string }) {
  ...
  return (
    <>
      ...
      <div className={`${CARD} mb-4 ${PALETTE.goldLight.border} bg-[#FDF8E8] text-[0.85rem] leading-relaxed text-[#5a4a1e]`}>
        <strong>ESG scores positively correlate with greenwashing</strong> (ScienceDirect 2025). BP gets ESG
        &apos;AA&apos; but SoulScore <strong className={PALETTE.red.text}>F-25</strong>. SoulScore&apos;s{" "}
        ...
        <table className="w-full min-w-225 border-collapse font-mono text-[0.6rem]">
          <thead>
            <tr className="border-b-2 border-[#0A0A10]">
              ...
              {DIMENSIONS.map((d) => (
                <th key={d.id} className={`p-1 text-center text-[0.55rem] tracking-[0.05em] ${PALETTE[d.colorKey].text}`}>
                  {d.shortLabel}
                </th>
              ))}
          ...
          <tbody>
            {allEntities.map((ent, idx) => {
              ...
              return (
                ...
                >
                  <td className={`p-2 whitespace-nowrap ${isUser ? PALETTE.gold.text : "text-[#0A0A10]"}`}>
                    {ent.tag && <span className="mr-1 text-[#999]">{ent.tag}</span>}
                    {ent.name}
                  </td>
                  {ent.scores.map((s, si) => {
                    const n = normalize(s, DIMENSIONS[si]);
                    const colorClass = n >= 70 ? PALETTE.green.text : n >= 40 ? PALETTE.amber.text : n < 25 ? PALETTE.red.text : "text-[#666]";
                    return (
  ...
  return (
    <>
      ...
      <p className="mb-6 text-[0.9rem] leading-relaxed text-[#666]">
        Beyond carbon. {SUPPLY_TIERS.length} tiers, {totalFactors} sub-factors. Click any tier to expand and score
        individual factors.
      </p>
      ...
      <div className="flex flex-col">
        {SUPPLY_TIERS.map((tier, ti) => {
          ...
          const avgColorClass = avg >= 70 ? PALETTE.green.text : avg >= 40 ? PALETTE.amber.text : PALETTE.red.text;
          const avgHex = avg >= 70 ? PALETTE.green.hex : avg >= 40 ? PALETTE.amber.hex : PALETTE.red.hex;
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <div className={`flex size-7.5 items-center justify-center rounded-full font-mono text-[0.6rem] font-bold ${avgColorClass} ${isExpanded ? "bg-[#F5F0E6]" : "bg-white"}`}>
                        {avg.toFixed(0)}
                      </div>
                    </div>
                    <span className={`text-[0.8rem] text-[#999] transition-transform ${isExpanded ? "rotate-180" : ""}`}>▼</span>
                  </div>
                </div>
              </button>
                  ...
                  {tier.factors.map((f, fi) => (
                    <div key={f} className="mb-3">
                      <div className="mb-1 flex justify-between">
                        ...
                        <span className={`font-mono text-[0.7rem] font-bold ${c.text}`}>{tierScores[ti][fi]}</span>
                      </div>
                      <input
                        type="range"
                        min={0}
...
function FailuresTab() {
  return (
    <>
      ...
      <p className="mb-6 text-[0.9rem] leading-relaxed text-[#666]">
        {FAILURES.length} failure patterns. Each one diagnosed. Each one fixed by SoulScore&apos;s architecture.
      </p>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {FAILURES.map((f) => {
          const sev = severityClasses(f.severity);
          return (
            <div key={f.title} className={`flex flex-col gap-2 rounded-lg border border-[#E8E4DC] border-t-3 ${sev.border} bg-white p-5`}>
              <div className="flex items-start justify-between gap-2">
                <span className="flex-1 text-[0.85rem] leading-snug font-bold text-[#0A0A10]">{f.title}</span>
                <span className={`rounded-sm ${sev.bgSolid} px-1.5 py-0.5 font-mono text-[0.6rem] font-bold whitespace-nowrap text-white`}>
                  SEV {f.severity}
                </span>
              </div>
    ...
    <>
      ...
      <div className={`${CARD} mb-4 grid gap-4 sm:grid-cols-2`}>
        ...
        <div>
          ...
          <div className={`mt-1 font-heading text-2xl font-bold ${PALETTE.gold.text}`}>{consciousness}</div>
        </div>
        <div>
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-3">
        <div className={CARD}>
          ...
          <div className="mt-1 font-heading text-2xl font-bold text-[#666]">{formatCurrency(cnpv.standard)}</div>
        </div>
        <div className={`${CARD} border-2 ${PALETTE.goldLight.border} bg-[#FDF8E8]`}>
          <div className={MONO_LABEL}>Consciousness-Adjusted NPV</div>
          <div className={`mt-1 font-heading text-2xl font-bold ${PALETTE.gold.text}`}>{formatCurrency(cnpv.adjusted)}</div>
        </div>
        <div className={CARD}>
          <div className={MONO_LABEL}>Premium / Penalty</div>
          <div className={`mt-1 font-heading text-2xl font-bold ${cnpv.premium >= 0 ? PALETTE.green.text : PALETTE.red.text}`}>
            <AnimNum value={cnpv.premium} suffix="%" />
          </div>
          <div className="mt-1 font-mono text-[0.65rem] text-[#999]">Delta: {formatCurrency(cnpv.adjusted - cnpv.standard)}</div>
        </div>
      </div>
        ...
        {CNPV_REFERENCE_POINTS.map((r, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <span className={`font-mono text-[0.75rem] font-bold ${r.premium.startsWith("-") ? PALETTE.red.text : PALETTE.green.text}`}>
                {r.premium}
              </span>
            </div>
          </div>
  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-4">
        {GIG_STATS.map((s, i) => (
          <div key={s.label} className={`${CARD} text-center ${i === 2 ? `${PALETTE.red.border} ${PALETTE.red.bgSoft}` : ""}`}>
            <div className={`font-heading text-2xl font-bold ${i === 2 ? PALETTE.red.text : "text-[#0A0A10]"}`}>{s.value}</div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className={`${CARD} border-t-3 ${PALETTE.red.borderT}`}>
          <div className={`mb-2 font-mono text-[0.6rem] tracking-[0.15em] ${PALETTE.red.text}`}>EXTRACTIVE MODEL</div>
          <div className="mb-3 font-heading text-lg font-bold text-[#0A0A10]">Platform Gig Worker</div>
        ...
        <div className={`${CARD} border-t-3 ${PALETTE.green.borderT}`}>
          <div className={`mb-2 font-mono text-[0.6rem] tracking-[0.15em] ${PALETTE.green.text}`}>REGENERATIVE MODEL</div>
          <div className="mb-3 font-heading text-lg font-bold text-[#0A0A10]">ImpactSoul Impact Worker</div>
          <div className="flex flex-col gap-1.5 text-[0.8rem] text-[#666]">
            <div className="flex justify-between">
              <span>Hourly Rate</span>
              <span className="font-bold text-[#0A0A10]">$20/hr</span>
