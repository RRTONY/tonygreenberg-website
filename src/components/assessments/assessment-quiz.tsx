  ...
  const handleAnswer = useCallback(
    (archetype: ArchetypeKey) => {
      ...
      if (step < ASSESSMENT_QUESTIONS.length - 1) {
        ...
        const winner = (Object.entries(nextScores) as [ArchetypeKey, number][]).reduce((a, b) =>
          a[1] >= b[1] ? a : b,
        )[0];
        setResult(winner);
      }
      ...
      <div className="mx-auto max-w-2xl px-6 py-16 pb-30">
        {!result ? (
          ...
          <div className="text-center">
            {arch && (
              <arch.icon className="mx-auto mb-4 size-12 text-brand-gold" strokeWidth={1.5} />
            )}
            <h1 className="mb-2 font-heading text-[clamp(2rem,5vw,3rem)] text-foreground">
              {arch!.name}
            </h1>
            <div className="mb-8 font-mono text-[0.8rem] tracking-[0.2em] text-brand-gold uppercase">
              {arch!.tagline}
            </div>
            <p className="mx-auto mb-10 max-w-140 text-lg leading-relaxed text-foreground/80">
              {arch!.description}
            </p>

            <div className="mb-5 font-mono text-[0.7rem] tracking-[0.2em] text-brand-gold uppercase">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {subscribed ? (
                    <>
                      Subscribed
                      <Check aria-hidden="true" className="ml-1 size-3.5" />
                    </>
                  ) : (
                    "Subscribe"
                  )}
