...
function SubSection({ children, dark }: { children: React.ReactNode; dark?: boolean }) {
  return (
    <div className={`border-t py-12 ${dark ? "border-[#3A3530]" : "border-pri-border"}`}>
      {children}
    </div>
  );
}
    ...
    <>
      <section id="compass" className="bg-pri-cream px-5 py-16 text-pri-ink">
        <div className="mx-auto max-w-275">
          ...
          <h2 className="mb-4 font-heading text-[clamp(1.8rem,4.5vw,3rem)] text-pri-ink">
            Assessment — v2
          </h2>
          <p className="mb-6 max-w-200 text-[1.2rem] text-pri-brown italic">
          ...
          <SubSection>
            <div className="mb-4 text-[.85rem] font-extrabold tracking-[0.12em] text-pri-purple uppercase">
              What&apos;s New in Version 2
            </div>
            <p className="mb-4 max-w-200 text-[1.05rem] leading-[1.75] text-pri-brown">
              Version 1 was thin on substance breadth and missed several contraindications that real
              iboga clinics screen for. Version 2 closes those gaps and adds a priority-rank step so
              the user can fine-tune what matters most.
            </p>
          ...
          <SubSection>
                ...
                <tbody>
                  {COMPASS_RANK_MULTIPLIERS.map((r, i) => (
                    <tr key={r.position} className={i % 2 ? "bg-pri-parchment" : ""}>
                      ...
                      <td className={`${tdClass} font-mono font-bold text-pri-purple`}>
                        {r.multiplier}
                      </td>
            ...
            <p className="mt-4 max-w-200 text-[.95rem] text-pri-brown">
      ...
      <section id="compass-questions" className="bg-pri-ink px-5 py-16 text-pri-cream">
        <div className="mx-auto max-w-275">
          ...
          <h2 className="mb-8 font-heading text-[clamp(1.6rem,4vw,2.5rem)] text-pri-cream">
            Full Question Set
          </h2>
          ...
          {COMPASS_QUESTIONS.map((section) => {
            ...
            return (
              <div key={section.sectionNum} className="mb-8">
                ...
                >
                  <span className="min-w-10 font-mono text-[.85rem] font-bold text-pri-purple-light">
                    {String(section.sectionNum).padStart(2, "0")}
                  </span>
                  <span className="flex-1 text-[1.05rem] font-bold text-pri-cream">
                  ...
                  <span
                    className={`text-xl text-pri-purple-light transition-transform ${isOpen ? "rotate-180" : ""}`}
                  >
                    ▾
                  </span>
                </button>
                  ...
                  <div className="rounded-b-lg border border-t-0 border-[#3A3530] bg-white/2 px-5 py-6">
                    {section.questions.map((q) => (
                      <div key={q.id} className="mb-6">
                        ...
                        {q.note && (
                          <p className="mb-2 text-[.9rem] text-pri-cream/45 italic">{q.note}</p>
                        )}
                          ...
                          {q.options.map((opt) => (
                            <li key={opt} className="text-[.95rem] leading-[1.7] text-pri-cream/80">
                              {opt.includes("→") ? (
                                <>
                                  {opt.split("→")[0]}
                                  <span className="font-semibold text-pri-purple-light">
                                    <ForwardIcon aria-hidden="true" /> {opt.split("→")[1]}
                                  </span>
                                </>
          ...
          <p className="mt-8 text-[.95rem] text-pri-cream/45">
            <strong className="text-pri-purple-light">Final Step — Priority Rank:</strong> After 28
            questions, the user sees their pre-ranked 10 dimensions. They can tap up/down arrows to
            reorder. Top 3 weighted ×1.4 to ×2.0. Bottom 3 weighted ×0.5 to ×0.7.
          </p>
        </div>
      </section>
      ...
      <section id="compass-routing" className="bg-pri-cream px-5 py-16 text-pri-ink">
        <div className="mx-auto max-w-275">
          ...
          <h2 className="mb-4 font-heading text-[clamp(1.4rem,3.5vw,2.2rem)] text-pri-ink">
            ...
            {COMPASS_SOURCES.map((s) => (
              <li key={s.url}>
                <a
                  ...
                  className="text-[.95rem] text-pri-purple-light underline"
                >
                  {s.text}
                </a>
              </li>
      ...
      <section id="compass-validity" className="bg-pri-cream px-5 py-12 text-pri-ink">
        <div className="mx-auto max-w-225">
          <div className="border-l-4 border-[#581C87] bg-[#F3E8FF] p-5">
            ...
            <p className="m-0 text-[.95rem] leading-[1.7] text-pri-brown">
              {COMPASS_VALIDITY_NOTICE}
            </p>
          </div>
        </div>
      ...
      <section id="compass-reading" className="bg-pri-ink px-5 py-12 text-pri-cream">
        <div className="mx-auto max-w-225">
          ...
          <p className="text-center font-heading text-[1.1rem] text-pri-purple-light italic">
          ...
          <div className="my-10 text-center">
            <Link
              href="/iboga-compass"
              className="inline-block rounded-lg bg-linear-to-br from-pri-purple to-pri-purple-mid px-10 py-4 text-[1.1rem] font-bold tracking-[0.04em] text-white shadow-[0_4px_20px_rgba(107,33,168,.35)]"
            >
              Take the Compass Assessment
            </Link>
            <p className="mt-3 text-[.85rem] text-pri-cream/45">
              28 questions · 5 minutes · ranked facility matches
            </p>
          </div>
