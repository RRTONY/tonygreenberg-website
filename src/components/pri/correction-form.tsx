...
export function CorrectionForm({ medicine, onClose }: { medicine: MedicineWithSafety; onClose: () => void }) {
  ...
  return (
    <div className="py-6">
      ...
      <div className="mb-4">
        <label className="mb-1.5 block text-xs font-bold tracking-[0.08em] text-pri-tan uppercase">Your Suggested Correction *</label>
        <textarea
      ...
      <div className="mb-4">
        <label className="mb-1.5 block text-xs font-bold tracking-[0.08em] text-pri-tan uppercase">Source / Citation (optional)</label>
        <input type="url" value={sourceUrl} onChange={(e) => setSourceUrl(e.target.value)} placeholder="https://pubmed.ncbi.nlm.nih.gov/..." className={inputClass} />
      </div>

      <div className="mb-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
          <label className="mb-1.5 block text-xs font-bold tracking-[0.08em] text-pri-tan uppercase">Your Name (optional)</label>
          <input type="text" value={submitterName} onChange={(e) => setSubmitterName(e.target.value)} placeholder="Name" className={inputClass} />
        </div>
        <div>
          <label className="mb-1.5 block text-xs font-bold tracking-[0.08em] text-pri-tan uppercase">Email (optional)</label>
          <input type="email" value={submitterEmail} onChange={(e) => setSubmitterEmail(e.target.value)} placeholder="email@example.com" className={inputClass} />
        </div>
      </div>

      <div className="flex gap-3">
        <button onClick={onClose} className="flex-1 border-[1.5px] border-pri-border px-6 py-3 font-mono text-sm font-bold tracking-wide text-pri-tan uppercase">
          Cancel
        </button>
        <button
