  ...
  return (
    <section className="px-5 py-16 md:py-24">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <h1 className="mb-4 font-heading text-[clamp(36px,8vw,88px)] leading-[1.1] font-bold text-kava-ink">
          {title}
        </h1>
        {subtitle && (
          <p className="mx-auto max-w-2xl text-lg leading-[1.75] text-kava-ink/70 md:text-xl">
            {subtitle}
          </p>
        )}
      </div>
...
export function KavaSection({
  ...
  return (
    <section
      className={`px-5 py-12 md:py-16 ${bg === "sand" ? "bg-kava-sand" : bg === "white" ? "bg-white" : ""} ${className}`}
    >
...
export function KavaSectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="mb-6 font-heading text-[clamp(24px,4vw,40px)] leading-[1.2] font-bold text-kava-ink">
      {children}
    </h2>
  );
}
...
export function KavaCard({
  ...
  return (
    <div
      className={`rounded-xl border border-kava-sand-muted bg-white p-6 transition-all duration-200 ${className}`}
      onClick={onClick}
    >
      {children}
    </div>
...
export function KavaBadge({
  ...
  children: React.ReactNode;
  color?: keyof typeof BADGE_CLASS;
}) {
  return (
    <span
      className={`inline-block rounded-full px-2.5 py-1 text-xs font-bold tracking-wide uppercase ${BADGE_CLASS[color] ?? BADGE_CLASS.saffron}`}
    >
      {children}
    </span>
  );
...
export function KavaDivider() {
  return (
    <div className="mx-auto my-8 h-0.5 max-w-30 bg-linear-to-r from-transparent via-kava-saffron to-transparent" />
  );
}
...
export function KavaDisclaimer() {
  return (
    <div className="mt-8 rounded-lg bg-kava-sand-muted p-4 text-sm leading-[1.75] text-kava-ink/80">
      <strong>Disclaimer:</strong> For educational and harm reduction purposes. Consult a licensed
      medical professional before making any changes to medications or health protocols.
    </div>
  );
}
