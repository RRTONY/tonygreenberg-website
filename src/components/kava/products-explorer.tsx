...
function Stars({ count }: { count: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <div key={n} className={`size-4 rounded-full ${n <= count ? "bg-kava-saffron" : "bg-kava-sand-muted"}`} />
      ))}
    </div>
  );
}
        ...
        {category.products.map((p) => (
          <KavaCard key={p.name}>
            ...
            <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {p.cultivar && (
                <div>
                  <p className="text-xs font-bold tracking-wide text-kava-saffron uppercase">Cultivar</p>
                  <p className="text-sm text-kava-ink">{p.cultivar}</p>
                </div>
              )}
              {p.chemotype && (
                <div>
                  <p className="text-xs font-bold tracking-wide text-kava-saffron uppercase">Chemotype</p>
              ...
              {p.extra && (
                <div className="sm:col-span-2">
                  <p className="text-xs font-bold tracking-wide text-kava-saffron uppercase">Details</p>
