...
type Severity = "HARD STOP" | "CRITICAL" | "MODERATE" | "CAUTION" | "COMPLEX" | "RESEARCH FRONTIER" | "GENERALLY OK" | "SYNERGISTIC" | "ELEVATED RISK" | "ADDITIVE" | "NOTABLE";
...
export function InteractionsExplorer() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      results = results.filter((i) => i.substance.toLowerCase().includes(q) || i.aliases.some((a) => a.toLowerCase().includes(q)));
    }
          ...
          return (
            <KavaCard key={interaction.substance}>
              <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
                <h3 className="font-heading text-lg font-bold text-kava-ink">{interaction.substance}</h3>
                <span className={`rounded-full border px-3 py-1 text-xs font-bold tracking-wide uppercase ${cfg.borderClass} ${cfg.bgClass} ${cfg.textClass}`}>
                  {interaction.severity}
                </span>
              </div>
              <div className={`mb-3 rounded-lg border-l-3 p-3 ${cfg.borderClass} ${cfg.bgClass}`}>
                <p className={`mb-1 text-xs font-bold tracking-wide uppercase ${cfg.textClass}`}>Mechanism</p>
                <p className="text-sm leading-[1.75] text-kava-ink">{interaction.mechanism}</p>
              </div>
              <div className="rounded-lg bg-kava-sand-muted/50 p-3">
                <p className="mb-1 text-xs font-bold tracking-wide text-kava-saffron uppercase">PRI Protocol</p>
