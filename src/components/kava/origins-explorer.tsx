...
export const ISLANDS: Island[] = [
  {
    ...
    ceremony: "Traditionally men-only nakamals; kava opens Parliament sessions, welcomes foreign ambassadors, celebrates independence day.",
  ...
  },
  {
    ...
    ceremony: "Sevusevu gifting ceremony is the gateway to Fijian hospitality. The tanoa bowl is a national symbol.",
  ...
  },
  {
    ...
    ceremony: "'Awa is good for the farmer when he is weary and sore after laboring day and night, and for the fisherman who has been diving, rowing, pulling and bending.",
  ...
  },
  {
    ...
    ceremony: "Outside of Vanuatu, Solomon Islands, and Papua New Guinea, there are no indigenous varieties with heavy kavalactones — Pohnpei is the exception.",
  },
];
          ...
          return (
            <button
              ...
              className={`rounded-xl border-2 p-4 text-center transition-all hover:shadow-md ${
                isActive ? `${island.activeBgClass} border-transparent text-white` : "border-kava-sand-muted bg-white text-kava-ink"
              }`}
            >
      ...
      {active && (
        <KavaCard>
          <div className="mb-4 flex items-start justify-between">
            <div>
              <h3 className={`font-heading text-2xl font-bold ${active.colorClass}`}>{active.name}</h3>
          ...
          {active.cultivar && (
            <div className="mb-4 rounded-lg border-l-3 border-l-kava-saffron bg-kava-saffron/10 p-4">
              <p className="mb-1 text-xs font-bold tracking-wide text-kava-saffron uppercase">Key Cultivar: {active.cultivar.name}</p>
          ...
          {active.safety && (
            <div className="rounded-lg border-l-3 border-l-kava-terracotta bg-kava-terracotta/10 p-4">
              <p className="mb-1 text-xs font-bold tracking-wide text-kava-terracotta uppercase">Safety Note</p>
