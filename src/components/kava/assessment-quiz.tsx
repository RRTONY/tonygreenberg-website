...
const DOMAINS: Domain[] = [
  {
    ...
    questions: [
      ...
      {
        id: "s3",
        text: "No active infection, surgery, or acute illness?",
        type: "yesno",
        points: 4,
      },
      {
        id: "s4",
        text: "No active liver disease or significantly elevated liver enzymes?",
        type: "yesno",
        points: 4,
      },
      {
        id: "s5",
        text: "Full medication review completed and cleared via Drug Interaction Checker?",
        type: "yesno",
        points: 4,
      },
      {
  ...
  },
  {
    ...
    questions: [
      {
        id: "p1",
        text: "Intention formulated clearly and written down?",
        type: "yesno",
        points: 4,
      },
      ...
      {
        id: "p3",
        text: "No active psychosis, mania, or dissociative disorder?",
        type: "yesno",
        points: 5,
      },
      {
        id: "p4",
        text: "Previous altered state experience? (context, not scored)",
        type: "yesno",
        points: 0,
      },
      {
  ...
  },
  {
    ...
    questions: [
      {
        id: "r1",
        text: "Journey group formed and met at least twice before ceremony?",
        type: "yesno",
        points: 4,
      },
      {
        id: "r2",
        text: "Facilitator rapport established over minimum 2 sessions?",
        type: "yesno",
        points: 4,
      },
      {
      ...
      {
        id: "r4",
        text: "Participant willing to speak truth in group circle?",
        type: "yesno",
        points: 4,
      },
    ],
  },
  {
    ...
    questions: [
      {
        id: "c1",
        text: "Participant understands Pacific Island origin of kava and its significance?",
        type: "yesno",
        points: 3,
      },
      {
        id: "c2",
        text: "Lineage acknowledgment demonstrated — authentic, not performative?",
        type: "yesno",
        points: 3,
      },
      {
        id: "c3",
        text: "Demonstrates reciprocity orientation toward plant medicines generally?",
        type: "yesno",
        points: 3,
      },
      {
  ...
  },
  {
    ...
    questions: [
      {
        id: "i1",
        text: "Post-journey support structure identified before ceremony date?",
        type: "yesno",
        points: 3,
      },
      {
        id: "i2",
        text: "Integration circle scheduled within 7 days of journey?",
        type: "yesno",
        points: 3,
      },
      {
        id: "i3",
        text: "Trusted community member or therapist available for follow-up?",
        type: "yesno",
        points: 4,
      },
...
const RECOMMENDATION_STYLES: Record<
  Recommendation,
  { text: string; ring: string; softBg: string; desc: string }
> = {
  CLEARED: {
    text: "text-[#16a34a]",
    ring: "border-[#16a34a]",
    softBg: "bg-[#16a34a]/10",
    desc: "Proceed to journey.",
  },
  CONDITIONAL: {
    text: "text-[#d97706]",
    ring: "border-[#d97706]",
    softBg: "bg-[#d97706]/10",
    desc: "Address flagged domains; re-assess within 2 weeks.",
  },
  DEFER: {
    text: "text-[#ea580c]",
    ring: "border-[#ea580c]",
    softBg: "bg-[#ea580c]/10",
    desc: "Additional preparation work required; 30-day minimum before re-assessment.",
  },
  "NOT READY": {
    text: "text-[#dc2626]",
    ring: "border-[#dc2626]",
    softBg: "bg-[#dc2626]/10",
    desc: "Significant support intervention needed before PRI pathway continues.",
  },
};
        ...
        <KavaSection>
          <div ref={resultsRef}>
            <div className="mb-8 text-center">
              <div
                className={`mx-auto mb-4 flex h-40 w-40 flex-col items-center justify-center rounded-full border-[6px] ${recStyle.ring} ${recStyle.softBg}`}
              >
                <p className={`font-heading text-5xl font-bold ${recStyle.text}`}>{totalScore}</p>
                <p className={`text-xs font-bold tracking-wide uppercase ${recStyle.text}`}>
                  / 100
                </p>
              </div>
              <p className={`mb-2 font-heading text-2xl font-bold ${recStyle.text}`}>
                {recommendation}
              </p>
                ...
                return (
                  <KavaCard key={d.id}>
                    ...
                    <div className="h-3 rounded-full bg-kava-sand-muted">
                      <div
                        className={`h-3 rounded-full transition-all duration-500 ${style.bar}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </KavaCard>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            <button
              onClick={reset}
              className="flex items-center gap-2 rounded-lg bg-kava-saffron px-6 py-3 text-sm font-bold text-white"
            >
              <RotateCcw size={16} />
              Start Over
            </button>
    ...
    <>
      ...
      <KavaSection>
        <div className="mb-8 flex items-center gap-2 overflow-x-auto pb-2">
          {DOMAINS.map((d, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`flex size-5 items-center justify-center rounded-full text-xs ${active ? style.iconActive : style.iconInactive}`}
                >
                  {i + 1}
                </span>
        ...
        <div className="mb-6">
          ...
          <h2 className={`mt-2 font-heading text-2xl font-bold ${domainStyle.text}`}>
            {domain.name}
          </h2>
        ...
        <div className="flex items-center justify-between">
          ...
          {currentDomain < DOMAINS.length - 1 ? (
            ...
            <button
              ...
              className="rounded-lg bg-kava-saffron px-6 py-2.5 text-sm font-bold text-white"
            >
              View Results
            </button>
          )}
