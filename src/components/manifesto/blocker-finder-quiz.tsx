...
function getThreatLevel(volume: string | undefined) {
  if (volume === "extreme")
    return {
      label: "CRITICAL",
      colorClass: "text-crusade-red border-crusade-red bg-crusade-red/12",
      desc: "Your inbox has been overrun. Immediate action required.",
    };
  if (volume === "high")
    return {
      label: "HIGH",
      colorClass: "text-crusade-ember border-crusade-ember bg-crusade-ember/12",
      desc: "Your inbox is actively under siege.",
    };
  return {
    label: "MODERATE",
    colorClass: "text-brand-gold border-brand-gold bg-brand-gold/12",
    desc: "Your inbox is under pressure but manageable.",
  };
}
      ...
      <>
        <div className="mb-8 flex items-center gap-2">
          {QUESTIONS.map((_, i) => (
            <div
              key={i}
              className={`h-1.5 flex-1 rounded-full transition-all ${i <= step ? "bg-crusade-teal" : "bg-black/6"}`}
            />
          ))}
        </div>
  ...
  return (
    <>
      <div className="mb-8 text-center">
        ...
        <div
          className={`mb-4 inline-block rounded-full border-2 px-6 py-2 text-xl font-bold ${threat.colorClass}`}
        >
          THREAT LEVEL: {threat.label}
        </div>
        <p className="text-lg text-crusade-muted">{threat.desc}</p>
      </div>
      <h3 className="mb-6 font-heading text-2xl font-bold text-crusade-ink">
          ...
          <GlassCard key={tool.name} variant={i === 0 ? "danger" : "teal"} glow={i === 0}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="mb-2 flex items-center gap-2">
                  <span
                    className={`rounded-full px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase ${
                      i === 0
                        ? "bg-crusade-red/10 text-crusade-red"
                        : "bg-crusade-teal/10 text-crusade-teal"
                    }`}
                  >
                    {i === 0 ? "TOP PICK" : `#${i + 1}`}
                  </span>
                ...
                <h4 className="mb-1 font-heading text-xl font-bold text-crusade-ink">
                  {tool.name}
                </h4>
                ...
                <div className="mt-2 flex items-center gap-1">
                  {Array.from({ length: 5 }).map((_, s) => (
                    <Star
                      ...
                      size={16}
                      className={
                        s < tool.rating ? "fill-brand-gold text-brand-gold" : "text-black/10"
                      }
                    />
                  ))}
                </div>
      ...
      <div className="mt-6 text-center">
        <button
          onClick={reset}
          className="text-sm font-medium text-crusade-teal/60 hover:underline"
        >
          Retake Assessment
        </button>
      </div>
