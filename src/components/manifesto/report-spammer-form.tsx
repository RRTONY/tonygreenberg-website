...
const inputClass =
  "w-full rounded-xl border border-black/10 bg-white/60 px-4 py-3.5 text-base text-crusade-ink outline-none";
...
export function ReportSpammerForm() {
  ...
  const validate = () => {
    ...
    if (!form.senderEmail.trim() || !form.senderEmail.includes("@"))
      e.senderEmail = "Valid email required";
    ...
    return (
      <GlassCard variant="teal" glow className="mx-auto max-w-lg text-center">
        ...
        <p className="mb-6 text-base leading-relaxed text-crusade-muted">
          Your email app just opened with your report ready to send. Send it, and it goes straight
          to Tony — there&apos;s no public database yet, so this is reviewed by hand for now.
        </p>
        <button
          onClick={() => {
            setSubmitted(false);
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="mb-6 flex items-start gap-3 rounded-xl border border-crusade-red/12 bg-crusade-red/6 p-4">
        ...
        <p className="text-sm leading-relaxed text-crusade-brown">
          This opens your email app with a report addressed to Tony directly — only submit factual
          information about actual spam you have received.
        </p>
      </div>
      ...
      <GlassCard>
        <div className="space-y-5">
          <div>
            ...
            />
            {errors.companyName && (
              <p className="mt-1 text-[0.8rem] text-crusade-red">{errors.companyName}</p>
            )}
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-bold text-crusade-ink">
            ...
            />
            {errors.senderEmail && (
              <p className="mt-1 text-[0.8rem] text-crusade-red">{errors.senderEmail}</p>
            )}
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-bold text-crusade-ink">
            ...
            {errors.spamType && (
              <p className="mt-1 text-[0.8rem] text-crusade-red">{errors.spamType}</p>
            )}
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-bold text-crusade-ink">
            ...
            {errors.frequency && (
              <p className="mt-1 text-[0.8rem] text-crusade-red">{errors.frequency}</p>
            )}
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-bold text-crusade-ink">
            ...
            />
            {errors.description && (
              <p className="mt-1 text-[0.8rem] text-crusade-red">{errors.description}</p>
            )}
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-bold text-crusade-ink">
