...
export function CompareExplorer({ coffees }: { coffees: CatalogItem[] }) {
  ...
  const scored = useMemo(
    () => coffees.map((c) => ({ ...c, scores: computeToolScores(c) })),
    [coffees],
  );
  ...
  const rows = [
    ...
    {
      label: "Cupping",
      fn: (c: (typeof selected)[0]) => (c.cuppingScore ? `${c.cuppingScore}` : "—"),
    },
    ...DIMS.map((d) => ({
      label: d.toUpperCase(),
      fn: (c: (typeof selected)[0]) => c.scores[d].toFixed(1),
    })),
          ...
          <table className="w-full border-collapse text-[0.85rem]">
            <thead>
              <tr className="border-b-2 border-[#6F4E37]/10">
                ...
                {selected.map((c) => (
                  <th
                    key={c.id}
                    className="px-2 py-3 text-center font-heading text-[0.9rem] text-[#2C1810]"
                  >
            ...
            <tbody>
              {rows.map((row, i) => (
                <tr
                  key={row.label}
                  className={`border-b border-[#6F4E37]/5 ${i % 2 ? "bg-[#6F4E37]/2" : ""}`}
                >
