...
export function ExperiencesExplorer({ experiences }: { experiences: ExperienceEntry[] }) {
  ...
  const filtered = selected === "all" ? experiences : experiences.filter((e) => e.country === selected);
        ...
        {["all", ...countries].map((c) => (
          <button
            ...
            className={`rounded-full px-4 py-2 font-mono text-[0.7rem] ${
              selected === c ? "border-2 border-[#6F4E37] bg-[#6F4E37]/6 text-[#6F4E37]" : "border border-[#6F4E37]/15 text-[#6B5B4F]"
            }`}
          >
            {c === "all" ? "All" : c}
          </button>
