...
export function RoastersExplorer({ roasters }: { roasters: ProducerEntry[] }) {
  ...
  const filtered = roasters.filter((r) => {
    if (search && !r.name.toLowerCase().includes(search.toLowerCase()) && !`${r.country} ${r.region || ""}`.toLowerCase().includes(search.toLowerCase())) {
      return false;
    }
        ...
        {filtered.map((r) => (
          <div key={r.id} className="rounded-xl border border-[#6F4E37]/8 bg-white p-6">
            <div className="mb-1.5 flex items-start justify-between gap-2">
              ...
              <span className={`shrink-0 rounded-full px-2 py-0.5 font-mono text-[0.65rem] ${GRADE_TAG_CLASS[r.overallGrade] ?? DEFAULT_TAG_CLASS}`}>
