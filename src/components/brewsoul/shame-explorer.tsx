  ...
  return (
    <>
        ...
        {["all", ...cats].map((c) => (
          <button
            ...
            className={`rounded-full px-4 py-2 font-mono text-[0.7rem] ${
              cat === c ? "border-2 border-[#8B2500] bg-[#8B2500]/6 text-[#8B2500]" : "border border-[#6F4E37]/15 text-[#6B5B4F]"
            }`}
          >
            {c === "all" ? "All" : c}
          </button>
      ...
      <div className="flex flex-col gap-4">
        {filtered.map((s) => (
          <div key={s.id} className="rounded-lg border border-[#6F4E37]/8 border-l-4 border-l-[#8B2500] bg-white p-5">
            <div className="mb-2 flex items-start justify-between gap-2">
              <h3 className="font-heading text-[1.05rem] font-bold text-[#2C1810]">
                #{s.rank} {s.brand}
              </h3>
              <span className="shrink-0 rounded-full bg-[#8B2500]/6 px-2.5 py-1 font-mono text-[0.62rem] text-[#8B2500]">{s.category}</span>
            </div>
