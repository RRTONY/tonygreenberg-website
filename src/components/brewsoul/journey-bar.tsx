...
function hydrateJourney() {
  ...
  void readVisitorState<BrewSoulJourneyState>("brewsoul")
    .then((state) => {
      const ids = Array.isArray(state?.visited)
        ? state.visited.filter((id): id is string => typeof id === "string")
        : [];
      visitedSteps = new Set(ids);
      emitJourneyChange();
    })
    .catch(() => undefined);
}
...
const JOURNEY_STEPS = [
  { id: "identity", label: "Identity Quiz", path: "/brewsoul", short: "Identity" },
  {
    id: "first-sip",
    label: "The First Sip",
  ...
  { id: "home", label: "Dashboard", path: "/brewsoul/home", short: "Home" },
  { id: "quiz", label: "Taste Quiz", path: "/brewsoul/quiz", short: "Taste" },
  { id: "browse", label: "Catalog", path: "/brewsoul/browse", short: "Browse" },
  { id: "chains", label: "Chain Rankings", path: "/brewsoul/chains", short: "Chains" },
] as const;
  ...
  "/brewsoul/decaf": [
    {
      ...
      why: "Every certified farm on earth",
    },
    { label: "Coffee & Health", path: "/brewsoul/health", why: "What science says" },
    {
      label: "Coffee Prescription",
...
export function JourneyBar() {
  ...
  const visited = useSyncExternalStore(
    subscribeJourney,
    getJourneySnapshot,
    () => EMPTY_VISITED as Set<string>,
  );

  const currentStep = JOURNEY_STEPS.find((s) =>
    s.path === "/brewsoul" ? pathname === "/brewsoul" : pathname.startsWith(s.path),
  );
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-[9990] border-t border-[#C5A23C]/20 bg-[#2C1810]/92 px-3 py-2.5 backdrop-blur-md sm:px-8">
        ...
        {JOURNEY_STEPS.map((step) => {
          ...
          return (
            <Link key={step.id} href={step.path} className="flex items-center gap-1">
              ...
              />
              {isCurrent && (
                <span className="font-mono text-[0.58rem] tracking-wide text-[#D4B96A] uppercase">
                  {step.short}
                </span>
              )}
            </Link>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        <span className="inline-flex items-center gap-1 font-mono text-[0.58rem] tracking-wide text-white/40 uppercase">
          Next
...
// Not a hook — it can be called by a small client island on server-rendered
// pages to persist the real route visit through the managed state boundary.
export async function markVisited(stepId: string) {
  const state = await readVisitorState<BrewSoulJourneyState>("brewsoul");
  const visited = Array.isArray(state?.visited)
    ? state.visited.filter((id): id is string => typeof id === "string")
    : [];
  if (visited.includes(stepId)) return;
  await writeVisitorState("brewsoul", { visited: [...visited, stepId] });
}
