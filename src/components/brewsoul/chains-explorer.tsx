...
function ScoreBar({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`w-6 text-right font-mono text-[0.72rem] font-bold ${textCls}`}>{value}</span>
    </div>
  );
}
...
function ChainCard({ chain, expanded, onToggle }: { chain: ChainEntry; expanded: boolean; onToggle: () => void }) {
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="mb-1 flex items-center gap-3">
            <span className={`font-mono text-[1.1rem] font-bold ${TIER_TEXT_CLASS[chain.tier]}`}>#{chain.rank}</span>
      ...
      <p className={`text-[0.88rem] leading-relaxed text-[#6B5B4F] ${expanded ? "mt-3 mb-4" : "mt-3"}`}>{chain.verdict}</p>
      ...
      {expanded && (
        <div className="mt-2">
          ...
          <div className="mb-4 grid grid-cols-2 gap-4">
            ...
            <div>
              <div className="mb-1 font-mono text-[0.62rem] tracking-[0.15em] text-[#8B2500] uppercase">Weaknesses</div>
...
export function ChainsExplorer({ chains }: { chains: ChainEntry[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQ.trim()) {
      ...
      list = list.filter((c) => c.name.toLowerCase().includes(q) || c.hq.toLowerCase().includes(q) || c.verdict.toLowerCase().includes(q));
    }
    return list;
  ...
  return (
    <>
        ...
        {(["all", "specialty", "premium", "mass-market", "fast-food"] as const).map((t) => (
          <button
            ...
            className={`rounded-full px-3 py-1.5 font-mono text-[0.68rem] ${
              typeFilter === t ? "border-2 border-[#6F4E37] bg-[#6F4E37]/6 text-[#6F4E37]" : "border border-[#6F4E37]/10 text-[#6B5B4F]"
            }`}
          >
      ...
      <div className="flex flex-col gap-3">
        {filtered.map((chain) => (
          <ChainCard key={chain.id} chain={chain} expanded={expandedId === chain.id} onToggle={() => setExpandedId(expandedId === chain.id ? null : chain.id)} />
        ))}
      </div>
