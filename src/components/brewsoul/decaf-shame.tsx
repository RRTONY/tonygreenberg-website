  ...
  return (
    <>
      ...
      {show && (
        <div>
          {SHAME.map((b) => (
            <div key={b.name} className="mb-2.5 rounded-md border border-[#9b3030]/20 border-l-4 border-l-[#9b3030] bg-[#9b3030]/8 p-5">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <div className="text-base font-bold text-[#e8dcc8]">{b.name}</div>
                  <div className="mt-0.5 font-mono text-[10px] tracking-wide text-[#9b3030]">Process: {b.process}</div>
                </div>
          ...
          <div className="mt-5 rounded-lg border border-white/10 bg-white/5 p-6">
            <div className="mb-2 text-sm font-bold text-[#e8dcc8]">Clean Label Project Lawsuits (2020-present)</div>
            <p className="text-sm leading-relaxed text-[#f5efe0]/88">
              CLP filed lawsuits in D.C. Superior Court against AmazonFresh, J.M. Smucker&apos;s (Café Bustelo), Peet&apos;s Coffee &amp; Tea Holdco, Keurig
              Green Mountain, and Kraft Heinz (Maxwell House) for false and misleading advertising — labeling products as &ldquo;pure&rdquo; or
              &ldquo;natural&rdquo; while containing methylene chloride residue without disclosure. The Organic &amp; Natural Health Association endorsed
              CLP&apos;s findings.
            </p>
          </div>

          <div className="mt-4 rounded-lg border border-white/10 bg-white/5 p-6">
            <div className="mb-2 text-sm font-bold text-[#e8dcc8]">The Industry&apos;s Defense (and Why It&apos;s Weak)</div>
            <p className="text-sm leading-relaxed text-[#f5efe0]/88">
              A coffee industry group wrote the FDA claiming that &ldquo;true coffee aficionados in blind tastings&rdquo; prefer MC-processed decaf. As STAT
              reporter Nick Florko noted: &ldquo;which is a pretty funny claim if you consider the fact that we&apos;re talking about coffee here
              that&apos;s essentially rinsed in paint thinner.&rdquo; The FDA&apos;s risk assessment is from 1985, assumes a 5oz cup size (less than half a
