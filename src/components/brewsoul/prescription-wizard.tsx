  ...
  return (
    <button
      ...
      className={`rounded-lg px-4.5 py-2.5 text-sm transition-all ${
        selected ? "border-2 border-[#c4873b] bg-[#c4873b]/8 font-semibold text-[#1a0e08]" : "border border-[#5d3a28]/12 bg-white text-[#5c3a28]"
      }`}
    >
      {children}
    </button>
...
function generatePrescription(a: Answers) {
  ...
  return { firstCupTime, lastCafTime, roast, roastReason, cafCups, decafCups, brands, decafBrand, dailyCost, warnings };
}
...
export function PrescriptionWizard() {
  ...
  const toggleList = (key: "healthGoals" | "flavorPref", val: string) => {
    setAnswers((prev) => ({ ...prev, [key]: prev[key].includes(val) ? prev[key].filter((v) => v !== val) : [...prev[key], val] }));
  };
  ...
  return (
    <div className="bg-[#f5efe0] text-[#2d1810]">
      <section className="bg-linear-to-br from-[#1a0e08] via-[#2d1810] to-[#3d2517] px-7 pt-24 pb-16 text-center">
        <div className="mb-4 font-mono text-[11px] tracking-[0.3em] text-[#c5a23c] uppercase">BrewSoul · Personalized Intelligence</div>
        <h1 className="mb-4 font-heading text-[clamp(32px,5vw,56px)] leading-[1.1] font-bold text-[#f5efe0] italic">Your Coffee Prescription</h1>
        <p className="mx-auto max-w-xl text-[17px] leading-relaxed text-[#f5efe0]/65">
          Answer 5 questions. Get a personalized daily protocol based on your genetics, timing, health goals, and taste — with specific brand
      ...
      <div className="mx-auto max-w-175 px-7 pt-12 pb-20">
        {step === "genetics" && (
          <div>
            <h2 className="mb-2 font-heading text-[28px] font-bold text-[#1a0e08] italic">How does caffeine affect you?</h2>
            <p className="mb-7 text-[15px] leading-relaxed text-[#5c3a28]">
              This determines your CYP1A2 metabolizer type — the single most important factor in your coffee prescription.
            </p>
              ...
              {[
                ...
                <button
                  ...
                  className={`rounded-lg p-5 text-left ${
                    answers.caffeineSensitivity === o.val ? "border-2 border-[#c4873b] bg-[#c4873b]/6" : "border border-[#5d3a28]/10 bg-white"
                  }`}
                >
        ...
        {step === "timing" && (
          <div>
            <h2 className="mb-2 font-heading text-[28px] font-bold text-[#1a0e08] italic">When do you wake up?</h2>
            <p className="mb-7 text-[15px] leading-relaxed text-[#5c3a28]">
              Your cortisol awakening response (CAR) peaks 30-45 min after waking. We&apos;ll time your first cup to the dip.
            </p>
            <div className="mb-8 flex flex-wrap gap-2.5">
              {["5:00", "5:30", "6:00", "6:30", "7:00", "7:30", "8:00", "8:30", "9:00"].map((t) => (
                <Chip key={t} selected={answers.wakeTime === t} onClick={() => update({ wakeTime: t })}>
                  {t} AM
                </Chip>
              ))}
            </div>
            <h3 className="mb-2 font-heading text-[22px] font-bold text-[#1a0e08] italic">How many cups per day?</h3>
            <div className="mb-8 flex gap-2.5">
              {[1, 2, 3, 4, 5].map((n) => (
                <Chip key={n} selected={answers.cupsPerDay === n} onClick={() => update({ cupsPerDay: n })}>
                  {n}
                </Chip>
              ))}
            </div>
            <h3 className="mb-2 font-heading text-[22px] font-bold text-[#1a0e08] italic">Sleep optimization priority?</h3>
            <div className="mb-4 flex flex-wrap gap-2.5">
              {[
                ...
                <Chip key={o.val} selected={answers.sleepGoal === o.val} onClick={() => update({ sleepGoal: o.val })}>
                  {o.label}
                </Chip>
              ))}
            </div>
            <label className="mt-4 flex cursor-pointer items-center gap-2.5 text-sm text-[#5c3a28]">
              <input type="checkbox" checked={answers.pregnant} onChange={(e) => update({ pregnant: e.target.checked })} className="size-4.5" />
        ...
        {step === "health" && (
          <div>
            <h2 className="mb-2 font-heading text-[28px] font-bold text-[#1a0e08] italic">What are your health goals?</h2>
            <p className="mb-7 text-[15px] leading-relaxed text-[#5c3a28]">
              Select all that apply. We&apos;ll optimize roast level, brand, and timing for your specific goals.
            </p>
        ...
        {step === "flavor" && (
          <div>
            <h2 className="mb-2 font-heading text-[28px] font-bold text-[#1a0e08] italic">What flavors do you love?</h2>
            <p className="mb-7 text-[15px] leading-relaxed text-[#5c3a28]">Select all that appeal to you. This shapes roast and origin recommendations.</p>
        ...
        {step === "budget" && (
          <div>
            <h2 className="mb-2 font-heading text-[28px] font-bold text-[#1a0e08] italic">What&apos;s your budget?</h2>
            <p className="mb-7 text-[15px] leading-relaxed text-[#5c3a28]">Better coffee often costs less than daily Starbucks. We&apos;ll find the best value for your tier.</p>
            <div className="flex flex-col gap-3">
              {[
        ...
        {step === "result" && rx && (
          <div>
            <div className="mb-10 text-center">
              <div className="mb-2 font-mono text-[11px] tracking-[0.2em] text-[#c5a23c]">Your Prescription</div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[13px] text-[#5c3a28]">
                      ...
                      {rx.cafCups > 1 ? `${rx.cafCups - 1} more caffeinated cup(s) before ${rx.lastCafTime}.` : ""}
                    </div>
                  </div>
                </div>
                {rx.decafCups > 0 && (
            ...
            <div className="mb-5 rounded-lg border border-[#5d3a28]/8 border-l-4 border-l-[#c4873b] bg-white p-6">
              <div className="mb-2 font-mono text-[11px] tracking-wide text-[#c5a23c]">Recommended Roast</div>
              <div className="mb-1.5 font-heading text-[22px] font-bold text-[#1a0e08] italic">{rx.roast}</div>
              <div className="text-sm leading-relaxed text-[#5c3a28]">{rx.roastReason}</div>
            </div>

            <div className="mt-8 mb-3 font-mono text-[11px] tracking-wide text-[#c5a23c]">Caffeinated Picks</div>
            {rx.brands.map((b) => (
              <Link key={b.name} href={b.link} className="mb-2.5 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-[#5d3a28]/8 bg-white p-5">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[15px] font-bold text-[#1a0e08]">{b.name}</span>
                    <span className="rounded-sm bg-[#3a7a4a] px-1.5 py-0.5 font-mono text-[9px] tracking-wide text-white">{b.tier}</span>
                  </div>
            ...
            {rx.decafCups > 0 && (
              <>
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-[15px] font-bold text-[#1a0e08]">{rx.decafBrand.name}</span>
                      <span className="rounded-sm bg-[#4a7c8c] px-1.5 py-0.5 font-mono text-[9px] tracking-wide text-white">{rx.decafBrand.tier}</span>
                    </div>
            ...
            <div className="mt-8 rounded-lg bg-[#1a0e08] p-7">
              <div className="mb-3 font-mono text-[11px] tracking-wide text-[#c5a23c]">The Simplification</div>
              <div className="mb-3 font-heading text-xl font-bold text-[#f5efe0] italic">Fewer cups. Better cups. Right timing.</div>
              <div className="text-sm leading-relaxed text-[#f5efe0]/70">
                Your prescription: {rx.cafCups} caffeinated + {rx.decafCups} decaf per day, {rx.roast.toLowerCase()} roast, first cup at {rx.firstCupTime},
                last caf by {rx.lastCafTime}. Estimated daily cost: ${rx.dailyCost} — less than a single Starbucks latte. Maximum bioactive compounds. Zero
                pesticides. Zero mycotoxins. Protected sleep.
              </div>
            </div>
