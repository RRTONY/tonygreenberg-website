    ...
    <>
      ...
      {expanded && (
        <div className="flex flex-col gap-3">
          {risks.map((r) => (
            <div key={r.title} className={`rounded-lg border border-white/10 bg-white/5 p-5 border-l-4 ${SEVERITY_BORDER_CLASS[r.severity]}`}>
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="text-base font-bold text-[#f5efe0]">{r.title}</div>
                <span className={`rounded-sm px-2 py-0.5 font-mono text-[9px] tracking-wide text-white ${SEVERITY_BADGE_CLASS[r.severity]}`}>{r.severity}</span>
              </div>
