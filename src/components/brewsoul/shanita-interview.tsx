...
const QAS: QA[] = [
  {
    ...
    highlight:
      "Fair Trade USA makes 70% of its $10M budget from licensing fees. The system exists to market Fair Trade, not protect farmers. Non-certified farmers often earn more per pound than Fair Trade farmers.",
    ...
    sources: [
      {
        ...
        url: "https://ssir.org/articles/entry/the_problem_with_fair_trade_coffee",
      },
      {
        label: "Camano Island Coffee",
        url: "https://camanoislandcoffee.com/fair-trade-coffee-problems/",
      },
    ],
  },
  {
    ...
    sources: [
      {
        label: "Fair World Project",
        url: "https://fairworldproject.org/low-prices-and-exploitation-recurring-themes-in-coffee/",
      },
      {
        label: "Coffee Intelligence",
        url: "https://intelligence.coffee/2025/02/high-coffee-prices-and-cash-flow/",
      },
    ],
  },
  {
    ...
    sources: [
      {
        ...
        url: "https://www.sciencedirect.com/science/article/abs/pii/S0305750X25001883",
      },
  ...
  },
  {
    ...
    sources: [
      {
        label: "42 Days Coffee",
        url: "https://www.42dayscoffee.com/blogs/news/understanding-the-differences-between-specialty-coffee-and-commodity-coffee",
      },
      {
        label: "Union Hand-Roasted",
        url: "https://unionroasted.com/pages/speciality-vs-commodity-coffee",
      },
    ],
  },
  {
    ...
    highlight:
      "The C Market doesn't price specialty coffee—it establishes the baseline anxiety that forces specialty roasters to compromise. Recent volatility? Speculation by commodity traders with zero relation to harvest reality.",
    sources: [
      {
        label: "Perfect Daily Grind",
        url: "https://perfectdailygrind.com/2025/07/high-coffee-prices-divide-between-specialty-and-commercial/",
      },
      {
        label: "Coffee Intelligence",
        url: "https://intelligence.coffee/2024/04/specialty-coffee-buyers-and-c-market/",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    highlight:
      "Sustainability is what brands use to charge premiums. Regeneration actually changes systems.",
  },
  {
    num: 8,
    ...
    sources: [
      {
        label: "Coffee Intelligence Pricing Data",
        url: "https://intelligence.coffee/2024/04/specialty-coffee-buyers-and-c-market/",
      },
    ],
  },
  {
    ...
    highlight:
      "That's financial architecture for regeneration—not blockchain on an extractive supply chain.",
    sources: [
      {
        label: "Tony Greenberg",
        url: "https://tonygreenberg.com/what-solutions-are-best-built-with-blockchain/",
      },
    ],
  },
  {
    ...
    highlight:
      "That single change—pre-harvest financing—proves you're serious about regeneration, not marketing it.",
    sources: [
      {
        label: "Fair World Project",
        url: "https://fairworldproject.org/low-prices-and-exploitation-recurring-themes-in-coffee/",
  ...
  return (
    <div className="mb-5 overflow-hidden rounded-lg border-t-4 border-t-[#D4AF37] bg-white shadow-[0_2px_10px_rgba(0,0,0,0.05)]">
      <button onClick={toggle} className="flex w-full items-start gap-4 px-7 py-6 text-left">
        <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-[#8b4c2a] font-mono text-sm font-bold text-white">
          {qa.num}
        </span>
        <span className="flex-1 font-heading text-[clamp(16px,2.5vw,20px)] leading-snug font-semibold text-[#1a0e08]">
          {qa.q}
        </span>
        <svg
          width="16"
          ...
          viewBox="0 0 16 16"
          fill="none"
          className={`mt-1.5 shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`}
        >
          <path
            d="M4 6l4 4 4-4"
            stroke="#6b5a4e"
      ...
      {isOpen && (
        <div className="px-7 pb-7 pl-20">
          ...
          {qa.highlight && (
            <p className="my-3.5 text-base leading-[1.75] font-semibold text-[#8b4c2a]">
              {qa.highlight}
            </p>
          )}
          {qa.data && (
            <div className="my-3.5 rounded-sm border-l-3 border-l-[#D4AF37] bg-[#f9f1e6] px-4 py-3 text-sm font-medium text-[#2d3a24]">
              {qa.data}
            </div>
          )}
          {qa.sources && qa.sources.length > 0 && (
            <div className="mt-4 rounded-sm border-l-3 border-l-[#8b4c2a] bg-[#f5f1e8] px-4 py-3">
              <strong className="mb-1.5 block font-mono text-[11px] tracking-[0.1em] text-[#8b4c2a]">
                Sources
              </strong>
              {qa.sources.map((s) => (
                <div key={s.url} className="relative my-1 pl-4 text-[13px]">
                  <ArrowRight
                    aria-hidden="true"
                    className="absolute top-0.5 left-0 size-3 text-[#8b4c2a]"
                  />
                  <a
                    href={s.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[#2d3a24]"
                  >
                    {s.label}
                  </a>
                </div>
  ...
  return (
    <>
      <div className="mb-4 flex justify-end">
        <button
          onClick={toggleAll}
          className="rounded-sm border border-[#8b4c2a] px-3.5 py-1.5 font-mono text-xs tracking-wide text-[#8b4c2a]"
        >
...
export function ShareRow() {
  ...
  const text =
    "Shanita Nicholas breaks the coffee industry's comfortable lies — Tony G Guest Series";
  ...
  return (
    <div className="mx-auto flex max-w-200 flex-wrap justify-center gap-3 px-6 pt-6">
      ...
      >
        <Share2 aria-hidden="true" className="mr-1 inline size-3.5" />
        Share on X
      </a>
      <a
