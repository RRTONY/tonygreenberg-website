...
export function SubmitForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const subject = encodeURIComponent(
      form.type === "submit"
        ? `BrewSoul coffee submission: ${form.name}`
        : `BrewSoul score appeal: ${form.name}`,
    );
  ...
  if (sent) {
    return (
      <div className="rounded-xl border border-[#6F4E37]/8 bg-white p-12 text-center">
        <div className="mb-2 font-heading text-[1.3rem] text-[#2C1810]">
          Opening your email client...
        </div>
        <p className="text-[0.9rem] text-[#6B5B4F]">
          Send the pre-filled message and we&apos;ll review your{" "}
          {form.type === "submit" ? "submission" : "appeal"}.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {(["submit", "appeal"] as const).map((t) => (
          <button
            ...
            className={`rounded-lg px-5 py-2 font-mono text-[0.78rem] ${
              form.type === t
                ? "border-2 border-[#C5A23C] bg-[#C5A23C]/8 text-[#8B6914]"
                : "border border-[#6F4E37]/15 bg-white text-[#6B5B4F]"
            }`}
          >
      ...
      <textarea
        placeholder={
          form.type === "submit"
            ? "Why should we add this coffee?"
            : "What did we get wrong and why?"
        }
        value={form.notes}
