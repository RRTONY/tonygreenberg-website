...
function RelatedLinkRow({ link }: { link: ArticleFooterRelatedLink }) {
  ...
  return (
    <Link href={href} className="block rounded-md border border-border p-4 transition-colors hover:bg-secondary">
            ...
            {data.furtherReading.map((item) => {
              ...
              return (
                <li key={title} className="relative pl-5 text-foreground/80">
                  ...
                  {url ? (
                    <a href={url} target="_blank" rel="noopener noreferrer" className="text-brand-gold underline underline-offset-2">
                      {title}
                    </a>
                  ) : (
                    title
