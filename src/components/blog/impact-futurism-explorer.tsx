...
function TagBadge({ tag }: { tag: string }) {
  return (
    <span className={`rounded-sm px-2.5 py-1 font-mono text-[0.6rem] tracking-widest uppercase ${TAG_BADGE[tag] ?? "bg-neutral-900 text-neutral-100"}`}>
      {tag}
    </span>
  );
}
    ...
    <div>
      ...
      <div className="relative overflow-hidden bg-[#0A0A10] px-6 py-20 sm:px-10">
        ...
        <div className="relative z-10 mx-auto max-w-3xl">
          <div className="mb-4 font-mono text-xs tracking-[0.25em] text-brand-gold-light uppercase">◆ Tony Greenberg</div>
          <h1 className="mb-5 font-heading text-4xl leading-tight text-white/95 italic sm:text-6xl">Impact Futurism</h1>
          <p className="mb-8 max-w-xl leading-relaxed text-white/65">
            The systems that run your health, your money, and your future are broken. These essays are about what
            comes next — written from the inside of $10B+ in transactions, six psychedelic investments, and
            twenty-five years of watching institutions fail the people they were built to serve.
          </p>
          <div className="flex flex-wrap gap-3">
            <Link href="/blog" className="rounded-sm bg-brand-gold-light px-7 py-3 font-mono text-xs font-semibold tracking-wide text-[#0A0A10] uppercase">
              Get the Dispatch
            </Link>
            <Link
              href="/articles"
      ...
      <div className="bg-background px-6 py-14 sm:px-10">
        <div className="mx-auto max-w-5xl">
          <div className="mb-6 font-mono text-xs tracking-[0.2em] text-brand-gold uppercase">Featured</div>
      ...
      <div className="bg-linear-to-br from-[#0A0A10] to-[#1a1020] px-6 py-16 text-center sm:px-10">
        <div className="mx-auto max-w-md">
          <div className="mb-3 font-mono text-xs tracking-[0.2em] text-brand-gold-light uppercase">◆ The Dispatch</div>
          ...
          <p className="mb-6 leading-relaxed text-white/55">
            No cadence. No algorithm. Just the things I couldn&apos;t not write — about the systems breaking around
