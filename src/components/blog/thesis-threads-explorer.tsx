...
const THREAD_ACCENT: Record<string, { border: string; text: string; wash: string }> = {
  ...
  "trust-economics": { border: "border-emerald-500/30", text: "text-emerald-400", wash: "bg-emerald-500/10" },
  "regenerative-proof": { border: "border-amber-500/30", text: "text-amber-400", wash: "bg-amber-500/10" },
  "founder-track-record": { border: "border-violet-500/30", text: "text-violet-400", wash: "bg-violet-500/10" },
  "market-intelligence": { border: "border-blue-500/30", text: "text-blue-400", wash: "bg-blue-500/10" },
  "culture-decay": { border: "border-rose-500/30", text: "text-rose-400", wash: "bg-rose-500/10" },
};
const DEFAULT_ACCENT = { border: "border-brand-gold/30", text: "text-brand-gold-light", wash: "bg-brand-gold/10" };
    ...
    <button onClick={onClick} className="block w-full text-left">
      ...
      >
        <div className="mb-2 flex items-center justify-between">
          ...
          {isActive && <span className={`size-2 rounded-full ${accent.text.replace("text-", "bg-")}`} />}
        </div>
  ...
  const threadArticles = useMemo(
    () =>
      THESIS_THREADS.map((thread) => ({
        thread,
        posts: thread.articleSlugs.map((slug) => postBySlug.get(slug)).filter((p): p is ArchivePost => Boolean(p)),
      })),
    [postBySlug],
  );
    ...
    <div className="bg-[#0A0A10] text-white/90">
      ...
      <div className="mx-auto max-w-5xl px-6 py-12 sm:px-10">
          ...
          <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
            ...
            <div>
              <GlassCard className="mb-6 p-6">
                ...
                <p className="mb-3 text-sm text-brand-gold-light italic">{current.thread.tagline}</p>
                <p className="text-sm leading-relaxed text-white/65">{current.thread.description}</p>
              </GlassCard>
        ...
        {tab === "sections" && (
          <div>
            <p className="mb-8 max-w-2xl text-sm leading-relaxed text-white/55">
              Each section of the site maps to a specific facet of the thesis. The anchor content, data points, and
              tone are calibrated to serve that facet while maintaining the site-wide voice.
            </p>
            <div className="grid gap-6 md:grid-cols-2">
              {SECTION_THESIS_MAP.map((section) => (
                <GlassCard key={section.slug} className="p-6">
                  ...
                  <p className="mb-4 text-sm leading-relaxed text-white/65">{section.impactSoulConnection}</p>
                  <div className="flex flex-wrap gap-2">
                    {section.dataPoints.map((dp) => (
                      <span key={dp} className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-xs text-white/50">
                        {dp}
                      </span>
                    ))}
                  </div>
        ...
        {tab === "research" && (
          <div>
            <p className="mb-8 max-w-2xl text-sm leading-relaxed text-white/55">
              The research library. Cite as needed. Every article should include at least one external data point or
              research citation.
            </p>
              ...
              {RESEARCH_LIBRARY.map((cite, i) => (
                <GlassCard key={`${cite.author}-${cite.year}`} className="p-5">
                  <div className="flex items-start gap-4">
                    <span className="font-heading text-2xl leading-none text-brand-gold-light/40">{i + 1}</span>
                    <div>
        ...
        {tab === "rules" && (
          <div className="max-w-2xl">
            <GlassCard className="mb-8 p-8">
              ...
              <p className="mb-3 text-sm leading-relaxed text-white/65">{CORE_THESIS.authorVoice}</p>
