        ...
        <li key={post.slug} className="border-b border-border py-3.5">
          <div className="flex flex-wrap items-start gap-4">
            ...
            <div className="min-w-50 flex-1">
              <Link
                href={`/blog/${post.slug}`}
                className="block font-heading text-base font-semibold text-foreground hover:text-brand-gold"
              >
                {post.title}
              </Link>
              {post.subtitle && (
                <span className="mt-0.5 block text-sm text-muted-foreground">{post.subtitle}</span>
              )}
            </div>
            {post.category && (
...
export function ArticlesExplorer({ posts }: { posts: ArchivePost[] }) {
  ...
  const categories = useMemo(() => {
    const cats = new Set(
      posts.map((p) => p.category?.title).filter((c): c is string => Boolean(c)),
    );
  ...
  return (
    <div>
      ...
      <div
        className={`overflow-x-auto border-b border-border bg-secondary px-6 sm:px-10 ${activeMode !== "All" ? "hidden" : ""}`}
      >
            ...
            {CATEGORY_ORDER.filter((cat) => grouped?.[cat]).map((cat) => (
              <div key={cat} className="mb-12">
                ...
                {CATEGORY_DESCRIPTIONS[cat] && (
                  <p className="mt-0 mb-5 leading-relaxed text-muted-foreground">
                    {CATEGORY_DESCRIPTIONS[cat]}
                  </p>
                )}
