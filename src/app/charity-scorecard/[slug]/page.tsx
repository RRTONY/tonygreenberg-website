...
import { ArrowLeft, ExternalLink, Award, BarChart3, Users, MapPin, Calendar, DollarSign, Globe, TrendingUp } from "lucide-react";
...
export async function generateMetadata({ params }: PageProps<"/charity-scorecard/[slug]">): Promise<Metadata> {
...
export default async function CharityProfilePage({ params }: PageProps<"/charity-scorecard/[slug]">) {
  const { slug } = await params;
  const charity = CHARITIES.find((c) => c.slug === slug);
  if (!charity) notFound();

  const rank = CHARITIES.indexOf(charity) + 1;
  const similar = CHARITIES.filter((c) => c.sector === charity.sector && c.slug !== charity.slug).slice(0, 5);
        ...
        <div className="mx-auto max-w-5xl px-4">
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-start">
            <div className="flex-1">
              ...
              <h1 className="mb-3 font-heading text-3xl font-bold text-[#E8E4DC] md:text-5xl">{charity.name}</h1>
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="font-mono text-3xl font-bold text-brand-gold">{charity.totalScore}</span>
              <span className="text-xs tracking-wide text-[#E8E4DC]/50 uppercase">Unified Score</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-5xl px-4 py-10">
              ...
              <div className="space-y-3 rounded-lg border border-border bg-card p-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`rounded px-2 py-0.5 text-xs font-bold ${
                        charity.givewell === "top" ? "bg-emerald-100 text-emerald-800" : "bg-blue-100 text-blue-800"
                      }`}
                    >
                ...
                {charity.charityWatch && (
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`rounded px-2 py-0.5 text-xs font-bold ${
                        charity.charityWatch.startsWith("A") ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"
                      }`}
                    >
                      {charity.charityWatch}
                    </span>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`rounded px-2 py-0.5 text-xs font-bold capitalize ${
                        charity.candidSeal === "platinum" ? "bg-purple-100 text-purple-800" : "bg-amber-100 text-amber-800"
                      }`}
                    >
                      {charity.candidSeal}
                    </span>
        ...
        {similar.length > 0 && (
          <div className="mt-12 border-t border-border pt-8">
            <h2 className="mb-4 font-heading text-xl font-bold">Other {charity.sector} Charities</h2>
              ...
              {similar.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 font-mono text-sm font-bold text-brand-gold">{c.totalScore}</div>
                  </Link>
                );
              })}
            </div>
