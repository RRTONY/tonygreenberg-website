...
export const metadata: Metadata = {
  ...
  description: "Mycotoxin contamination is real but overhyped by wellness marketers — here's what the science says, and which coffees are independently tested.",
...
export default function BrewSoulMoldFreePage() {
  ...
  const untested = BREWSOUL_COFFEES.filter((c) => !c.moldTestStatus || c.moldTestStatus === "untested");
  ...
  return (
    <section className="mx-auto max-w-225 px-6 py-12">
      ...
      <div className="mb-10 flex flex-col gap-3">
        {verified.length === 0 && <p className="text-[0.88rem] text-[#999]">No verified coffees yet.</p>}
        {verified.map((c) => (
          <Link
            key={c.id}
          ...
          >
            ...
            <div className="font-mono text-[0.68rem] text-[#4A7C59]">{c.moldTestSource || "Third-party tested"}</div>
          </Link>
        ))}
      </div>
