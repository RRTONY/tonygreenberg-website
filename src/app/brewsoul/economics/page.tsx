...
export const metadata: Metadata = {
  ...
  description: "The specialty coffee economy at a glance — pricing trends, farmer equity, and the real cost of your morning cup.",
...
export default function BrewSoulEconomicsPage() {
  ...
  const topOrigins = origins
    .map((o) => ({ origin: o, count: BREWSOUL_COFFEES.filter((c) => c.originCountry === o).length }))
  ...
  return (
    <section className="mx-auto max-w-225 px-6 py-12">
      ...
      <div className="mb-10 flex flex-col gap-2">
        {PRICE_BUCKETS.map((b) => {
          const count = BREWSOUL_COFFEES.filter((c) => c.priceUsd && c.priceUsd >= b.min && c.priceUsd < b.max).length;
          const pct = BREWSOUL_COFFEES.length ? (count / BREWSOUL_COFFEES.length) * 100 : 0;
          return (
            <div key={b.range} className="flex items-center gap-3">
              <span className="w-15 shrink-0 text-right font-mono text-[0.72rem] text-[#6B5B4F]">{b.range}</span>
              ...
              <span className="w-6 shrink-0 font-mono text-[0.72rem] font-bold text-[#2C1810]">{count}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="flex flex-col gap-2">
        {topOrigins.map((o) => (
          <div key={o.origin} className="flex items-center gap-3">
            <span className="w-22.5 shrink-0 text-right font-mono text-[0.72rem] text-[#6B5B4F]">{o.origin}</span>
            <div className="h-5.5 flex-1 overflow-hidden rounded-sm bg-[#6F4E37]/4">
              <div
            ...
            <span className="w-6 shrink-0 font-mono text-[0.72rem] font-bold text-[#2C1810]">{o.count}</span>
          </div>
        ))}
      </div>
