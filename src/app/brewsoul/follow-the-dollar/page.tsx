...
export const metadata: Metadata = {
  ...
  description:
    "Where your coffee dollar actually goes — real supply-chain economics at every price point.",
  ...
  return (
    <section className="mx-auto max-w-225 px-6 py-12">
      ...
      <p className="mb-10 max-w-2xl text-[0.95rem] leading-relaxed text-[#6B5B4F]">
        When you pay for a bag of specialty coffee, where does that money actually go? We break down
        the supply chain economics at every price point.
      </p>
            ...
            <div className="flex flex-col gap-2">
              {segs.map((seg) => (
                <div key={seg.label} className="flex items-center gap-3">
                  <span className="w-18 shrink-0 text-right font-mono text-[0.68rem] text-[#6B5B4F]">
                    {seg.label}
                  </span>
                  ...
                  <span className="w-10 shrink-0 font-mono text-[0.72rem] font-bold text-[#2C1810]">
                    {seg.pct}%
                  </span>
                </div>
              ))}
            </div>
            <p className="mt-3 text-[0.85rem] leading-relaxed text-[#6B5B4F] italic">
              {db.commentary}
            </p>
          </div>
        );
      ...
      <NextSteps
        steps={[
          ...
          {
            label: "Economics Deep Dive",
            path: "/brewsoul/economics",
            description: "The full picture",
          },
          {
            label: "Browse Ethical Coffees",
            path: "/brewsoul/browse",
            description: "Vote with your cup",
          },
        ]}
      />
    </section>
