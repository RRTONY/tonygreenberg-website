...
export const metadata: Metadata = {
  ...
  description:
    "How a coffee cherry is processed after picking determines up to 60% of its final flavor — every method explained.",
...
export default function BrewSoulProcessingPage() {
  return (
    <section className="mx-auto max-w-225 px-6 py-12">
      <div className="mb-2 font-mono text-xs tracking-[0.25em] text-[#6F4E37] uppercase">
        From Cherry to Cup
      </div>
      ...
      <div className="flex flex-col gap-6">
        {PROCESSING_METHODS.map((pm) => (
          <div
            key={pm.id}
            className="rounded-xl border border-[#6F4E37]/8 border-l-4 border-l-[#6F4E37] bg-white p-6"
          >
            ...
            {pm.controversy && (
              <p className="mt-2 text-[0.82rem] text-[#8B6914] italic">{pm.controversy}</p>
            )}
          </div>
        ))}
      </div>
      ...
      <NextSteps
        steps={[
          {
            label: "Varieties Guide",
            path: "/brewsoul/varieties",
            description: "Bean types explained",
          },
