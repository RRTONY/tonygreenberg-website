...
import {
  ...
  FINAL_OBSERVATION,
  type EsotericCoffee,
} from "@/lib/content/brewsoul-esoteric";
...
export const metadata: Metadata = {
  ...
  description:
    "Rare, unusual, and experimental coffees for the serious enthusiast — where to actually buy the legendary coffees.",
  ...
  return (
    <div className="flex items-center gap-[3px]">
      {Array.from({ length: 10 }).map((_, i) => (
        <div
          key={i}
          className={`size-2 rounded-full ${i < score ? "bg-[#8B6914]" : "bg-[#8B6914]/15"}`}
        />
      ))}
  ...
  return (
    <div className="mt-2 flex flex-wrap gap-1.5">
      {notes.map((note) => (
        <span
          key={note}
          className="rounded-full border border-[#8B6914]/20 bg-[#8B6914]/8 px-2.5 py-1 font-mono text-[10px] tracking-wide text-[#8B6914] uppercase"
        >
          {note}
        </span>
      ))}
  ...
  return (
    <div className="mb-5 rounded-xl border border-[#8B6914]/12 bg-white p-7 transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(139,105,20,0.12)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="font-heading text-[clamp(22px,3vw,30px)] leading-[1.2] font-bold text-[#0A0A10]">
            {coffee.name}
          </h3>
          {coffee.origin && (
            <span className="font-mono text-[11px] tracking-[0.1em] text-[#6b5a4e] uppercase">
              {coffee.origin}
            </span>
          )}
        </div>
        <div className="font-mono text-lg font-bold whitespace-nowrap text-[#8B6914]">
          {coffee.typicalCost}
        </div>
      </div>

      <p className="my-3.5 text-base leading-relaxed text-[#2d1810] italic">
        {coffee.whyItMatters}
      </p>
      ...
      <div className="mt-4">
        <span className="font-mono text-[10px] tracking-[0.1em] text-[#6b5a4e] uppercase">
          Rarity
        </span>
      ...
      {coffee.buySources.length > 0 && (
        <div className="mt-4.5">
          <span className="mb-2 block font-mono text-[10px] tracking-[0.1em] text-[#6b5a4e] uppercase">
            Where to Buy
          </span>
          <div className="flex flex-wrap gap-2">
            {coffee.buySources.map((src) => (
              ...
              >
                {src.name} {src.note ? `(${src.note})` : ""} <ForwardIcon aria-hidden="true" />
              </a>
            ))}
          </div>
        </div>
      ...
      {coffee.additionalSources && coffee.additionalSources.length > 0 && (
        <div className="mt-3">
          <span className="mb-1.5 block font-mono text-[10px] tracking-[0.1em] text-[#6b5a4e] uppercase">
            Additional Sources
          </span>
          <div className="flex flex-wrap gap-1.5">
            {coffee.additionalSources.map((src) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="font-mono text-[10px] text-[#8B6914] underline underline-offset-3"
              >
                {src.name}
              </a>
            ))}
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7]">
      <section className="mx-auto max-w-225 px-5 pt-[clamp(60px,10vw,120px)] pb-15 text-center">
        <div className="mb-3 font-mono text-[11px] tracking-[0.3em] text-[#6b5a4e] uppercase opacity-80">
          BrewSoul Intelligence
        </div>
        <h1 className="mb-4 font-heading text-[clamp(32px,5vw,56px)] leading-[1.1] font-bold text-[#0A0A10]">
          Esoteric Coffee Index
        </h1>
        <p className="mb-6 font-heading text-[clamp(18px,2.5vw,24px)] text-[#8B6914] italic">
          Where To Actually Buy The Legendary Coffees
        </p>
        <p className="mx-auto max-w-150 text-base leading-relaxed text-[#6b5a4e]">
          Most coffee drinkers never encounter these. Not because they&apos;re hidden — but because
          nobody tells you where to find them. Until now.
        </p>
        <div className="mt-6 font-mono text-[10px] tracking-[0.1em] text-[#6b5a4e] opacity-60 uppercase">
      ...
      <section className="mx-auto max-w-215 px-5 pb-15">
        <div className="mb-8 border-b-2 border-[#8B6914] pb-3">
          ...
          <h2 className="font-heading text-[clamp(26px,4vw,40px)] leading-[1.15] font-bold text-[#0A0A10]">
      ...
      <section className="mx-auto max-w-215 px-5 pb-15">
        <div className="rounded-2xl bg-[#0A0A10] px-9 py-10 text-[#f5efe0]">
          <div className="mb-3 font-mono text-[11px] tracking-[0.3em] text-[#e8dcc8] uppercase opacity-80">
            BrewSoul
          </div>
          <h2 className="mb-6 font-heading text-[clamp(24px,3.5vw,36px)] leading-[1.15] font-bold text-[#D4B96A]">
          ...
          <ol className="m-0 list-none pl-0 text-[17px] leading-[2.2] text-[#e8dcc8]">
            {ACQUISITION_PRIORITY.map((name, i) => (
              <li key={name} className={i < 9 ? "border-b border-[#D4B96A]/10" : ""}>
                <span className="mr-3 font-mono text-xs text-[#D4B96A]">
      ...
      <section className="mx-auto max-w-215 px-5 pb-15">
        <div className="mb-3 font-mono text-[11px] tracking-[0.3em] text-[#6b5a4e] uppercase opacity-80">
          Coffee Explorer Score
        </div>
        <h2 className="mb-6 font-heading text-[clamp(24px,3.5vw,36px)] leading-[1.15] font-bold text-[#0A0A10]">
          The Scorecard
        </h2>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b-2 border-[#8B6914]">
                <th className="p-4 text-left font-mono text-[10px] tracking-[0.1em] text-[#6b5a4e] uppercase">
                  Coffee
                </th>
                <th className="p-3 text-center font-mono text-[10px] tracking-wide text-[#6b5a4e] uppercase">
                  Taste Shock
                </th>
                <th className="p-3 text-center font-mono text-[10px] tracking-wide text-[#6b5a4e] uppercase">
                  Rarity
            ...
            <tbody>
              {EXPLORER_SCORES.map((row, i) => (
                <tr
                  key={row.coffee}
                  className={`border-b border-black/6 ${i % 2 === 1 ? "bg-[#8B6914]/2" : ""}`}
                >
                  <td className="p-3 pl-4 font-semibold text-[#0A0A10]">{row.coffee}</td>
                  <td
                    className={`p-3 text-center ${row.tasteShock >= 9 ? "font-bold text-[#8B6914]" : "text-[#2d1810]"}`}
                  >
                    {row.tasteShock}
                  </td>
                  <td
                    className={`p-3 text-center ${row.rarity >= 9 ? "font-bold text-[#8B6914]" : "text-[#2d1810]"}`}
                  >
                    {row.rarity}
                  </td>
                  <td
                    className={`p-3 text-center ${row.story >= 9 ? "font-bold text-[#8B6914]" : "text-[#2d1810]"}`}
                  >
                    {row.story}
                  </td>
                  <td
                    className={`p-3 text-center ${row.worthBuying >= 9 ? "font-bold text-[#8B6914]" : "text-[#2d1810]"}`}
                  >
                    {row.worthBuying}
                  </td>
                </tr>
      ...
      <section className="mx-auto max-w-215 px-5 pb-20">
        <div className="my-10 border-l-4 border-[#8B6914] pl-7">
          {FINAL_OBSERVATION.split("\n").map((line, i) => (
            <p
              key={line}
              className={`mb-2 font-heading leading-relaxed text-[#0A0A10] italic ${i === 0 ? "text-xl" : "text-[17px]"}`}
            >
              {line}
            </p>
          ))}
      ...
      <NextSteps
        steps={[
          ...
          },
          {
            ...
            description: "Deep dive into genetics",
          },
          {
            label: "BrewSoul Home",
            path: "/brewsoul/home",
            description: "Return to intelligence hub",
          },
        ]}
      />
    </div>
