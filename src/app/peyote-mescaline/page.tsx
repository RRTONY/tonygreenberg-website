  ...
  return (
    <div className="bg-pri-cream font-body text-pri-ink leading-[1.7]">
        ...
        <div className="relative z-10">
          ...
          <div className="mb-6 flex flex-wrap items-center gap-3">
            {MESCALINE_META.tags.map((t) => (
              <span
                key={t}
                className={`border px-2 py-0.5 text-xs font-bold tracking-[0.08em] uppercase ${TAG_CLASS[t] ?? DEFAULT_TAG_CLASS}`}
              >
                {t}
              </span>
            ))}
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link
              ...
              className="border-[1.5px] border-pri-ink px-8 py-3.5 text-sm font-bold tracking-[0.05em] text-pri-ink uppercase"
            >
      ...
      <PriSection id="pharmacology">
        ...
        <h2 className="mb-6 font-heading text-[clamp(1.5rem,4vw,2.5rem)] text-pri-ink">
          Key Receptors
        </h2>
        ...
        <p className="mt-4 text-[.82rem] leading-[1.7] text-pri-tan">
          Unlike MDMA and SSRIs, mescaline has{" "}
          <strong className="text-pri-ink">no serotonin transporter (SERT) affinity</strong> — it
          does not dump serotonin. Its psychedelic action is purely receptor-mediated via 5-HT2A
          partial agonism, with unique prosocial effects from low-affinity dopamine binding and
          sympathomimetic arousal from adrenergic agonism.
        </p>
      </PriSection>
      ...
      <PriSection id="latuda-mirror" dark>
        ...
        <h2 className="mb-3 font-heading text-[clamp(1.5rem,4vw,2.5rem)] text-pri-cream">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-115 border-collapse">
            <thead>
              <tr>
                <th className="border-b border-pri-cream/10 bg-pri-cream/8 px-3 py-2.5 text-left text-xs font-extrabold tracking-[0.08em] whitespace-nowrap text-pri-cream/50 uppercase">
                  Receptor
                </th>
                <th className="border-b border-pri-cream/10 bg-pri-cream/8 px-3 py-2.5 text-left text-xs font-extrabold tracking-[0.08em] whitespace-nowrap text-pri-cream/50 uppercase">
                  Latuda
            ...
            <tbody>
              {LATUDA_MIRROR.map((r, i) => (
                <tr key={r.receptor} className={i % 2 ? "bg-pri-cream/3" : ""}>
                  <td className="border-b border-pri-cream/6 px-3 py-2.5 text-[.85rem] font-bold text-pri-cream">
                    {r.receptor}
                  </td>
        ...
        <div className="mt-8 border-l-4 border-pri-purple bg-pri-purple/12 px-8 py-6">
          <p className="m-0 font-heading text-[clamp(1rem,3vw,1.2rem)] leading-[1.5] font-bold text-pri-cream">
      ...
      <PriSection id="outcomes">
        ...
        <h2 className="mb-1 font-heading text-[clamp(1.5rem,4vw,2.5rem)] text-pri-ink">
          Clinical Outcomes
        </h2>
        ...
        <div className="mb-6 grid grid-cols-[repeat(auto-fit,minmax(180px,1fr))] gap-px border border-pri-border bg-pri-border">
          {MESCALINE_OUTCOMES.map((o) => (
            <div key={o.condition} className="bg-pri-parchment px-4 py-6 text-center">
              <div className="font-heading text-[clamp(2rem,6vw,3rem)] leading-none font-black text-pri-purple">
                {o.improved}
              </div>
              <div className="mt-1 text-xs font-bold tracking-[0.08em] text-pri-tan uppercase">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="border-l-3 border-[#C9A84C] bg-pri-parchment p-5">
            <div className="mb-1 text-xs font-extrabold tracking-[0.08em] text-[#C9A84C] uppercase">
          ...
          <div className="border-l-3 border-[#6B8F71] bg-pri-parchment p-5">
            <div className="mb-1 text-xs font-extrabold tracking-[0.08em] text-[#6B8F71] uppercase">
              Intent Paradox
            </div>
      ...
      <PriSection id="pharma-alternatives" dark>
        ...
        <h2 className="mb-4 font-heading text-[clamp(1.5rem,4vw,2.5rem)] text-pri-cream">
        ...
        <div className="mb-8 border-l-4 border-[#581C87] bg-[#581C87]/15 p-5">
          ...
          <p className="m-0 text-[.82rem] leading-[1.6] text-pri-cream/55">
            Do not stop or modify any prescribed medication based on this content. Dangerous
            interactions exist between plant medicines and psychiatric drugs. Always consult
      ...
      <PriSection id="supplements">
        ...
        <h2 className="mb-8 font-heading text-[clamp(1.5rem,4vw,2.5rem)] text-pri-ink">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-6">
          {SUPPLEMENT_STACKS.map((phase) => (
            <div key={phase.phase} className="border border-pri-border bg-pri-parchment">
              <div
                className={`px-4 py-3 ${phase.phase === "PRE" ? "bg-[#6B8F71]" : phase.phase === "DAY-OF" ? "bg-[#C9A84C]" : "bg-pri-purple"}`}
              >
                <div className="text-xs font-extrabold tracking-[0.1em] text-white uppercase">
                  {phase.phase}
                </div>
              ...
              <div className="p-4">
                {phase.items.map((item, i) => (
                  <div
                    key={item.name}
                    className={`flex items-start justify-between gap-2 py-2 ${i < phase.items.length - 1 ? "border-b border-[#E8E2D8]" : ""}`}
                  >
                    <div>
                    ...
                    {item.searchUrl && (
                      <a
                        ...
                        className="shrink-0 border border-pri-purple/20 px-1.5 py-0.5 text-[.75rem] font-bold whitespace-nowrap text-pri-purple"
                      >
      ...
      <PriSection id="dimensions" dark>
        ...
        <h2 className="mb-8 font-heading text-[clamp(1.5rem,4vw,2.5rem)] text-pri-cream">
            ...
            <tbody>
              {MEDICINE_SELECTOR.map((m, i) => {
                ...
                return (
                  <tr
                    key={m.medicine}
                    className={isMesc ? "bg-pri-purple/6" : i % 2 ? "bg-pri-parchment" : ""}
                  >
                    <td
                    ...
                    <td className={tdClass}>
                      <span
                        className={`border px-1.5 py-0.5 text-xs font-bold ${
                          m.evidence.startsWith("Strong")
                            ? "border-[#6B8F71]/30 bg-[#6B8F71]/15 text-[#3D6B44]"
                            : "border-[#C9A84C]/30 bg-[#C9A84C]/15 text-[#8B6914]"
                        }`}
                      >
                        {m.evidence}
                      </span>
      ...
      <PriSection id="sources" dark>
        ...
        <h2 className="mb-6 font-heading text-[clamp(1.3rem,3vw,1.8rem)] text-pri-cream">
          Research &amp; References
        </h2>
      ...
      <footer className="bg-pri-ink px-5 py-12 text-center text-pri-cream/40">
        <div className="mx-auto max-w-150">
          ...
          <div className="my-4 flex flex-wrap justify-center gap-6">
            <Link
              href="/psychedelic-readiness-index"
              className="text-[.72rem] font-semibold tracking-[0.06em] text-pri-cream/35 uppercase"
            >
              <BackIcon aria-hidden="true" /> Back to PRI
            </Link>
            <Link
