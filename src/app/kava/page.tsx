...
const TRIAD = [
  { name: "Mezcal", role: "Earth Anchor", note: "Agave terpenes, 2,500 years Mesoamerican ceremony", colorClass: "border-kava-terracotta/20 text-kava-terracotta", bgClass: "bg-kava-terracotta", step: "1" },
  { name: "Kava", role: "Threshold Essence", note: "GABA + MAO-B prime, anxiolysis without sedation", colorClass: "border-kava-saffron/20 text-kava-saffron", bgClass: "bg-kava-saffron", step: "2" },
  { name: "Peyote", role: "The Journey", note: "5-HT2A activation, 5,500 years indigenous use", colorClass: "border-kava-cobalt/20 text-kava-cobalt", bgClass: "bg-kava-cobalt", step: "3" },
];
    ...
    <>
      ...
      <KavaSection bg="sand">
        <div className="mx-auto max-w-3xl text-center">
          <p className="mb-4 text-xs font-bold tracking-[0.25em] text-kava-saffron uppercase">The Threshold Doctrine</p>
          <blockquote className="font-heading text-xl leading-[1.6] font-medium text-kava-ink md:text-2xl">
            Kava is not a psychedelic. That is precisely its power. It dissolves anxiety, softens ego armor, elevates prosocial bonding, and prepares the
            nervous system to receive what comes next. It is the ultimate preparation medicine for collective consciousness work.
          </blockquote>
        </div>
      </KavaSection>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {TRIAD.map((t) => (
            <div key={t.name} className={`rounded-xl border-2 p-6 text-center ${t.colorClass}`}>
              <div className={`mx-auto mb-3 flex size-10 items-center justify-center rounded-full text-sm font-bold text-white ${t.bgClass}`}>{t.step}</div>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {STATS.map((s) => (
            <div key={s.label} className="py-4 text-center">
              <p className="mb-1 font-heading text-3xl font-bold text-kava-saffron md:text-4xl">{s.value}</p>
