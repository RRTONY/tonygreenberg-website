...
import { KavaCard, KavaDisclaimer, KavaDivider, KavaHero, KavaSection, KavaSectionTitle } from "@/components/kava/kava-ui";
...
export const metadata: Metadata = {
  ...
  description: "How kava's CYP1A2 inhibition affects caffeine metabolism, a caffeine cutoff protocol for ceremony, and kava vs. coffee compared head to head.",
      ...
      <KavaSection>
        ...
        <div className="mb-6 rounded-xl border-l-4 border-kava-saffron bg-kava-saffron/10 p-6">
          <p className="text-base leading-[1.75] text-kava-ink">
            Kava inhibits the liver enzyme <strong>CYP1A2 by 56%</strong>. CYP1A2 is the primary enzyme responsible for metabolizing caffeine. When kava is
            active in the system, caffeine is metabolized 56% slower — making it last longer and accumulate to higher blood levels than expected. This is
            the same mechanism by which grapefruit juice affects many medications. Kava&apos;s kavalactone components, rather than other components of the
            extract, are responsible for CYP enzyme inhibition.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <KavaCard>
            <h4 className="mb-2 font-heading text-base font-bold text-kava-saffron">The &ldquo;Hippie Speedball&rdquo;</h4>
            <p className="text-sm leading-[1.75] text-kava-ink/80">
              Some people enjoy the combination — the kava bar community calls it a &ldquo;hippie speedball&rdquo; — experiencing alert relaxation. However,
              people sensitive to caffeine will experience amplified jitters, anxiety, and restlessness that is counterproductive to ceremonial readiness.
            </p>
          </KavaCard>
          <KavaCard>
            <h4 className="mb-2 font-heading text-base font-bold text-[#16a34a]">Yerba Mate Alternative</h4>
            <p className="text-sm leading-[1.75] text-kava-ink/80">
              ...
              clinically studied but worth tracking.
            </p>
          </KavaCard>
        </div>
      </KavaSection>
          ...
          {PROTOCOL.map((p, i) => (
            <KavaCard key={p.time}>
              <div className="flex items-start gap-3">
                <div className="flex size-8 shrink-0 items-center justify-center rounded-full bg-kava-saffron text-sm font-bold text-white">{i + 1}</div>
                <div>
          ...
          <table className="w-full text-sm text-kava-ink">
            <thead>
              <tr className="border-b-2 border-kava-sand-muted">
                ...
                <th className="px-4 py-3 text-left font-heading font-bold text-[#78350F]">Coffee</th>
                <th className="px-4 py-3 text-left font-heading font-bold text-kava-saffron">Kava</th>
              </tr>
            </thead>
            <tbody>
              {COMPARISON.map((row, i) => (
                <tr key={row.aspect} className={`border-b border-kava-sand-muted ${i % 2 === 0 ? "" : "bg-kava-sand-muted/25"}`}>
