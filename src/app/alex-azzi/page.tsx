...
function Section({ id, num, title, children }: { id: string; num: string; title: string; children: React.ReactNode }) {
  return (
    <div id={id} className="mt-14 scroll-mt-20">
      <p className="mb-1.5 font-mono text-xs tracking-[0.2em] text-red-800 uppercase">Section {num}</p>
...
function OpinionBox({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-6 rounded-md border border-amber-700/25 border-l-4 border-l-amber-700 bg-amber-700/5 p-6">
      <div className="mb-2 font-mono text-xs font-bold tracking-[0.2em] text-amber-800 uppercase">Author&apos;s Opinion</div>
      <div className="leading-[1.8] text-amber-950 dark:text-amber-200">{children}</div>
    </div>
  );
}

function HealBox({ children, title = "The Path Back" }: { children: React.ReactNode; title?: string }) {
  return (
    <div className="my-6 rounded-md border border-green-700/25 border-l-4 border-l-green-700 bg-green-700/5 p-6">
      <div className="mb-2 font-mono text-xs font-bold tracking-[0.2em] text-green-800 uppercase">{title}</div>
...
function QuoteBlock({ quote, attribution }: { quote: string; attribution: string }) {
  return (
    <div className="my-6 rounded-md border border-amber-700/25 border-l-4 border-l-amber-700 bg-amber-700/5 p-7">
      <div className="mb-3 text-lg leading-relaxed text-amber-950 italic dark:text-amber-200">&quot;{quote}&quot;</div>
...
function Badge({ children, variant = "red" }: { children: React.ReactNode; variant?: "red" | "amber" | "green" }) {
  const cls =
    variant === "red"
      ? "bg-red-800"
      : variant === "amber"
        ...
        : "bg-green-700";
  return (
    <span className={`rounded-sm px-2.5 py-1 font-mono text-[0.65rem] font-bold tracking-wide text-white uppercase ${cls}`}>
      {children}
    </span>
  );
}
    ...
    <div>
      <div className="mx-auto max-w-3xl px-6 py-10 sm:px-10">
        ...
        <p className="mx-auto mb-8 max-w-xl text-center text-foreground/70">
          Alex Azzi is the CEO of XRWorkout, a VR fitness startup, and the founder of Biohackers UAE.
          This is a documented case of unpaid invoices and unfulfilled equity agreements spanning{" "}
          {SUBJECT.yearsOfPattern} years — signed contracts, timestamped invoices, and a written
          admission of a shortfall. Built on the SunlightProtocol: verify, don&apos;t gossip;
          document, don&apos;t destroy; offer a path back.
        </p>
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
          <div className="flex flex-wrap">
            ...
            <div className="flex flex-1 flex-col justify-center p-8">
              <h2 className="mb-1 font-heading text-3xl font-black text-foreground">{SUBJECT.name}</h2>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { label: "Years of Pattern", value: String(SUBJECT.yearsOfPattern), highlight: false },
                  { label: "Case File", value: SUBJECT.caseNumber, highlight: false },
                ].map((f) => (
                  <div key={f.label}>
                    <div className="font-mono text-[0.6rem] tracking-wide text-muted-foreground uppercase">{f.label}</div>
      ...
      <div className="mx-auto max-w-2xl px-6 pb-16 sm:px-10">
        <div className="mb-10 rounded-md border border-amber-700/25 bg-amber-700/5 p-7">
          ...
          <div className="text-lg leading-relaxed font-medium text-amber-950 dark:text-amber-200">
            ...
            path back. Standing idle is complicit. Recklessness is destructive. Verification is
            the middle way.
          </div>
          <p className="mt-4 text-sm leading-[1.8] text-amber-900/80 dark:text-amber-200/70">
            Inspired by Byron Katie&apos;s{" "}
            <a href="https://thework.com" target="_blank" rel="noopener noreferrer" className="underline">
              The Work
            </a>{" "}
            and the ethical technology framework of Tristan Harris &amp; Aza Raskin at the{" "}
            <a href="https://www.humanetech.com" target="_blank" rel="noopener noreferrer" className="underline">
        ...
        <Section id="case" num="01" title="The Case Against Alex Azzi">
          <P>
            ...
            plus 3% + up to 2% equity in XRWorkout. Payment due within 10 days. Late payments
          ...
          <div className="my-6 grid gap-4 sm:grid-cols-3">
            {SOL_CARDS.map((c) => (
              <div key={c.invoice} className={`rounded-md border-2 ${SOL_BORDER[c.colorClass]} bg-card p-5`}>
                <div className={`mb-2 font-mono text-xs font-bold tracking-wide uppercase ${SOL_TEXT[c.colorClass]}`}>
                  {c.status}
                </div>
                ...
                <div className="h-3 overflow-hidden rounded-full bg-foreground/10">
                  <div className={`h-full rounded-full ${SOL_BAR[c.colorClass]} ${SOL_WIDTH[c.pct]}`} />
                </div>
              ...
              <tbody>
                {EVIDENCE_ROWS.map((r) => (
                  <tr key={r.claim} className="border-t border-border align-top">
                    ...
                    <td className="px-4 py-2.5 font-mono text-xs text-muted-foreground">{r.source}</td>
                    <td className="px-4 py-2.5">
                      <span className={`rounded-sm px-2 py-0.5 font-mono text-[0.65rem] font-bold tracking-wide uppercase ${STRENGTH_BADGE[r.strength]}`}>
                        {r.strength}
                      </span>
                    </td>
                  </tr>
          ...
          <div className="my-6 rounded-md border border-border bg-card p-6">
            ...
            <div className="flex flex-col gap-3">
              {ESCALATION_SPECTRUM.map((level) => (
                <div key={level.level} className="border-b border-border/60 pb-3 last:border-b-0 last:pb-0">
            ...
            <p className="mt-4 text-sm leading-[1.8] text-foreground/70 italic">
              ...
              pattern is always the same: silence enables escalation. But the answer is never
              gossip — it&apos;s verified, documented, consequential truth.
            </p>
          </div>
        ...
        {/* ============ PLAYBOOK ============ */}
        <Section id="playbook" num="04" title="The Playbook — This Is Not an Accident. It Is a Strategy.">
          <P>
        ...
        {/* ============ TIMELINE ============ */}
        <Section id="timeline" num="05" title={`A ${SUBJECT.yearsOfPattern}-Year Paper Trail of Broken Promises`}>
          <div className="relative border-l-2 border-red-800/30 pl-6">
            {TIMELINE.map((item) => (
              <div key={item.date + item.title} className="relative mb-7">
                ...
                <h3 className="mb-1 font-heading text-base font-bold text-foreground">{item.title}</h3>
        ...
        <Section id="where" num="06" title={`Where in the World Is ${SUBJECT.name}?`}>
          <P>
            He moves. He networks. He speaks at conferences. He shakes hands. If you see him at
            any of these events, now you know what questions to ask — and to get the answers in
            writing.
          </P>
          <div className="grid gap-4 sm:grid-cols-2">
            {MAP_PINS.map((p) => (
              <div key={p.city + p.event} className="rounded-md border-l-4 border-l-red-800 bg-card p-4">
        ...
        <Section id="note" num="07" title="A Personal Note from Tony Greenberg">
          ...
          <P>
            But I took Alex on because a kind, trusted friend introduced us. He begged. He
            pleaded. He had genuine potential and a genuinely cool product. My team believed in
            him. We worked. We delivered. We made introductions that opened real doors.
          </P>
          <P>
            And then he decided that $200,000 in his bank account was more important than
            honoring a signed contract with the people who helped put him on the map.
          </P>
          <P>
            This isn&apos;t about the money anymore. This is about protecting the next person —
            the next kind friend, the next advisor, the next vendor — from the pattern. Because
            the smile never stops, even when he doesn&apos;t respond to your emails, even when he
            dodges your calls.
          </P>
          <OpinionBox>
            Shame on you, Alex. Not just to me — to your parents, to your community, to every
            person who invested their time, energy, and belief in you. Pay your bills. Say
            you&apos;re sorry. Figure it out.
          </OpinionBox>
        </Section>
                  ...
                  >
                    ...
                    <span
                      className={`rounded-sm px-2 py-0.5 font-mono text-[0.65rem] font-bold tracking-wide uppercase ${
                        item.status === "partial" ? "bg-amber-700/15 text-amber-800" : "bg-red-800/15 text-red-800"
                      }`}
                    >
                      {item.status}
                    </span>
        ...
        <Section id="evidence-collection" num="09" title="Has This Happened to You?">
          <P>
            We are not conducting a smear campaign. We are building a community evidence record.
            If you recognize this pattern from your own experience with {SUBJECT.name}, your story
          ...
          <div className="mt-10 rounded-md border border-red-800/20 bg-red-800/5 p-6 text-center">
            <h3 className="mb-2 font-heading text-xl font-bold text-foreground">Add to the Record</h3>
        ...
        {/* ============ HEALING FRAMEWORK ============ */}
        <Section id="healing" num="10" title="Protect Others. Heal Our Community. Become Who We Want to Become.">
          <P>
            ...
            outcome of the human race is, but we know that integrity, accountability, and
            collective protection are non-negotiable foundations.
          </P>
          <div className="grid gap-4 sm:grid-cols-3">
            {HEALING_FRAMEWORK.map((p) => (
              <div key={p.title} className="rounded-md border border-green-700/20 bg-green-700/5 p-5">
        ...
        <Section id="the-work" num="11" title="Doing The Work on Alex Azzi">
          ...
          {BYRON_KATIE_APPLIED.map((item) => (
            <div key={item.num} className="my-4 rounded-md border-l-4 border-l-amber-700 bg-card p-6">
        ...
        <Section id="about" num="12" title={`Who Is ${SUBJECT.name}?`}>
          <P>
            ...
            XRWorkout and the founder of Biohackers UAE. He has appeared as a speaker at
            technology and startup conferences across the Middle East and Europe.
          </P>
          <div className="my-6 grid gap-3 rounded-md border border-border bg-card p-6 sm:grid-cols-2">
            {KEY_FACTS.map((fact) => (
              <div key={fact.label} className="border-l-2 border-l-brand-gold pl-3">
                <div className="font-mono text-xs tracking-wide text-muted-foreground uppercase">{fact.label}</div>
          ...
          <AlertBox>
            ...
            {SUBJECT.totalDocumented} in unpaid invoices and unfulfilled equity agreements
            spanning {SUBJECT.yearsOfPattern} years. The documentation includes signed contracts,
            timestamped invoices, and email correspondence.
          </AlertBox>
        ...
        <Section id="reading" num="13" title="This Is Part of a Larger Journey">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {ECOSYSTEM_LINKS.map((link) => (
              ...
              >
                <div className="mb-1 font-mono text-xs tracking-wide text-muted-foreground uppercase">{link.tag}</div>
