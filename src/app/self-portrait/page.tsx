...
import { parseAssessmentResults } from "@/lib/assessments/result-log";
import {
  SELF_PORTRAIT_CATALOG,
  SELF_PORTRAIT_CATALOG_BY_SLUG,
} from "@/lib/content/self-portrait-catalog";
import { getVisitorState } from "@/lib/visitor-state";
...
// subset: a genuine completed-assessments log read server-side from managed
// visitor state — with no client hydration flash — plus a
...
export default async function SelfPortraitPage() {
  const resultState = await getVisitorState("assessment-results");
  const completed = parseAssessmentResults(resultState?.results).sort((a, b) =>
  ...
  return (
    <div className="relative z-1 min-h-screen">
      ...
      <div className="relative z-1 mx-auto max-w-135 px-6 pt-[clamp(6rem,10vw,8rem)] pb-12 text-center">
        <h1 className="mb-2 font-heading text-[clamp(1.8rem,4vw,2.4rem)] leading-tight font-normal text-foreground">
          Your Self-Portrait
        </h1>
        <p className="mx-auto max-w-105 text-base leading-relaxed text-muted-foreground">
          Every assessment you&apos;ve completed, in one place — a running record of what
        ...
        {completed.length === 0 ? (
          <div className="rounded-2xl border border-brand-gold/15 bg-background/85 p-8 text-center backdrop-blur-sm">
            <p className="mb-4 text-[0.95rem] leading-relaxed text-foreground/80">
              Nothing here yet. Complete an assessment and it&apos;ll show up on this page
              automatically — no account, no login, just this browser.
            </p>
            <Link
              href="/find-my"
            ...
            {completed.map((entry) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="inline-flex shrink-0 items-center gap-1 font-mono text-xs text-brand-gold">
                    Retake
        ...
        <section className="relative z-1 mx-auto mt-10 max-w-130 px-6">
          ...
          <div className="flex flex-col gap-2.5">
            {remaining.map((entry) => (
              ...
              >
                <div className="min-w-0 flex-1 text-left">
                  <div className="font-heading text-[0.95rem] leading-tight font-medium text-foreground">
                    {entry.title}
                  </div>
