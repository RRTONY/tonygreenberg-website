...
function StatusBadge({ status }: { status: "live" | "coming" }) {
  ...
  return <span className="shrink-0 rounded-full bg-muted px-2 py-0.5 font-mono text-[0.55rem] tracking-[0.1em] text-muted-foreground uppercase">Coming Soon</span>;
}
  ...
  return (
    <div className="relative z-1 min-h-screen">
      ...
      <div className="relative z-1 mx-auto max-w-135 px-6 pt-[clamp(6rem,10vw,8rem)] pb-24 text-center">
        ...
        <div className="mx-auto mt-5 max-w-120 rounded-xl border border-brand-gold/12 bg-brand-gold/4 p-6 text-left">
          <p className="mb-2.5 text-[0.85rem] leading-relaxed text-foreground/80">
            <strong className="text-brand-gold">What you&apos;re looking at:</strong> Every assessment Tony has built — from identity and relationships to
            health, consciousness, and impact — organized in one place. Each one takes 5–10 minutes and gives you an immediate, personalized result.
          </p>
          <p className="mb-2.5 text-[0.85rem] leading-relaxed text-foreground/80">
            <strong className="text-brand-gold">Why it matters:</strong> Most people make major life decisions — what to eat, who to trust, how to heal —
            with borrowed frameworks or no framework at all. These tools give you your own data about yourself.
          </p>
          <p className="text-[0.85rem] leading-relaxed text-foreground/80">
            <strong className="text-brand-gold">What to do:</strong> Start with a Featured assessment, or scroll by category. Your progress bar tracks
            which ones you&apos;ve completed. No login required.
          </p>
        </div>

        <div className="mt-3 font-mono text-[0.68rem] tracking-[0.15em] text-muted-foreground uppercase">5–10 min each · Free · Instant results</div>
      </div>
      ...
      <section className="relative z-1 mx-auto mt-12 max-w-130 px-6">
        <div className="mb-4 text-center font-mono text-[0.68rem] tracking-[0.18em] text-brand-gold uppercase">Featured</div>
      ...
      <section className="relative z-1 mx-auto mt-12 max-w-130 px-6 text-center">
        <div className="rounded-2xl border border-brand-gold-light/20 bg-[#0A0A10] p-8">
          <div className="mb-3 font-mono text-[0.68rem] tracking-[0.18em] text-brand-gold-light uppercase">Your Running Record</div>
          <h2 className="mb-2 font-heading text-[1.4rem] font-normal text-background">Self-Portrait</h2>
          <p className="mb-5 text-[0.9rem] leading-relaxed text-[#999]">
            Every assessment you&apos;ve completed, in one place — tracked automatically in this browser, no login required.
          </p>
          <Link
            href="/self-portrait"
