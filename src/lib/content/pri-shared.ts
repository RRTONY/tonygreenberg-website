...
export function getMedicineWithSafety(m: Medicine): MedicineWithSafety {
  const safety = SAFETY_DATA[m.id] || {
    ...
    safetyWarning:
      "Safety data is being compiled. Always consult a qualified healthcare professional.",
...
export const READINESS_LEVELS: ReadinessLevel[] = [
  {
    threshold: 85,
    label: "Ready to Go Deep",
    description:
      "You have built something real. The high-intensity work... ayahuasca, iboga, 5-MeO... is accessible to you with the right people. Do not skip the facilitation. The container matters as much as the medicine.",
  },
  {
    threshold: 70,
    label: "Strong Ground",
    description:
      "Most of the foundation is there. Psilocybin, MDMA therapy, San Pedro... these are honest next steps. A few dimensions are worth tending before you go further. You are closer than you think.",
  },
  {
    threshold: 55,
    label: "Building Toward It",
    description:
      "The container is not quite ready yet. Start with lower-intensity medicines. Do the preparation work like it matters. Because it does. Every single time.",
  },
  {
    threshold: 40,
    label: "Not Yet. And That Is Honest.",
    description:
      "The groundwork is still ahead of you. Breathwork, microdosing, somatic practice, a real integration community. These are not consolation prizes. They are the work. The actual work.",
  },
  {
    threshold: 0,
    label: "Start Here",
    description:
      "This is where it begins. And beginning here is the right call. Cacao ceremony, breathwork, meditation, intentional cannabis. Build from the ground up. The deeper work will wait for you. It always does.",
  },
];
...
export function computeScores(answers: number[]): {
  ...
  (Object.keys(dims) as DimKey[]).forEach((d) => {
    dimScores[d] =
      dims[d].length > 0
        ? Math.round((dims[d].reduce((a, b) => a + b, 0) / dims[d].length) * 10)
        : 50;
  });
  ...
  const level =
    READINESS_LEVELS.find((l) => overall >= l.threshold) ||
      ...
      : overall >= 55
        ? [
            ...
            },
            {
              ...
              cost: top.pricing[1]?.amount || "$1,000–$4,000",
            },
          ]
        : [
            {
